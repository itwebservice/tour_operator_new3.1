...
function package_name_validation(id) {

	var pass1 = document.getElementById(id).value;
	if (!/^[a-zA-Z0-9\s]+$/.test(pass1)) {
		error_msg_alert('It should not allow special character.');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
...
function get_to_datetime(from_date, to_date) {
	...
	if (from_date1 != '') {
		...
		if (typeof edatetime[2] === 'undefined') {
...
function validate_spattration(id) {
	// AI Builder quotations can have longer special attraction text — skip char limit
	if ((typeof jQuery !== 'undefined' && $('#is_ai_quotation').length && $('#is_ai_quotation').val() === '1')
		|| (typeof sessionStorage !== 'undefined' && sessionStorage.getItem('is_ai_quotation') === '1')) {
		if (id && typeof jQuery !== 'undefined') {
			$('#' + id).css({ border: '1px solid #ddd' });
		}
		return true;
	}
...
function validate_career(id) {
    var obj = document.getElementById(id).value.trim(); // Trim leading & trailing spaces
    obj = obj.replace(/<[^>]*>/g, ''); // Remove HTML tags
    obj = obj.replace(/\&nbsp;/g, ''); // Remove non-breaking spaces

    if (obj.length > 1000) {
        error_msg_alert('Character limit is 1000 characters');
        $('#' + id).css({ border: '1px solid red' }).focus();
        g_validate_status = false;
        return false;
    } else {
        $('#' + id).css({ border: '1px solid #ddd' });
        g_validate_status = true; // Ensure validation status is updated
        return true;
    }
}


// Validation for career text fields

function validate_career_1(id) {
	var obj = document.getElementById(id).value;
	obj = obj.replace(/^\s+|\s+$/g, '');
	obj = obj.replace(/<(.|\n)*?>/g, '');
	obj = obj.replace(/\&nbsp;/g, '');
	if (obj.length > 100) {
		error_msg_alert('Character limit is 100 characters');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
		return false;
...
function validate_services(id) {
	var obj = document.getElementById(id).value;
	obj = obj.replace(/^\s+|\s+$/g, '');
	obj = obj.replace(/<(.|\n)*?>/g, '');
	obj = obj.replace(/\&nbsp;/g, '');
	if (obj.length > 300) {
		error_msg_alert('Character limit is 300 characters');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
		return false;
...
function validate_services_1(id) {
	var obj = document.getElementById(id).value;
	obj = obj.replace(/^\s+|\s+$/g, '');
	obj = obj.replace(/<(.|\n)*?>/g, '');
	obj = obj.replace(/\&nbsp;/g, '');
	if (obj.length > 50) {
		error_msg_alert('Character limit is 50 characters');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
		return false;
...
function validate_about(id) {
	var obj = document.getElementById(id).value;
	obj = obj.replace(/^\s+|\s+$/g, '');
	obj = obj.replace(/<(.|\n)*?>/g, '');
	obj = obj.replace(/\&nbsp;/g, '');
	if (obj.length > 500) {
		error_msg_alert('Character limit is 500 characters');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
		return false;
...
//Validation for terms_conditions
function validate_terms_conditions(id) {
	var obj = document.getElementById(id).value;
	obj = obj.replace(/^\s+|\s+$/g, '');
	obj = obj.replace(/<(.|\n)*?>/g, '');
	obj = obj.replace(/\&nbsp;/g, '');
	if (obj.length > 2000) {
		error_msg_alert('Character limit is 2000 characters');
		$('#' + id).css({ border: '1px solid red' });
		$('#' + id).focus();
		g_validate_status = false;
		return false;
...
function ValidateIPaddress(inputText, multiple) {

	var ipAdress = $('#' + inputText).val();
	var flag = false;
	if (ipAdress != '') {
		if (multiple === true) {
			var ipAdresses = ipAdress.split(',');
		} else {
			var ipAdresses = ipAdress;
		}
...
function blockSpecialChar_json(e) {
	var k = e.keyCode;
	return (k!=39);
}
