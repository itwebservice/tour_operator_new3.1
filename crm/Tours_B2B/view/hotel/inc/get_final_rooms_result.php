    ...
    function get_result_array($cost_arr, $date_array_size)
    {
            ...
            for ($j = $i + 1; $j <= $i + 1; $j++) {
                if (isset($cost_arr[$j]['rooms']['category']) && $cost_arr[$i]['rooms']['category'] == $cost_arr[$j]['rooms']['category'] && $cost_arr[$i]['rooms']['room_count'] == $cost_arr[$j]['rooms']['room_count']) {
                    ...
                    for ($k = 0; $k < sizeof($cost_arr[$i]['rooms']); $k++) {
                        $temp_child_cost = isset($cost_arr[$i]['rooms']['child_cost'][$k]) ? $cost_arr[$i]['rooms']['child_cost'][$k] : 0;
                        $child_cost += $temp_child_cost;
                    }
                    $cost_arr1 = array(
                        'rooms' => array(
                    ...
                    $cost_arr2 = array(
                        'rooms' => array(
                            ...
                            "max_occupancy" =>   $cost_arr[$i]['rooms']['max_occupancy'],
                            "markup_type" =>     $cost_arr[$i]['rooms']['markup_type'],
                            "markup_amount" =>   (float)($cost_arr[$i]['rooms']['markup_amount']),
                            "offer_type" =>     $cost_arr[$i]['rooms']['offer_type'],
                            "offer_amount" =>   (float)($cost_arr[$i]['rooms']['offer_amount']),
                            "offer_in" =>       $cost_arr[$i]['rooms']['offer_in'],
                            "coupon_code" =>    $cost_arr[$i]['rooms']['coupon_code'],
                            "agent_type" =>     $cost_arr[$i]['rooms']['agent_type'],
                            "currency_id" =>    $cost_arr[$i]['rooms']['currency_id']
        ...
        for ($i = 0; $i < sizeof($category_array1); $i++) {
            ...
            for ($j = 0; $j < sizeof($category_array1[$i]); $j++) {
                ...
                array_push($room_cost_array, (float)($category_array1[$i][$j]['room_cost']));
                array_push($child_cost_array, (float)($category_array1[$i][$j]['child_cost']));
                array_push($extra_bed_cost_array, (float)($category_array1[$i][$j]['extra_bed_cost']));
