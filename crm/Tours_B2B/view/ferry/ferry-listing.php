        ...
        if ($sq_tariff_count > 0) {
          ...
          while (($row_tariff  = mysqli_fetch_assoc($sq_tariff))) {
            ...
            if ($row_tariff['markup_in'] == 'Flat') {
              ...
              $adult_markup = ((float)($row_tariff['adult_cost']) * $row_tariff['markup_cost'] / 100);
            }
            $adult_cost = (float)($row_tariff['adult_cost']) + $adult_markup;
            // ///////Child///////////
            if ($row_tariff['markup_in'] == 'Flat') {
              $child_markup = $row_tariff['markup_cost'];
            } else {
              $child_markup = ((float)($row_tariff['child_cost']) * $row_tariff['markup_cost'] / 100);
            }
            $child_cost = (float)($row_tariff['child_cost']) + $child_markup;
            // ///////Infant///////////
            if ($row_tariff['markup_in'] == 'Flat') {
              $infant_markup = $row_tariff['markup_cost'];
            } else {
              $infant_markup = ((float)($row_tariff['infant_cost']) * $row_tariff['markup_cost'] / 100);
            }
            $infant_cost = (float)($row_tariff['infant_cost']) + $infant_markup;
            ...
            array_push($ferry_result_array, array(
              ...
              "seating_capacity" => intval($row_tariff['no_of_seats']),
              'image' => $newUrl,
              ...
              'adult_cost' => (float)($row_tariff['adult_cost']),
              'child_cost' => (float)($row_tariff['child_cost']),
              'infant_cost' => (float)($row_tariff['infant_cost']),
              'currency_id' => $row_tariff['currency_id'],
              'total_cost' => (float)($total_cost1),
              'adult_cost' => (float)($adult_cost1),
              'child_cost' => (float)($child_cost1),
              'infant_cost' => (float)($infant_cost1),
              "best_lowest_cost" => array(
                'id' => intval($row_tariff['currency_id']),
                'cost' => (float)($total_cost1)
              ),
              "travel_date" => $date1,
...
<script>
  ...
  function get_price_filter_data_result(final_arr) {
    var base_url = $('#base_url').val();
    $.post(base_url + 'Tours_B2B/view/ferry/ferry_results.php', {
      final_arr: final_arr
    }, function(data) {
