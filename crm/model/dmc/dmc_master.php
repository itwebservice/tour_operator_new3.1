...
public function dmc_save()
{
  ...
  $company_name = addslashes($company_name);
	$sq_dmc_count = mysqli_num_rows(mysqlQuery("select * from dmc_master where company_name='$company_name'"));
	if($sq_dmc_count>0){
		rollback_t();
  ...
  $dmc_address = mysqlREString($dmc_address);
	...
	if($sq_dmc){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "DMC Supplier has been successfully saved.";
	      exit;
	    }
	    else{
	      rollback_t();
		...
		rollback_t();
		echo "error--DMC Supplier not saved!";
		exit;
	}
}
...
public function dmc_update()
{
  ...
  $email_id = $encrypt_decrypt->fnEncrypt($email_id, $secret_key);
	begin_t();
  $company_name = addslashes($company_name);
	$sq_dmc_count = mysqli_num_rows(mysqlQuery("select * from dmc_master where company_name='$company_name' and dmc_id!='$dmc_id'"));
	if($sq_dmc_count>0){
		rollback_t();
  ...
  $dmc_address = mysqlREString($dmc_address);
	...
	if($sq_dmc){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "DMC Supplier has been successfully updated.";
	      exit;
	    }
	    else{
	      rollback_t();
		...
		rollback_t();
		echo "error--DMC Supplier not updated!";
		exit;
	}
}
    ...
    if($handle !== false) {
        ...
        while(($data = fgetcsv($handle, 4000,",")) !== FALSE){
            ...
            if($count>0){
                ...
                $sq_max_id = mysqli_fetch_assoc(mysqlQuery("select max(dmc_id) as max from dmc_master"));
                ...
                if(preg_match('/^[0-9]*$/', $city_id) && preg_match('/^[a-zA-Z \s]*$/', $dmc_name) && preg_match('/^[0-9]*$/', $state_id) && (strlen($mobile)<=20)){

                      $sq_dmc_count = mysqli_num_rows(mysqlQuery("select * from dmc_master where company_name='$dmc_name'"));
                      if($sq_dmc_count==0)
                      {
                          $validCount++;
                          global $encrypt_decrypt, $secret_key;
