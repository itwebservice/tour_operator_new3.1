...
public function refund_mail_send($booking_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
	...
	$sq_ref_pay_total = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from b2b_booking_refund_master where booking_id='$booking_id' and clearance_status!='Cancelled'"));
	...
	if(sizeof($group_list_arr)>0){
		...
		for($i=0;$i<sizeof($group_list_arr);$i++){
			...
			for($j=0;$j<count(array($services));$j++){
				...
				$gransfer_total += $room_cost1;
				$grtax_total += $tax_amount1;
			}
		} 
	}
	
	$main_total += $hotel_total + $transfer_total + $activity_total + $tours_total + $ferry_total + $gransfer_total;
	$main_tax_total += $tax_total + $trtax_total + $acttax_total + $tourstax_total + $ferrytax_total + $grtax_total;
	...
	$remaining = $sq_b2b_info['total_refund_amount'] - $refund_paid_amount;
