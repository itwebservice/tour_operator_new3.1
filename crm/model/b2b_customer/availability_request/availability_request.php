...
class b2b_request{
    function availability_request(){
        ...
        $sq_timestamp = mysqli_num_rows(mysqlQuery("select request_id from hotel_availability_request where unique_timestamp='$unique_timestamp'"));
        if($sq_timestamp == 0){

            $sq_reg = mysqli_fetch_assoc(mysqlQuery("select company_name,cp_first_name,cp_last_name,mobile_no,email_id from b2b_registration where register_id='$register_id'"));
            $guest_name = $sq_reg['cp_first_name'].' '.$sq_reg['cp_last_name'];

            $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(request_id) as max from hotel_availability_request"));
            $request_id = $sq_max['max'] + 1;
            $sq_request = mysqlQuery("insert into hotel_availability_request (request_id,register_id,cust_name,concern_person,mobile_no,email_id,cart_data,status,created_at,unique_timestamp,user_id,agent_flag) values ('$request_id','$register_id','$sq_reg[company_name]','$guest_name','$sq_reg[mobile_no]','$sq_reg[email_id]','$hotel_list_arr','false','$date','$unique_timestamp','$user_id','$agent_flag')");

            if($sq_request){
                $this->request_mail_toAdmin($request_id,$sq_reg['company_name'],$hotel_list_arr);
                mysqlQuery("update b2b_login set request_id='$request_id' where user_id='$user_id' and agent_flag='$agent_flag'");
    ...
    function response_save(){
        ...
        $sq_res = mysqlQuery("update hotel_availability_request set response='$response_arr' where request_id='$request_id'");
        if($sq_res){
    ...
    function request_delete(){
        ...
        $sq_delete = mysqlQuery("update hotel_availability_request set active_status='disabled' where request_id='$request_id'");
        if($sq_delete){
    ...
    function request_mail_toAdmin($request_id,$company_name,$cart_data){
        ...
        for($i=0;$i<sizeof($cart_data);$i++){
            ...
            $sq_hotel = mysqli_fetch_assoc(mysqlQuery("select city_id,mobile_no,email_id from hotel_master where hotel_id='$hotel_id'"));
            if($city_id != ''){
    ...
    function request_mail_toSupplier(){
        ...
        $sq_req = mysqli_fetch_assoc(mysqlQuery("select cart_data from hotel_availability_request where request_id='$request_id'"));
        ...
        for($i=0;$i<sizeof($cart_data);$i++){
            ...
            $sq_hotel = mysqli_fetch_assoc(mysqlQuery("select city_id,mobile_no,email_id,alternative_email_1,alternative_email_2 from hotel_master where hotel_id='$hotel_id'"));
        ...
        mysqlQuery("update hotel_availability_request set supplier_mail='1' where request_id='$request_id'");
    ...
    function response_mail_toAgent(){
        ...
        $sq_avail = mysqli_fetch_assoc(mysqlQuery("select cart_data,response from hotel_availability_request where request_id='$request_id'"));
        $cart_data = ($sq_avail['cart_data']!='')?json_decode($sq_avail['cart_data']):[];
        $response = ($sq_avail['response']!='')?json_decode($sq_avail['response']):[];
        if(sizeof($cart_data) == sizeof($response)){
                    ...
                    <tr>
                        ...
                        if($item->status=='Not Available' && sizeof($item->options)!=0){
                            ...
                            for($hi=0;$hi<sizeof($item->options);$hi++){
                                ...
                                $sq_ophotel = mysqli_fetch_assoc(mysqlQuery("select hotel_name,hotel_id,city_id from hotel_master where hotel_id='$option_hotel_id'"));
                                $sq_opcity = mysqli_fetch_assoc(mysqlQuery("select city_name,city_id from city_master where city_id='$sq_ophotel[city_id]'"));
                                $content .= '
            ...
            $model->app_email_send('118',$sq_reg['company_name'],$sq_reg['email_id'], $content,$subject,'1');
    ...
    function supplier_response_save(){
        ...
        $sq_res = mysqlQuery("update hotel_availability_request set response='$response_arr',response_received='$response_received' where request_id='$request_id'");
        if($sq_res){
    ...
    function supplier_response_toAdmin($request_id,$response_arr){
        ...
        $sq_avail = mysqli_fetch_assoc(mysqlQuery("select cart_data,response,register_id from hotel_availability_request where request_id='$request_id'"));
        $cart_data = json_decode($sq_avail['cart_data']);
        for($i=0;$i<sizeof($cart_data);$i++){
            array_push($hotel_id_arr,$cart_data[$i]->id);
        }

        $sq_reg = mysqli_fetch_assoc(mysqlQuery("select company_name from b2b_registration where register_id='$sq_avail[register_id]'"));
