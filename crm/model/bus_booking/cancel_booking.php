...
public function cancel_booking_save()
{
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
		$sq_cancel = mysqlQuery("update bus_booking_entries set status='Cancel' where entry_id='$entry_id_arr[$i]'");
		if(!$sq_cancel){
			echo "error--Sorry, Bus Booking Cancellation has been successfully done!";
			exit;
		}
	}
	...
	echo "Bus Booking Cancellation has been successfully done!";
	exit;
...
public function cancel_mail_send($entry_id_arr)
{
	...
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from bus_booking_entries where entry_id='$entry_id_arr[0]'"));
	$sq_bus_info = mysqli_fetch_assoc(mysqlQuery("select * from bus_booking_master where booking_id='$sq_entry[booking_id]' and delete_status='0'"));
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++)
	{
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from bus_booking_entries where entry_id='$entry_id_arr[$i]'"));
	...
	$subject = 'Bus Booking Cancellation Confirmation('.get_bus_booking_id($sq_entry['booking_id'],$year).' )';
	global $model;
	$model->app_email_send('36',$cust_name,$email_id , $content, $subject);
}
