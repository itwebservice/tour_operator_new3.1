...
public function refund_save()
{
	...
	if($refund_mode == 'Credit Note'){
		...
		$sq_payment = mysqlQuery("insert into credit_note_master (id, financial_year_id, module_name, module_entry_id, customer_id, payment_amount,refund_id,created_at,branch_admin_id) values ('$id', '$financial_year_id', 'Bus Booking', '$booking_id', '$customer_id','$refund_amount','$refund_id','$refund_date','$branch_admin_id') ");
	}
	if(!$sq_payment){
		$GLOBALS['flag'] = false;
...
public function refund_mail_send($booking_id,$refund_amount,$refund_date,$refund_mode,$transaction_id)
{
	...
	$sq_sq_bus_info = mysqli_fetch_assoc(mysqlQuery("select * from bus_booking_master where booking_id='$booking_id'"));
	$date = $sq_sq_bus_info['created_at'];
	$yr = explode("-", $date);
	$year =$yr[0];
	$cust_email = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_sq_bus_info[customer_id]'"));
	...
	$sq_refund_sum = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from bus_booking_refund_master where booking_id='$booking_id' and clearance_status!='Cancelled'"));

	$net_total = $sq_sq_bus_info['net_total'];
	$paid_amount = $sq_payment_sum['sum'];
	$cancel_amount = $sq_sq_bus_info['cancel_amount'];

	$remaining = $sq_sq_bus_info['refund_net_total']- $sq_refund_sum['sum'];
	...
	<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
		...
		<tr><td style="text-align:left;border: 1px solid #888888;">Selling Amount</td>   <td style="text-align:left;border: 1px solid #888888;">'.$net_total.'</td></tr>
	...
	$model->app_email_send('37',$cust_name,$email_id, $content, $subject);
}
