...
public function vendor_save()
{
	...
	$sq_count = mysqli_num_rows(mysqlQuery("select * from site_seeing_vendor where vendor_name='$vendor_name'"));
	if($sq_count>0){
		rollback_t();
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Activity supplier has been Successfully Saved.";
	      exit;
	    }
	    else{
	      rollback_t();     
		...
		rollback_t();
		echo "Activity Supplier Not Saved!";
		exit;
	}
}
public function vendor_update()
{
	...
	$email_id_1 = $encrypt_decrypt->fnEncrypt($email_id_1, $secret_key);
    $email_id_2 = $encrypt_decrypt->fnEncrypt($email_id_2, $secret_key);
	$sq_count = mysqli_num_rows(mysqlQuery("select * from site_seeing_vendor where vendor_name='$vendor_name' and vendor_id!='$vendor_id'"));
	if($sq_count>0){
		rollback_t();
		echo "error--Vendor name already exists!";
		exit;
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Activity Supplier has been successfully updated.";
	      exit;
	    }
	    else{
	      rollback_t();
		...
		rollback_t();
		echo "Activity vendor not updated!";
		exit;
	}
