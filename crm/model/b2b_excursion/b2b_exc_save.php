...
public function service_save(){
	...
	$photo_upload_url = isset($_POST['photo_upload_url']) ? $_POST['photo_upload_url'] : '';

	$transfer_option_array = isset($_POST['transfer_option_array']) ? $_POST['transfer_option_array']: [];
	$bfrom_date_array = isset($_POST['bfrom_date_array']) ? $_POST['bfrom_date_array']: [];
	$bto_date_array = isset($_POST['bto_date_array']) ? $_POST['bto_date_array']: [];
	$adult_cost_array = isset($_POST['adult_cost_array']) ? $_POST['adult_cost_array']: [];
	...
	$type_array = isset($_POST['type_array'])?$_POST['type_array']:[]; 
	$from_date_array = isset($_POST['from_date_array']) ? $_POST['from_date_array'] : [];
	$to_date_array = isset($_POST['to_date_array']) ? $_POST['to_date_array']: [];
	$offer_in_array = isset($_POST['offer_in_array']) ? $_POST['offer_in_array']: [];
	$coupon_code_array = isset($_POST['coupon_code_array']) ? $_POST['coupon_code_array']: [];
	$offer_array = isset($_POST['offer_array']) ? $_POST['offer_array']: [];
	$agent_type_array = isset($_POST['agent_type_array']) ? $_POST['agent_type_array']: [];

	$sq_count = mysqli_num_rows(mysqlQuery("select entry_id from excursion_master_tariff where excursion_name='$service_name' and city_id='$city_id'"));
	if($sq_count>0){
		echo "error--Sorry, Activity name already exists!";
		exit;
	}

	$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_master_tariff"));
	$exc_id = $sq_max['max'] + 1;
	...
	if($sq_service){
		...
		if($photo_upload_url!=''){
			...
			for($i=0;$i<sizeof($image_array);$i++){
				$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_master_images"));
				$entry_ids = $sq_max['max'] + 1;
				$sq_image = mysqlQuery("insert into excursion_master_images (entry_id, exc_id, image_url) values ('$entry_ids', '$exc_id', '$image_array[$i]')");
				if(!$sq_image){
					echo "error--Sorry, Activity Images not saved!";
					exit;
				}
			}
		}
		...
		for($i=0;$i<sizeof($bfrom_date_array);$i++){
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_master_tariff_basics"));
		...
		for($i=0;$i<sizeof($type_array);$i++){
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_master_offers"));
			$offer_entry = $sq_max['max'] + 1;
			$from_date_array[$i] = get_date_db($from_date_array[$i]);
			$to_date_array[$i] = get_date_db($to_date_array[$i]);
			$sq_offer = mysqlQuery("INSERT INTO `excursion_master_offers`(`entry_id`, `exc_id`, `type`, `from_date`, `to_date`, `offer_in`,`coupon_code`, `offer_amount`, `agent_type`) VALUES ('$offer_entry', '$exc_id','$type_array[$i]','$from_date_array[$i]','$to_date_array[$i]','$offer_in_array[$i]','$coupon_code_array[$i]','$offer_array[$i]','$agent_type_array[$i]')");
			if(!$sq_offer){
				echo "error--Sorry, Activity Offers/Coupon not saved!";
				exit;
			}
		}

		echo "Activity has been successfully saved.";
		exit;
	}
	else{
		echo "error--Sorry, Activity not saved!";
		exit;
	}
}
public function service_update(){
	...
	$offers_array = isset($_POST['offers_array']) ? $_POST['offers_array'] : [];

	$service_name = addslashes($service_name);
	$sq_count = mysqli_num_rows(mysqlQuery("select entry_id from excursion_master_tariff where excursion_name='$service_name' and city_id='$city_id' and entry_id!='$exc_entry_id'"));
	if($sq_count>0){
		echo "error--Sorry, Activity name already exists!";
		exit;
	}
	...
	$sq_service = mysqlQuery("update `excursion_master_tariff` set  `city_id`='$city_id', `excursion_name`= '$service_name', `duration`='$duration', `departure_point`='$dep_point', `rep_time`='$rep_time', `description`='$description', `note`='$note', `inclusions`='$inclusions', `exclusions`='$exclusions', `terms_condition`='$terms', `useful_info`='$upolicy', `booking_policy`='$bpolicy', `canc_policy`='$cpolicy', `currency_code`='$currency_code', `taxation`='$taxation', `active_flag`='$active_flag',`off_days`='$off_days' where entry_id='$exc_entry_id'");
	...
	if($sq_service){
		...
		for($i=0;$i<sizeof($bfrom_date_array);$i++){
			if($cost_array[$i] == 'true'){
				...
				$sq_query1 = mysqlQuery("DELETE FROM `excursion_master_tariff_basics` WHERE `entry_id`='$basic_entryid_array[$i]'");
			}
		}
		...
		for($i=0;$i<sizeof($type_array);$i++){
			if($offers_array[$i] == 'true'){
				if($offer_entryid_array[$i] == ''){
					...
					$from_date_array[$i] = get_date_db($from_date_array[$i]);
					$to_date_array[$i] = get_date_db($to_date_array[$i]);
					$sq_offer = mysqlQuery("INSERT INTO `excursion_master_offers`(`entry_id`, `exc_id`, `type`, `from_date`, `to_date`, `offer_in`,`coupon_code`, `offer_amount`, `agent_type`) VALUES ('$offer_entry', '$exc_entry_id','$type_array[$i]','$from_date_array[$i]','$to_date_array[$i]','$offer_in_array[$i]','$coupon_code_array[$i]','$offer_array[$i]','$agent_type_array[$i]')");
				}
				else{
					$from_date_array[$i] = get_date_db($from_date_array[$i]);
					$to_date_array[$i] = get_date_db($to_date_array[$i]);
					$sq_offer = mysqlQuery("update `excursion_master_offers` set `type`='$type_array[$i]',`from_date`='$from_date_array[$i]', `to_date`='$to_date_array[$i]', `offer_in`='$offer_in_array[$i]',`coupon_code`='$coupon_code_array[$i]', `offer_amount`='$offer_array[$i]', `agent_type`='$agent_type_array[$i]' where entry_id='$offer_entryid_array[$i]'");
				}
				if(!$sq_offer){
					echo "error--Sorry, Activity Offers/Coupons not updated!";
					exit;
				}
			}else{
				$sq_query1 = mysqlQuery("DELETE FROM `excursion_master_offers` WHERE `entry_id`='$offer_entryid_array[$i]'");
			}
		}

		echo "Activity has been successfully updated.";
		exit;
	}
	else{
		echo "error--Sorry, Activity not updated!";
		exit;
	}
}
...
public function image_delete(){
    ...
    $sq_delete = mysqlQuery("delete from excursion_master_images where entry_id='$image_id'");
    if($sq_delete){
		echo "Image Deleted";
    }
}
...
function image_update(){
	...
	//Activity Images
	$sq_count=mysqli_num_rows(mysqlQuery("select * from excursion_master_images where exc_id='$exc_entry_id'"));
	if($sq_count<3)
	{
		$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_master_images"));
		$entry_ids = $sq_max['max'] + 1;
		$sq_image = mysqlQuery("insert into excursion_master_images (entry_id, exc_id, image_url) values ('$entry_ids', '$exc_entry_id', '$upload_url')");
		if(!$sq_image){
...
function time_slot_update(){
	...
	$sq_service = mysqlQuery("update `excursion_master_tariff` set `timing_slots`='$time_slot_array' where entry_id='$exc_entry_id'");
	if($sq_service){
