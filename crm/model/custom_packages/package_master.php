...
function package_master_update($package_id1,$package_code,$package_name,$total_days,$total_nights,$inclusions,$exclusions, $status ,$city_name_arr, $hotel_name_arr, $hotel_type_arr,$total_days_arr,$hotel_check_arr,$vehicle_name_arr,$vehicle_check_arr,$drop_arr,$drop_type_arr,$pickup_arr,$pickup_type_arr,$tr_entry_arr,$checked_programe_arr, $day_program_arr,$special_attaraction_arr,$overnight_stay_arr,$meal_plan_arr,$day_image_arr, $entry_id_arr,$hotel_entry_id_arr,$adult_cost,$child_cost,$infant_cost,$child_with,$child_without,$extra_bed,$currency_id,$note,$dest_image, $seo_slug,$tour_theme){
   ...
   if ($seo_slug) {
      $sq_query_count = mysqli_num_rows(mysqlQuery("select * from custom_package_master where (seo_slug = '$seo_slug' and package_id != '$package_id')"));

      if ($sq_query_count > 0) {
         $GLOBALS['flag'] = false;
         echo "error--This seo friendly slug is already taken.";
         exit;
      }
   }
   ...
   $exclusions = addslashes($exclusions);
   $note = addslashes($note);
   $sq = mysqlQuery("update custom_package_master set package_code ='$package_code', package_name = '$package_name',  seo_slug = '$seo_slug', tour_theme = '$tour_theme', total_days = '$total_days', total_nights = '$total_nights',adult_cost='$adult_cost',child_cost='$child_cost',infant_cost='$infant_cost',child_with='$child_with',child_without='$child_without',extra_bed='$extra_bed',inclusions='$inclusions',exclusions ='$exclusions', status ='$status', currency_id='$currency_id',taxation='$taxation',note='$note',dest_image='$dest_image' where package_id = '$package_id'");
   global $b2c_flag;
   if($b2c_flag == '1'){
