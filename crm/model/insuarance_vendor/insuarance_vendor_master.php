...
public function vendor_save()
{
	...
	$sq_count = mysqli_num_rows(mysqlQuery("select * from insuarance_vendor where vendor_name='$vendor_name'"));
	if($sq_count>0){
		rollback_t();
	...
	$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(vendor_id) as max from insuarance_vendor"));
	$vendor_id = $sq_max['max'] + 1;
	$sq_vendor = mysqlQuery("insert into insuarance_vendor (vendor_id, vendor_name, mobile_no, landline_no, email_id, concern_person_name, immergency_contact_no, opening_balance, address, website,  bank_name, account_name,account_no, branch, ifsc_code, active_flag, created_at, service_tax_no,state_id,side,pan_no,as_of_date) values ('$vendor_id', '$vendor_name', '$mobile_no', '$landline_no', '$email_id', '$concern_person_name', '$immergency_contact_no','$opening_balance', '$address','$website', '$bank_name','$account_name','$account_no','$branch','$ifsc_code', '$active_flag', '$created_at', '$service_tax_no','$state','$side','$supp_pan','$as_of_date')");
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Insurance Supplier has been successfully saved.";
	      exit;
	    }
	    else{
	      rollback_t();     
...
public function vendor_update()
{
	...
	$mobile_no = $encrypt_decrypt->fnEncrypt($mobile_no, $secret_key);
	$email_id = $encrypt_decrypt->fnEncrypt($email_id, $secret_key);
	$sq_count = mysqli_num_rows(mysqlQuery("select * from insuarance_vendor where vendor_name='$vendor_name' and vendor_id!='$vendor_id'"));
	if($sq_count>0){
		rollback_t();
		echo "error--Supplier name already exists!";
		exit;
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Insurance Supplier has been successfully updated.";
	      exit;
	    }
	    else{
	      rollback_t();
