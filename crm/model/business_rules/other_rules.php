...
class other_rules_master{
    function save(){
        ...
        $cond_arr = isset($_POST['cond_arr']) ? json_encode($_POST['cond_arr']):'';
        begin_t();
        $created_at = date('Y-m-d');
        $from_date = get_date_db($from_date);
        $to_date = get_date_db($to_date);

        $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(rule_id) as max from other_master_rules"));
        $rule_id = $sq_max['max'] + 1;
        $sq = mysqlQuery("INSERT INTO `other_master_rules`(`rule_id`,`rule_for`, `name`,`type`, `validity`, `from_date`, `to_date`, `ledger_id`, `travel_type`,`apply_on`, `fee`,`fee_type`, `target_amount`, `conditions`, `status`,`created_at`) VALUES ('$rule_id','$rule_for','$name','$type','$validity','$from_date','$to_date','$ledger','$travel_type','$apply_on','$fee','$fee_type','$target_amount','$cond_arr','Active','$created_at')");
    ...
    function clone_rule(){
        ...
        $result = mysqlQuery("SHOW COLUMNS FROM other_master_rules"); 
        while ($r=mysqli_fetch_assoc($result)) {
            $cols[]= $r["Field"];
        }
        $result = mysqlQuery("SELECT * FROM other_master_rules WHERE rule_id='$rule_id'");
        ...
        echo "Other Rule Copied Successfully!";
        exit;
    }
    ...
    function update(){
        ...
        $sq = mysqlQuery("update other_master_rules set rule_for='$rule_for',name='$name',validity='$validity',from_date='$from_date',to_date='$to_date',ledger_id='$ledger',travel_type='$travel_type',apply_on='$apply_on',type='$type',fee='$fee',fee_type='$fee_type',target_amount='$target_amount',conditions='$cond_arr',status='$status' where rule_id='$rule_id'");
