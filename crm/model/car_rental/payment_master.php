...
class payment_master
{
  ...
  public function car_payment_delete()
  {
    ...
    $sq_car_payment = mysqli_fetch_assoc(mysqlQuery("select * from car_rental_payment where payment_id='$payment_id'"));
    $booking_id = $sq_car_payment['booking_id'];
    $credit_charges = isset($sq_car_payment['credit_charges']) ? $sq_car_payment['credit_charges'] : 0;
    $credit_card_details = isset($sq_car_payment['credit_card_details']) ? $sq_car_payment['credit_card_details'] : '';
    $payment_mode = $sq_car_payment['payment_mode'];
    $payment_amount = $sq_car_payment['payment_amount'];
    $bank_id1 = $sq_car_payment['bank_id'];
    $bank_name = $sq_car_payment['bank_name'];
    $transaction_id1 = $sq_car_payment['transaction_id'];
    $payment_date1 = $sq_car_payment['payment_date'];
    $canc_status = $sq_car_payment['status'];
    $sq_visa_booking = mysqli_fetch_assoc(mysqlQuery("select * from car_rental_booking where booking_id='$booking_id'"));
    ...
    $yr1 = $year1[0];
    $trans_id = get_car_rental_booking_payment_id($payment_id, $yr2) . ' : ' . $cust_name;
    $transaction_master->updated_entries('Car Rental Receipt', $booking_id, $trans_id, $payment_amount, 0);
    ...
    $delete_master->delete_master_entries('Receipt(' . $payment_mode . ')', 'Car Rental Receipt', $payment_id, get_car_rental_booking_payment_id($payment_id, $yr2), $cust_name, $payment_amount);
    ...
    if ($payment_mode != 'Credit Note') {
      ...
      if ($payment_mode == 'Credit Card') {
        ...
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_id($booking_id, $yr1), $payment_date, 0, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        ...
        $payment_amount = 0;
        $payment_date = $deleted_date;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_id($booking_id, $yr1), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
    ...
    $bank_id = $bank_id1;
    $particular = get_sales_paid_particular(get_car_rental_booking_id($booking_id, $yr1), $payment_date, $payment_amount, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id, $transaction_id, $canc_status);
  ...
  public function finance_save($payment_id, $branch_admin_id)
  {
    ...
    if ($payment_mode != 'Credit Note') {
      ...
      if ($payment_mode == 'Credit Card') {
        ...
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date, $credit_charges, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = $cust_gl;
        $payment_side = "Debit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        $payment_amount = $credit_charges;
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date1, $credit_charges, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = 224;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        //////Get Credit card company Charges///////
        $sq_credit_charges = mysqli_fetch_assoc(mysqlQuery("select * from credit_card_company where entry_id='$entry_id'"));
        $credit_charges_cal = isset($sq_credit_charges['credit_card_charges']) ? $sq_credit_charges['credit_card_charges'] : 0;
        $tax_credit_charges_cal = isset($sq_credit_charges['tax_on_credit_card_charges']) ? $sq_credit_charges['tax_on_credit_card_charges'] : 0;
        $charges_in = isset($sq_credit_charges['charges_in']) ? $sq_credit_charges['charges_in'] : 'Flat';
        $tax_charges_in = isset($sq_credit_charges['tax_charges_in']) ? $sq_credit_charges['tax_charges_in'] : 'Flat';
        //////company's credit card charges
        $company_card_charges = ($charges_in == 'Flat') ? $credit_charges_cal : ($payment_amount1 * ($credit_charges_cal / 100));
        //////company's tax on credit card charges
        $tax_charges = ($tax_charges_in == 'Flat') ? $tax_credit_charges_cal : ($company_card_charges * ($tax_credit_charges_cal / 100));
        ...
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date1, $finance_charges, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = 231;
        $payment_side = "Debit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = $company_gl;
        $payment_side = "Debit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date, $payment_amount1, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = $pay_gl;
        $payment_side = "Debit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
      ...
      $payment_amount = $payment_amount1;
      $payment_date = $payment_date1;
      $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date1, $payment_amount1, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
      $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
      $gl_id = $cust_gl;
      $payment_side = "Credit";
      $clearance_status = "";
  ...
  public function bank_cash_book_save($payment_id, $branch_admin_id)
  {
    ...
    $sq_car_rental_info = mysqli_fetch_assoc(mysqlQuery("select customer_id from car_rental_booking where booking_id='$booking_id'"));
    if ($payment_mode == 'Credit Card') {
    ...
    $transaction_id = $transaction_id;
    $bank_id = $bank_id;
    $particular = get_sales_paid_particular(get_car_rental_booking_payment_id($payment_id, $yr1), $payment_date1, $payment_amount, $sq_car_rental_info['customer_id'], $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id, $transaction_id, $canc_status);
    $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
    $payment_side = "Debit";
    $payment_type = ($payment_mode == "Cash") ? "Cash" : "Bank";
  ...
  public function payment_update()
  {
    ...
    if (!$sq_payment) {
      ...
      if ((float)($payment_old_value) != (float)($payment_amount)) {
        ...
        $year = $yr[0];
        $sq_package = mysqli_fetch_assoc(mysqlQuery("select customer_id from car_rental_booking where booking_id='$booking_id'"));
        ...
        $trans_id = get_car_rental_booking_payment_id($payment_id, $year) . ' : ' . $cust_name;
        $transaction_master->updated_entries('Car Rental Receipt', $booking_id, $trans_id, $payment_old_value, $payment_amount);
      }
      if ($GLOBALS['flag']) {
        commit_t();
  ...
  public function finance_update($sq_payment_info, $clearance_status1)
  {
    ...
    $sq_car_info = mysqli_fetch_assoc(mysqlQuery("select customer_id from car_rental_booking where booking_id='$booking_id'"));
    $customer_id = $sq_car_info['customer_id'];
    global $transaction_master;
    ...
    if ($payment_amount1 != $payment_old_value) {
      ...
      if ($payment_mode == 'Credit Card') {
        ...
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date, $credit_charges_old, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = $cust_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        $payment_amount = $credit_charges_old;
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date1, $credit_charges_old, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = 224;
        $payment_side = "Debit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
        ...
        $payment_date = $payment_date1;
        $payment_particular = get_sales_paid_particular(get_car_rental_booking_payment_id($booking_id, $yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_car_rental_booking_id($booking_id, $yr1), $bank_id1, $transaction_id1, $canc_status);
        $ledger_particular = get_ledger_particular('By', 'Cash/Bank');
        $gl_id = $pay_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode == "Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
  ...
  public function bank_cash_book_update($clearance_status)
  {
    ...
    if ($payment_mode == 'Credit Card') {
      ...
      $payment_amount = intval($payment_amount) + intval($credit_charges);
      $credit_card_details = explode('-', $credit_card_details);
      $entry_id = isset($credit_card_details[0]) ? $credit_card_details[0] : 0;
      $sq_credit_charges = mysqli_fetch_assoc(mysqlQuery("select bank_id from credit_card_company where entry_id ='$entry_id'"));
      $bank_id = isset($sq_credit_charges['bank_id']) ? $sq_credit_charges['bank_id'] : '';
    }
    $module_name = "Car Rental Booking Payment";
  ...
  public function payment_email_notification_send($booking_id, $payment_amount, $payment_mode, $payment_date)
  {
    ...
    $sq_car_rental = mysqli_fetch_assoc(mysqlQuery("select * from car_rental_booking where booking_id='$booking_id'"));

    $date = $sq_car_rental['created_at'];
    $yr = explode("-", $date);
    $year = $yr[0];
    $sq_customer_info = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_car_rental[customer_id]'"));
    ...
    $sq_total_amount = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum,sum(credit_charges) as sumc from car_rental_payment where booking_id='$booking_id' and clearance_status!='Cancelled'"));
    //  $paid_amount = $sq_total_amount['sum'];
    $credit_card_amount = $sq_total_amount['sumc'];
    $total_amount = intval($sq_car_rental['total_fees']) + intval($credit_card_amount);
    ...
    if ($sq_car_rental['status'] == 'Cancel') {
      ...
      $outstanding = ($total_pay_amt > $canc_amount) ? 0 : (float)($canc_amount) - (float)($total_pay_amt) + (float)($credit_card_amount);
    } else {
    ...
    global $model;
    $model->generic_payment_mail('46', $payment_amount, $payment_mode, $total_amount, $total_pay_amt, $payment_date, $due_date, $email_id, $subject, $customer_name, '', $outstanding);
  }
  ...
  public function payment_sms_notification_send($booking_id, $payment_amount, $payment_mode, $credit_charges)
  {
    ...
    $sq_car_rental = mysqli_fetch_assoc(mysqlQuery("select customer_id from car_rental_booking where booking_id='$booking_id'"));
    ...
    $total_paid_amount = intval($payment_amount) + intval($credit_charges);
    $message = "Dear " . $sq_customer_info['first_name'] . " " . $sq_customer_info['last_name'] . ", Acknowledge your payment of  " . $total_paid_amount . " " . $currency_code . ",  which we received for Car Rental Services installment.";
  ...
  public function payment_update_email_notification_send($payment_id)
  {
    ...
    $sq_total_amount = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum,sum(credit_charges) as sumc from car_rental_payment where booking_id='$booking_id' and clearance_status!='Cancelled'"));
    $credit_card_amount = $sq_total_amount['sumc'];
    $paid_amount = $sq_total_amount['sum'] + (float)($credit_card_amount);
    $total_amount = $sq_car_info['total_fees'] + (float)($credit_card_amount);

    $due_date = ($sq_car_info['due_date'] == '1970-01-01') ? '' : $sq_car_info['due_date'];
    $sq_customer_info = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_car_info[customer_id]'"));
    $email_id = $encrypt_decrypt->fnDecrypt($sq_customer_info['email_id'], $secret_key);
    $customer_name = ($sq_customer_info['type'] == 'Corporate' || $sq_customer_info['type'] == 'B2B') ? $sq_customer_info['company_name'] : $sq_customer_info['first_name'] . ' ' . $sq_customer_info['last_name'];
    ...
    global $model;
    $model->generic_payment_mail('111', $payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $due_date, $email_id, $subject, $customer_name);
  }
  ...
  public function whatsapp_send()
  {
    ...
    $sq_car_info = mysqli_fetch_assoc(mysqlQuery("select * from car_rental_booking where booking_id=" . $_POST['booking_id']));

    $sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum ,sum(`credit_charges`) as sumc from car_rental_payment where clearance_status!='Cancelled' and booking_id=" . $_POST['booking_id']));
    $credit_card_amount = $sq_pay['sumc'];
    $total_amount = intval($sq_car_info['total_fees']) + intval($credit_card_amount);
    ...
    $sq_customer = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id=" . $sq_car_info['customer_id']));
