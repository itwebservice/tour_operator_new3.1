...
class booking_master
{
  public function booking_save()
  {
    ...
    $sq = "INSERT into car_rental_booking( booking_id, customer_id,quotation_id, branch_admin_id,financial_year_id,pass_name, total_pax, days_of_traveling, from_date, to_date, vehicle_name, travel_type, places_to_visit, extra_km,basic_amount,markup_cost,markup_cost_subtotal, service_charge, service_tax_subtotal, total_cost, driver_allowance, permit_charges, toll_and_parking, state_entry_tax,other_charges, total_fees, created_at, due_date, emp_id,capacity,total_hrs,total_km,rate,total_max_km,extra_hr_cost,local_places_to_visit,traveling_date,reflections, roundoff, bsm_values,invoice_pr_id,currency_code ) values ( '$booking_id', '$customer_id','$quotation_id', '$branch_admin_id','$financial_year_id','$pass_name', '$total_pax', '$days_of_traveling','$from_date', '$to_date','$vehicle_name', '$travel_type', '$places_to_visit', '$extra_km','$basic_amount','$markup_cost','$markup_cost_subtotal','$service_charge', '$service_tax_subtotal', '$total_cost', '$driver_allowance', '$permit_charges', '$toll_and_parking', '$state_entry_tax','$other_charges', '$total_fees', '$booking_date', '$due_date','$emp_id','$capacity','$total_hrs','$total_km','$rate','$total_max_km','$extra_hr_cost','$local_places_to_visit','$traveling_date','$reflections', '$roundoff', '$bsmValues','$invoice_pr_id','$currency_code' )";
    //echo $sq;
  ...
  public function car_booking_delete()
  {
    ...
    $total_fees = $row_misc['total_fees'];
    ...
    $trans_id = get_bus_booking_id($booking_id, $year) . ' : ' . $cust_name;
    $transaction_master->updated_entries('Car Rental Sale', $booking_id, $trans_id, $total_fees, 0);
  ...
  public function finance_save($booking_id, $payment_id, $row_spec, $branch_admin_id, $particular)
  {
    ...
    $car_sale_amount = (float)($other_charges) + (float)($driver_allowance) + (float)($permit_charges) + (float)($toll_and_parking) + (float)($state_entry_tax) + (float)($actual_cost);
  ...
  public function booking_update()
  {
    ...
    if ($sq_enq) {
      ...
      global $transaction_master;
      if ((float)($old_total) != (float)($total_fees)) {
        ...
        $trans_id = get_car_rental_booking_id($booking_id, $year) . ' : ' . $cust_name;
        $transaction_master->updated_entries('Car Rental Sale', $booking_id, $trans_id, $old_total, $total_fees);
      }
  ...
  public function car_rental_booking_email_send($booking_id, $payment_amount)
  {
    ...
    $total_amount = (float)($sq_booking['total_fees']) + (float)($credit_card_amount);
    ...
    $content = '<tr>
      ...
      <tr><td style="text-align:left;border: 1px solid #888888;width:50%">Customer Name</td>   <td style="text-align:left;border: 1px solid #888888;" >' . $customer_name . '</td></tr>
		  <tr><td style="text-align:left;border: 1px solid #888888;width:50%">Vehicle Name</td>   <td style="text-align:left;border: 1px solid #888888;" >' . $sq_booking['vehicle_name'] . '</td></tr>
