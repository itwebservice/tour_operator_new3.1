...
if ($from_date != '' || $to_date != '') {
	...
	$pnl_date = ' (' . $from_date . ' To ' . $to_date . ')';
} else {
	$sq_finance = mysqli_fetch_assoc(mysqlQuery("select * from financial_year where financial_year_id='$financial_year_id'"));
	$pnl_date = ' (' . $sq_finance['from_date'] . ' To ' . $sq_finance['to_date'] . ')';
}
...
<section class="print_header main_block mg_bt_20">
	<div class="col-md-8 no-pad">
		<span class="title"><i class="fa fa-file-text"></i> Profit & Loss Report<?= $pnl_date ?></span>
	...
	<div class="col-md-4 no-pad">
		<div class="print_header_contact text-right">
			...
			<p><?php echo ($branch_status == 'yes' && $role != 'Admin'  && $role !='Accountant') ? $branch_details['address1'] . ',' . $branch_details['address2'] . ',' . $branch_details['city'] : $app_address ?></p>
			<p class="no-marg"><i class="fa fa-phone" style="margin-right: 5px;"></i> <?php echo ($branch_status == 'yes' && $role != 'Admin'  && $role !='Accountant') ?
	...
	<div class="col-md-6 pl_sheet">
		<div class="panel panel-default main_block">
			...
			<div class="quadrant main_block">
				...
				$count_d = 1;
				$head_count_d = 1;
				$sq_head = mysqlQuery("select * from head_master where head_id in('12','9')");
				while ($row_head = mysqli_fetch_assoc($sq_head)) { //2 Heads(D2)
				?>
					<div class="list_heading main_block">
						<div class="col-md-8 no-pad">
							<h4><?= $row_head['head_name'] ?></h4>
						</div>
					...
					<?php
					$sq_group = mysqlQuery("select group_id from group_master where head_id='$row_head[head_id]' order by group_id desc"); //Groups
					while ($row_group = mysqli_fetch_assoc($sq_group)) {

						$total_head_amount_d = 0;
						$sq_subgroup = mysqlQuery("select subgroup_id,subgroup_name from subgroup_master where group_id='$row_group[group_id]'");

						while ($row_subgroup = mysqli_fetch_assoc($sq_subgroup)) { //Sub groups
					...
					?>
							...
							<div class="part_heading main_block">
								<h4 class="main_block">
									<div class="col-md-6"><?= $row_subgroup['subgroup_name'] ?></div>
									<div class="col-md-4 no-pad text-right">
							...
							<?php
							$q = mysqlQuery("select * from ledger_master where group_sub_id='$row_subgroup[subgroup_id]'");
							while ($row_q = mysqli_fetch_assoc($q)) {
								...
								$sq_opening_balance = mysqlQuery($sq_p3);
								...
								if ($row_q['ledger_id'] != '' && $total_amount != 0) { ?>
									<div class="part_entry main_block">
										<div class="col-md-1 no-pad text-right"></div>
										<div class="col-md-8 no-pad">
			...
			<script>
				var tmp_head_amount_d1 = $('#head_amount_d1').html();
				var tmp_head_amount_d2 = $('#head_amount_d2').html();
				var tmp_head_amount_d = parseFloat(tmp_head_amount_d1) + parseFloat(tmp_head_amount_d2);
				$('#span_total_sales_d1').html(tmp_head_amount_d);
			</script>
			...
			<div class="quadrant main_block">
				...
				$count_d1 = 1;
				$head_count_d1 = 1;
				$sq_head = mysqlQuery("select head_name,head_id from head_master where head_id in('4','5','10')");
				while ($row_head = mysqli_fetch_assoc($sq_head)) { //Heads
				?>
					<!-- List Heading -->
					<div class="list_heading main_block">
						<div class="col-md-8 no-pad">
					...
					<?php
					$sq_group = mysqlQuery("select group_id from group_master where head_id='$row_head[head_id]' order by group_id desc");
					while ($row_group = mysqli_fetch_assoc($sq_group)) { //Groups

						$total_head_amount_d1 = 0;
						$sq_subgroup = mysqlQuery("select subgroup_name,subgroup_id from subgroup_master where group_id='$row_group[group_id]'");
						while ($row_subgroup = mysqli_fetch_assoc($sq_subgroup)) { //Sub groups
			...
			<script>
				var tmp_head_amount_d1 = $('#head_amount_d11').html();
				var tmp_head_amount_d2 = $('#head_amount_d12').html();
				var tmp_head_amount_d3 = $('#head_amount_d13').html();
				var tmp_head_amount_d = parseFloat(tmp_head_amount_d1) + parseFloat(tmp_head_amount_d2) + parseFloat(tmp_head_amount_d3);
				$('#span_total_sales_d2').html(tmp_head_amount_d);
			</script>
		</div>
	</div>
	<!-- //////////////////////////////////////////////////////DEBIT END////////////////////////////////////////////////////////////////////////////////////////// -->
	...
	<div class="col-md-6 pl_sheet">
		<div class="panel panel-default main_block">
			...
			<div class="quadrant main_block">
				...
				$count = 1;
				$head_count = 1;
				$row_head = mysqli_fetch_assoc(mysqlQuery("select head_name from head_master where head_id='13'"));
				?>
				<!-- List Heading -->
				<div class="list_heading main_block">
					<div class="col-md-8 no-pad">
				...
				<?php
				$sq_group = mysqlQuery("select group_id from group_master where head_id='13' order by group_id desc");
				while ($row_group = mysqli_fetch_assoc($sq_group)) { //Groups
			...
			<script>
				var head_amount = $('#head_amount1').html();
				$('#span_total_sales').html(head_amount);
			</script>
			<div class="result_entry_block main_block">
				<!--Profit Brought Down -->
			...
			<div class="quadrant main_block">
				...
				$count = 1;
				$head_count = 1;
				$row_head = mysqli_fetch_assoc(mysqlQuery("select head_name from head_master where head_id ='11'"));
				?>
				<!-- List Heading -->
				<div class="list_heading main_block">
					<div class="col-md-8 no-pad">
				...
				<?php
				$sq_group = mysqlQuery("select group_id from group_master where head_id='11' order by group_id desc");
				while ($row_group = mysqli_fetch_assoc($sq_group)) { //Groups
			...
			<script>
				var head_amount = $('#head_amount_n1').html();
				$('#span_total_sales2').html(head_amount);
			</script>
		</div>
	</div>
