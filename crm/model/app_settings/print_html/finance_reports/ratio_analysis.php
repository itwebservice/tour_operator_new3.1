...
$row_head = mysqli_fetch_assoc(mysqlQuery("select * from head_master where head_id='2'"));
$total_head_amount_a = 0;
$sq_group = mysqlQuery("select * from group_master where head_id='$row_head[head_id]' order by group_id desc");
 //Groups
while($row_group = mysqli_fetch_assoc($sq_group))
  { 		
  	$sq_subgroup = mysqlQuery("select * from subgroup_master where group_id='$row_group[group_id]'");
     //Sub groups
  	 while($row_subgroup = mysqli_fetch_assoc($sq_subgroup))
     { 	
     	$total_sub_group_amount = 0;
	    $sq_finance = "select * from ledger_master where group_sub_id='$row_subgroup[subgroup_id]'"; 
	    $q = mysqlQuery($sq_finance);
        while($row_q = mysqli_fetch_assoc($q))
        {
        	...
        	$sq_opening_balance = mysqlQuery($q2);
...
$row_head = mysqli_fetch_assoc(mysqlQuery("select * from head_master where head_id='3'"));
...
//Get Capital Total
$row_head = mysqli_fetch_assoc(mysqlQuery("select * from head_master where head_id='1'"));
...
//Loans liabilities
$sq_group = mysqlQuery("select * from group_master where group_id in('16','10','21')");
$total_loan_amount = 0;
 //Groups
while($row_group = mysqli_fetch_assoc($sq_group))
  { 		
...
$sq_group = mysqlQuery("select * from group_master where head_id='13' order by group_id desc");
 //Groups
 while($row_group = mysqli_fetch_assoc($sq_group))
      { 
      	$sq_subgroup = mysqlQuery("select * from subgroup_master where group_id='$row_group[group_id]'");
         ...
         { 	
			...
			<?php 		
	            ...
	            {		
				    ...
				    $sq_opening_balance = mysqlQuery($sq_p1);
	            	while($row_balance = mysqli_fetch_assoc($sq_opening_balance)){
	            		$total_amount += $row_balance['payment_amount'];
	            	}
...
$sq_head = mysqlQuery($tempq);
while($row_head = mysqli_fetch_assoc($sq_head))
{		
 $sq_group = mysqlQuery("select * from group_master where head_id='$row_head[head_id]' order by group_id desc");
 //Groups
 while($row_group = mysqli_fetch_assoc($sq_group))
        ...
        $total_operating_amount = 0;
         ...
         { 	
            ...
            {					
				    ...
				    $sq_opening_balance = mysqlQuery($sq_p2);
	            	while($row_balance = mysqli_fetch_assoc($sq_opening_balance)){
	            		$total_amount += $row_balance['payment_amount'];
	            	}
				   	if($sq_ledger['ledger_id'] != ''){					  	
