		...
		<div class="col-md-12 pl_sheet">
			<div class="panel panel-default main_block">
				...
				<div class="panel-group main_block no-marg" id="accordion" role="tablist" aria-multiselectable="true">
					...
					$count_d = 1;
					$head_count_d = 1;
					$sq_group = mysqlQuery("select group_id from group_master where group_id='2' order by group_id desc");
					//Groups
					...
					$count_d = 1;
					$head_count_d = 1;
					$sq_group = mysqlQuery("select group_id from group_master where group_id='20' order by group_id desc");
					//Groups
					while ($row_group = mysqli_fetch_assoc($sq_group)) {
						$sq_subgroup = mysqlQuery("select subgroup_id,subgroup_name from subgroup_master where group_id='$row_group[group_id]'");
							...
							<div class="panel panel-default main_block no-marg">
								...
								<div id="collapse1<?= $count_d ?>" class="panel-collapse collapse main_block in" role="tabpanel" aria-labelledby="heading1<?= $count_d ?>">
									...
									while ($row_q = mysqli_fetch_assoc($q)) {
										...
										$sq_opening_balance = mysqlQuery($q3);
										while ($row_balance = mysqli_fetch_assoc($sq_opening_balance)) {
											$debit_amount += $row_balance['payment_amount'];
										}
										...
										$sq_opening_balance = mysqlQuery($q4);
										while ($row_balance = mysqli_fetch_assoc($sq_opening_balance)) {
											$credit_amount += $row_balance['payment_amount'];
										}
										$total_amount = $total_amount + $debit_amount - $credit_amount;
					...
					$count_d = 1;
					$head_count_d = 1;
					$sq_subgroup = mysqlQuery("select subgroup_id,subgroup_name from subgroup_master where subgroup_id='105'");
						...
						<div class="panel panel-default main_block no-marg">
							...
							<div id="collapse3<?= $count_d ?>" class="panel-collapse collapse main_block in" role="tabpanel" aria-labelledby="heading3<?= $count_d ?>">
								...
								while ($row_q = mysqli_fetch_assoc($q)) {
									...
									$sq_opening_balance = mysqlQuery($q6);
									while ($row_balance = mysqli_fetch_assoc($sq_opening_balance)) {
										$debit_amount += $row_balance['payment_amount'];
									}
									...
									$sq_opening_balance = mysqlQuery($q7);
									while ($row_balance = mysqli_fetch_assoc($sq_opening_balance)) {
										$credit_amount += $row_balance['payment_amount'];
									}
									$total_amount = $total_amount + $credit_amount - $debit_amount;
