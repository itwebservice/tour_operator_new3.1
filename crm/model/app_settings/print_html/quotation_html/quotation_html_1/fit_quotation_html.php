        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="rounded-xl bg-white p-5 border border-[color:var(--gold)]/25" style="box-shadow:var(--shadow-card)">
            ...
            <ul class="mt-3 space-y-2">
              ...
              $o1_included_html = isset($incx['included']) ? $incx['included'] : '';
              $o1_included_items = array();

              if (preg_match_all('/<li[^>]*>(.*?)<\/li>/is', $o1_included_html, $matches)) {
                foreach ($matches[1] as $li) {
                  $o1_included_items[] = trim(strip_tags($li));
                }
              } else {
                $o1_included = trim(strip_tags($o1_included_html));
                $o1_included_items = preg_split('/\r\n|\r|\n|•|\x{2022}/u', $o1_included);
              }
          ...
          <div class="rounded-xl bg-white p-5 border border-[color:var(--gold)]/25" style="box-shadow:var(--shadow-card)">
            ...
            <ul class="mt-3 space-y-2">
              ...
              $o1_excluded_html = isset($incx['excluded']) ? $incx['excluded'] : '';
              $o1_excluded_items = array();

              if (preg_match_all('/<li[^>]*>(.*?)<\/li>/is', $o1_excluded_html, $matches)) {
                foreach ($matches[1] as $li) {
                  $o1_excluded_items[] = trim(strip_tags($li));
                }
              } else {
                $o1_excluded = trim(strip_tags($o1_excluded_html));
                $o1_excluded_items = preg_split('/\r\n|\r|\n|•|\x{2022}/u', $o1_excluded);
              }
