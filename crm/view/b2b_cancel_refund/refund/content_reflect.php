...
if(sizeof($group_list_arr)>0){
    ...
    for($i=0;$i<sizeof($group_list_arr);$i++){
      $tax_amount = 0;
      $tax_arr = explode(',',$group_list_arr[$i]->service->service_arr[0]->taxation);
      $package_item = explode('-',$group_list_arr[$i]->service->service_arr[0]->package_type);
...
$main_total += $hotel_total + $transfer_total + $activity_total + $tours_total + $ferry_total + $group_total;
...
$remaining_pay=$refund_amount-$sq_ref_pay_total['sum'];
?>
