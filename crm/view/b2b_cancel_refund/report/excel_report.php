...
if(($booking_id!="")){
    $sq_entry_date = mysqli_fetch_assoc(mysqlQuery("select * from b2b_booking_master where booking_id='$booking_id'"));
...
$row_count++;
    ...
    while($row_refund = mysqli_fetch_assoc($sq_refund)){
      ...
      $b2b_name = "";
      $sq_refund_entries = mysqlQuery("select * from b2b_booking_refund_entries where refund_id='$row_refund[refund_id]'");
      while($row_refund_entry = mysqli_fetch_assoc($sq_refund_entries)){
        ...
        $b2b_name .=  ($sq_b2b_info['type'] == 'Corporate' || $sq_b2b_info['type'] == 'B2B') ? $sq_b2b_info['company_name'] : $sq_b2b_info['first_name'].' '.$sq_b2b_info['last_name'];
      }
      $sq_entry_date = mysqli_fetch_assoc(mysqlQuery("select * from b2b_booking_master where booking_id='$row_refund[booking_id]'"));
      $date = $sq_entry_date['created_at'];
      $yr = explode("-", $date);
      $year =$yr[0];
...
header('Content-Disposition: attachment;filename="B2B Booking Refund('.date('d-m-Y H:i').').xls"');
