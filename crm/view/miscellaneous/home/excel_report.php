...
while ($row_visa = mysqli_fetch_assoc($sq_visa)) {
    ...
    $paid_amount = (float)($sq_paid_amount['sum']) + (float)($credit_card_charges);
    ...
    $total_visa_amount = $visa_total_amount - $cancel_amount  + (float)($credit_card_charges);

    //calculate total amounts
    $booking_amount = $booking_amount + $visa_total_amount + (float)($credit_card_charges);
...
// currency conversion
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H' . $row_count, number_format($visa_total_amount + (float)($credit_card_charges), 2))
        ->setCellValue('I' . $row_count, number_format($cancel_amount, 2))
        ->setCellValue('J' . $row_count, number_format($total_visa_amount, 2).$currency_amount)
