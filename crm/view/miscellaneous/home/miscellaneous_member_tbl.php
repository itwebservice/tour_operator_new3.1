<tr>
    ...
    <td><input class="form-control" type="text" id="last_name<?= $offset ?>1" name="last_name<?= $offset ?>1" onchange="fname_validate(this.id)" placeholder="Last Name" title="Last Name"/></td>
    <td><input class="form-control" type="text" id="birth_date<?= $offset ?>1" name="birth_date<?= $offset ?>1" placeholder="Birth Date" title="Birth Date" class="app_datepicker" value="<?= date('d-m-Y',  strtotime(' -1 day')) ?>" onchange="checkPassportDate(this.id);adolescence_reflect(this.id)"/></td>
...
var tom = date.setDate(date.getDate()+1);
    ...
    $('#birth_date1,#issue_date1').datetimepicker({ timepicker:false, format:'d-m-Y' });
