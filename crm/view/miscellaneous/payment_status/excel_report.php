...
$row_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Miscellaneous' and estimate_type_id='$row_visa[misc_id]' and delete_status='0'"));
// currency_conversion
if($total_purchase ==''){
	$currency_amount_3 =0;
}
...
$currency_amount_3= '';
    ...
    $sq_incentive = mysqli_fetch_assoc(mysqlQuery("select * from booker_sales_incentive where booking_id='$row_visa[misc_id]' and service_type='Miscellaneous Booking'"));
            ...
            $currency_amount2 = currency_conversion($currency,$row_visa['currency_code'],$paid_amount);
	if($row_visa['currency_code'] !='0' && $currency != $row_visa['currency_code']){
	  ...
	  $currency_amount4 = currency_conversion($currency,$row_visa['currency_code'],$total_bal);
	if($row_visa['currency_code'] !='0' && $currency != $row_visa['currency_code']){
		$currency_amount_4 = ' ('.$currency_amount4.')';
	}else{
		$currency_amount_4 = '';
	}
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('O' . $row_count, number_format($total_bal, 2).$currency_amount_4)
...
header('Content-Disposition: attachment;filename="MiscellaneousSummary(' . date('d-m-Y H:i') . ').xls"');
