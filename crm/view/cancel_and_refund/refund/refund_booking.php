...
$sq_total_tour_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum from package_payment_master where booking_id='$booking_id' and payment_for='Tour' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));

$sq_total_travel_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum from package_payment_master where booking_id='$booking_id' and payment_for='Travelling' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
...
$sq_total_ref_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id'"));
$sq_cancelled = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' and clearance_status='Cancelled' "));

$unique_timestamp =  md5(uniqid());
$sale_Amount=$sq_booking['tour_cost_total'];
                ...
                <div class="row">
                    <div class="col-xs-12 mg_bt_10">
                        <select id="traveler_id" name="traveler_id" multiple style="height:70px;" class="form-control" >
                            <?php 
                            $sq_traveler_query = mysqlQuery("select * from package_travelers_details where booking_id='$booking_id' ");
