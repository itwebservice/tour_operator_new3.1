...
<table id="refund_history" class="table table-bordered table-hover no-marg">
    ...
    $count = 0;
    $bg;
    $sq_refund = mysqlQuery("select * from package_refund_traveler_cancelation where booking_id='$booking_id' and total_refund!= 0");
    ...
    <tfoot>
        <?php 
        $sq_refund = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' "));
        $sq_pending = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' and clearance_status='Pending' "));
        $sq_cancel = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' and clearance_status='Cancelled' "));
        ?>
        <tr class="active">
