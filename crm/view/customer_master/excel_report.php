...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Customer Report')
...
while($row_customer = mysqli_fetch_assoc($sq_customer)){
    ...
    $sq_state = mysqli_fetch_assoc(mysqlQuery("select * from state_master where id='$row_customer[state_id]'"));
	...
	if($row_customer['type'] == 'Corporate'||$row_customer['type']=='B2B'){
		$cust_name = $row_customer['company_name'];
	}else{
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, $cust_name)
...
header('Content-Disposition: attachment;filename="Customer_Report('.date('d-m-Y H:i').').xls"');
