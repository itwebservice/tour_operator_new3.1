...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, "Invoice No")
        ->setCellValue('D'.$row_count, "Income Type")
        ->setCellValue('E'.$row_count, "Receipt From")
        ->setCellValue('F'.$row_count, "Personal Identification No(PIN)")
        ->setCellValue('G'.$row_count, "Receipt Date")
        ->setCellValue('H'.$row_count, "Mode")
        ->setCellValue('I'.$row_count, "Narration")
...
while($row_income = mysqli_fetch_assoc($sq_income)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, $sq_income_type_info['ledger_name'])
        ->setCellValue('E'.$row_count, $row_income['receipt_from'])
        ->setCellValue('F'.$row_count, strtoupper($row_income['pan_no']))
        ->setCellValue('G'.$row_count, get_date_user($row_income['receipt_date']))
        ->setCellValue('H'.$row_count, ($sq_paid['payment_mode']=='')?'NA':$sq_paid['payment_mode'])
        ->setCellValue('I'.$row_count, $row_income['particular'])
        ->setCellValue('J'.$row_count, number_format($sq_paid['payment_amount'],2));
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G'.$row_count, 'Paid Amount: '.number_format($paid_amount, 2))
        ->setCellValue('H'.$row_count, 'Pending Clearance: '.number_format($sq_pending_amount, 2))
        ->setCellValue('I'.$row_count, 'Cancelled: '.number_format($sq_cancel_amount, 2))
        ->setCellValue('J'.$row_count, 'Total Payment: '.number_format(($paid_amount - $sq_pending_amount - $sq_cancel_amount), 2));
