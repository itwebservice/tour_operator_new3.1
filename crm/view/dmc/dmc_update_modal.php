...
$dmc_id = $_POST['dmc_id'];
$sq_dmc_info = mysqli_fetch_assoc(mysqlQuery("select * from dmc_master where dmc_id='$dmc_id'"));
$vendor_name = addslashes($sq_dmc_info['company_name']);
$sq_vendor_login = mysqli_fetch_assoc(mysqlQuery("select * from vendor_login where username='$vendor_name' and password='$sq_dmc_info[mobile_no]' and vendor_type='DMC Vendor'"));
...
<div class="modal fade" id="dmc_update_modal" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Update DMC Supplier Details</h4>
      </div>
      <div class="modal-body">
          <form id="frm_dmc_update">
              ...
              <div class="row">
                  <div class="col-md-3 col-sm-6 mg_bt_10">
                     <select id="cmb_city_id1" name="cmb_city_id1" style="width:100%" title="City Name">
                        <?php $sq_city = mysqli_fetch_assoc(mysqlQuery("select city_id,city_name from city_master where city_id='$sq_dmc_info[city_id]'")); ?>
                ...
                <div class="col-sm-3 col-xs-6 mg_bt_10">
                  <select name="cust_state1" id="cust_state1" title="Select State/Country Name" style="width:100%" required class="app_select2">
                    ...
                    $sq_state = mysqli_fetch_assoc(mysqlQuery("select * from state_master where id='$sq_dmc_info[state_id]'"));
                    ?>
                ...
                <div class="col-md-3 col-sm-6 mg_bt_10">
                    <select name="active_flag1" id="active_flag1" title="Status" class="form-control" data-toggle='tooltip'>
                ...
                <div class="col-md-3 col-sm-6 mg_bt_10">
                  <select class="form-control" name="side1" id="side1" title="Balance Side" data-toggle='tooltip'>
                    <option value="<?= $sq_dmc_info['side']?>"><?= $sq_dmc_info['side']?></option>
                    <?php if($sq_dmc_info['side'] != 'Debit'){ ?>
                      <option value="Debit">Debit</option>
                    <?php }
                    if($sq_dmc_info['side'] != 'Credit'){ ?>
            ...
            <div class="panel panel-default panel-body app_panel_style feildset-panel hidden">
            ...
            <div class="row">
              <div class="col-md-3 col-sm-6 mg_bt_10">
               ...
               <div class="col-md-3 col-sm-6 mg_bt_10">
                  <input type="text" name="account_name" id="account_name" placeholder="A/c Type" title="A/c Type" value="<?= $sq_dmc_info['account_name']?>">
                </div> 
               <div class="col-md-3 col-sm-6 mg_bt_10">
...
$('#cust_state1').select2({
  dropdownParent: $("#dmc_update_modal")});
  $('.app_select2').select2();
