...
$sq_bank_info = mysqli_fetch_assoc(mysqlQuery("select * from cash_withdraw_master where withdraw_id='$withdraw_id'"));
?>
<input type="hidden" name="payment_old_amount" id="payment_old_amount" value="<?= $sq_bank_info['amount'] ?>">
<input type="hidden" name="withdraw_id" id="withdraw_id" value="<?= $sq_bank_info['withdraw_id'] ?>">
<form id="frm_update">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Update Cash Withdrawal</h4>
      </div>
      <div class="modal-body">            
            <div class="row">
              <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
                <select id="bank_id1" name="bank_id1" style="width:100%" title="Bank" class="form-control" disabled>
                    <?php $sq_bank = mysqli_fetch_assoc(mysqlQuery("select * from bank_master where bank_id='$sq_bank_info[bank_id]'"));  ?>
  ...
  submitHandler:function(form){
    ...
    $.post(base_url+'view/load_data/finance_date_validation.php', { check_date: payment_date }, function(data){
      if(data !== 'valid'){
        ...
        $.ajax({
          type:'post',
          url:base_url+'controller/bank_vouchers/cash_withdrawal_update.php',
          data: { withdraw_id : withdraw_id,bank_id : bank_id, payment_amount : payment_amount, payment_date : payment_date,payment_evidence_url : payment_evidence_url,payment_old_amount : payment_old_amount},
