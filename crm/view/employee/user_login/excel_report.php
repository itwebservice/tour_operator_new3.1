...
$count = 0;
$sq_log = mysqlQuery($query);
while($row_log = mysqli_fetch_assoc($sq_log)){
  if($row_log['login_time']!="00:00:00"){

    $sq_login = mysqli_fetch_assoc(mysqlQuery("select * from roles where emp_id='$row_log[login_id]'"));
    $sq_emp_info = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$sq_login[emp_id]'"));

    $login_date1 = $row_log['login_date'];
    if($login_date1=="" || $login_date1=="0000-00-00"){
      $login_date1 = "";
    }
    else{
      $login_date1= date('d-m-Y', strtotime($login_date1));
    }
    ...
    $objPHPExcel->setActiveSheetIndex(0)
      ...
      ->setCellValue('C'.$row_count, $login_date1.' '.$login_time)
...
header('Content-Disposition: attachment;filename="User_Log('.date('d-m-Y H:i').').xls"');
