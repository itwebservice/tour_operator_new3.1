...
{
    $_SESSION['dateqry'] = " and cancel_flag!=1";
    $_SESSION['package_qry'] = " and status!='Cancel'";
}
...
function package_hotel_rooms($id)
{
        ...
        if(!empty($fromdate) && !empty($todate)){
            $sql .= " and package_tour_booking_master.booking_date between '".$fromdate."' and '".$todate."'";
        }
        $sql .= " GROUP BY package_hotel_accomodation_master.id";
        $res=mysqlquery($sql);
        $rooms = 0; 
...
function hotel_rooms($id)
{
    ...
    if(!empty($fromdate) && !empty($todate)){
        $query1 .= " and hotel_booking_master.created_at between '".$fromdate."' and '".$todate."'";
    }
...
// else
// {   
    $query = "SELECT * FROM hotel_master INNER JOIN city_master on hotel_master.city_id = city_master.city_id INNER Join package_hotel_accomodation_master on hotel_master.hotel_id = package_hotel_accomodation_master.hotel_id INNER JOIN package_tour_booking_master on package_hotel_accomodation_master.booking_id = package_tour_booking_master.booking_id INNER JOIN customer_master on package_tour_booking_master.customer_id = customer_master.customer_id inner join vendor_estimate on package_tour_booking_master.booking_id = vendor_estimate.estimate_type_id where 1 ".$_SESSION['package_qry'];
    if(!empty($fromdate) && !empty($todate)){
        $query .= "and package_tour_booking_master.booking_date between '".$fromdate."' and '".$todate."'";
    }
    $query .= "group by hotel_master.hotel_id ";
