...
if (empty($fromdate) && empty($todate)) {
    $_SESSION['dateqry'] = " and cancel_type!=1 AND ticket_trip_entries.status!='Cancel'";
    $_SESSION['group_qry'] = " ";
...
for ($i = 0; $i <= count($main_location) - 1; $i++) {
    $totalBookings = get_total($From_location[$i], $to_location[$i]) + get_group_total($From_location[$i], $to_location[$i]) +  get_package_total($From_location[$i], $to_location[$i]);
    ...
    $temparr = array("data" => array(
        ...
        '<button class="btn btn-info btn-sm" onclick="view_com_sector_modal(`' . $From_location[$i] . '`,`' . $to_location[$i] . '`,`' . $i . '`)" data-toggle="tooltip" title="View Details" id="view_btn-' . $i . '"><i class="fa fa-eye"></i></button>'
    ), "bg" => '');
