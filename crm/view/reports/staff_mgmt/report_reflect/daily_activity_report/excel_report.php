...
if($emp_id1!=""){   
    $sq_emp = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$emp_id1'"));
...
$sq = mysqlQuery($query);
    ...
    {
      $count++;          
      $sq_emp1 = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$row[emp_id]'")); 

  $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('B'.$row_count, $count)
        ->setCellValue('C'.$row_count, date('d-m-Y',strtotime($row['activity_date'])))
        ->setCellValue('D'.$row_count, $sq_emp1['first_name'].' '.$sq_emp1['last_name'])
...
header('Content-Disposition: attachment;filename="Daily Activity Report('.date('d-m-Y H:i').').xls"');
