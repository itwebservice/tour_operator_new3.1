...
$row_count = 11;
       ...
       $objPHPExcel->setActiveSheetIndex(0)
        ...
        while($row_customer = mysqli_fetch_assoc($sq_customer)){
            ...
            $i=0;
            $p_due_date = '';
            $sq_purchase_count = mysqli_num_rows(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='B2B' and estimate_type_id='$row_customer[booking_id]' and delete_status='0'"));
            if($sq_purchase_count == 0){  $p_due_date = 'NA'; }
            $sq_purchase = mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='B2B' and estimate_type_id='$row_customer[booking_id]' and delete_status='0'");
            while($row_purchase = mysqli_fetch_assoc($sq_purchase)){			
                $purchase_amt = $row_purchase['net_total'] - $row_purchase['refund_net_total'];
                $total_purchase = $total_purchase + $purchase_amt;
            }
            $sq_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='B2B' and estimate_type_id='$row_customer[booking_id]' and delete_status='0'"));		
    	...
    	$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D'.$row_count, $sq_cust['company_name'].$cust_name1)
            ...
            ->setCellValue('O'.$row_count, $sq_cust['company_name']);
