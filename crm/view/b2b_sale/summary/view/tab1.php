	...
	<div class="col-md-12">
		<div class="profile_box main_block">
					...
					for($i=0;$i<sizeof($transfer_list_arr);$i++){
						...
						<div class="col-md-6 mg_bt_10">
							...
							<div class="row">
								...
								<div class='col-md-4 mg_bt_10'>No.Of Vehicles :  <strong><?= $transfer_list_arr[$i]->service->service_arr[$j]->no_of_vehicles ?></strong></div>
								<div class='col-md-8 mg_bt_10'>Service Duration :  <strong><?= $transfer_list_arr[$i]->service->service_arr[$j]->duration ?></strong></div>
				...
				<div class="row no-gutters">
					<legend>Holiday Details</legend>
						...
						<div class="col-md-6">
							...
							<div class="row">
								<div class='col-md-7'>Travel Date :  <strong><?= date('m-d-Y', strtotime($tours_list_arr[$i]->service->service_arr[0]->travel_date)); ?></strong></div>
								<div class='col-md-5'>Total Stay :  <strong><?= $tours_list_arr[$i]->service->service_arr[0]->nights.'N/'.$tours_list_arr[$i]->service->service_arr[0]->days.'D' ?></strong></div>
							</div>
							<div class="row">
			...
			if(sizeof($group_list_arr)>0){
				?>
				<div class="row no-gutters">
					<legend>Group Tours Details</legend>
					<?php
					for($i=0;$i<sizeof($group_list_arr);$i++){
						...
						for($j=0;$j<count(array($services));$j++){
							...
							<div class="col-md-6">
								<div class="row">
									<div class='col-md-12'>Tour Name :  <strong><?= $group_list_arr[$i]->service->service_arr[$j]->tour_name ?></strong></div>
								</div>
								<div class="row">
									<div class='col-md-12'>Travel Date :  <strong><?= $tour_group[1] ?></strong></div>
								</div>
								<div class="row">
									<div class='col-md-4'>Total Pax: <strong><?= intval($group_list_arr[$i]->service->service_arr[$j]->adult) + intval($group_list_arr[$i]->service->service_arr[$j]->childwo) + intval($group_list_arr[$i]->service->service_arr[$j]->childwi) + intval($group_list_arr[$i]->service->service_arr[$j]->infant) ?></strong></div>
									<div class='col-md-4'>Extra Bed: <strong><?= intval($group_list_arr[$i]->service->service_arr[$j]->extra_bed) ?></strong></div>
								</div>
								<?php
								...
								$gtoursprice_total += $room_cost1;
								$gtourstax_total += $tax_amount1;
