...
for($i=0;$i<sizeof($traveller_details);$i++){
    ...
    else if($traveller_details[$i]->service->name == 'Group Tours'){
        array_push($group_traveller_arr,$traveller_details[$i]);
    }
}
?>
	...
	<div class="col-md-12">
		<div class="profile_box main_block">
			<?php if(sizeof($hotel_traveller_arr)>0){ ?>
                <legend>Guest Details for Hotel</legend>
			...
			<?php if(sizeof($package_traveller_arr)>0){ ?>
			    <legend class="mg_tp_10">Guest Details for Holiday</legend>
                <?php
			...
			<?php if(sizeof($ferry_traveller_arr)>0){ ?>
			    <legend class="mg_tp_10">Guest Details for Ferry</legend>
                <?php
			...
			<?php if(sizeof($group_traveller_arr)>0){ ?>
			    <legend class="mg_tp_10">Guest Details for Group Tour</legend>
                <?php
                for($i=0;$i<sizeof($group_traveller_arr);$i++){
                    $tour_id = $group_traveller_arr[$i]->service->id;
                    $sq_tour = mysqli_fetch_assoc(mysqlQuery("select tour_name from tour_master where tour_id='$tour_id'"));
                    ?>
                    <h5 class="serviceTitle"><?= $sq_tour['tour_name'] ?></h5>
                    <div class="row">
                        <div class="col-md-12 profile_box">
                        <table class="table table-bordered no-marg">
                            <thead>
                            ...
                            <tbody>
                                <?php
                                $count = 0;
                                for($k=0;$k<sizeof($group_traveller_arr[$i]->service->adults);$k++){
                                    $pass_name =  ($group_traveller_arr[$i]->service->adults[$k]->honorofic.' '.$group_traveller_arr[$i]->service->adults[$k]->fname).' '.($group_traveller_arr[$i]->service->adults[$k]->lname);
                                    $birthdate = $group_traveller_arr[$i]->service->adults[$k]->birthdate;
                                    $meal = $group_traveller_arr[$i]->service->adults[$k]->meal;
                                    ?>
                                    <tr>
                                        <td><?= ++$count ?></td>
                                        <td><?= 'Adult' ?></td>
                                ...
                                <?php } ?>
                                <?php
                                $group_traveller_arr[$i]->service->cwb = ($group_traveller_arr[$i]->service->cwb != '' || $group_traveller_arr[$i]->service->cwb != null) ? $group_traveller_arr[$i]->service->cwb : [];
                                for($k=0;$k<sizeof($group_traveller_arr[$i]->service->cwb);$k++){
                                    $pass_name =  rawurldecode($group_traveller_arr[$i]->service->cwb[$k]->honorofic.' '.$group_traveller_arr[$i]->service->cwb[$k]->fname).' '.($group_traveller_arr[$i]->service->cwb[$k]->lname);
                                    $birthdate = $group_traveller_arr[$i]->service->cwb[$k]->birthdate;
                                    $meal = $group_traveller_arr[$i]->service->cwb[$k]->meal;
                                    ?>
                                    <tr>
                                        <td><?= ++$count ?></td>
                                        <td><?= 'ChildWithBed' ?></td>
                                ...
                                <?php } ?>
                                <?php
                                $group_traveller_arr[$i]->service->cwob = ($group_traveller_arr[$i]->service->cwob != '' || $group_traveller_arr[$i]->service->cwob != null) ? $group_traveller_arr[$i]->service->cwob : [];
                                for($k=0;$k<sizeof($group_traveller_arr[$i]->service->cwob);$k++){
                                    $pass_name =  rawurldecode($group_traveller_arr[$i]->service->cwob[$k]->honorofic.' '.$group_traveller_arr[$i]->service->cwob[$k]->fname).' '.($group_traveller_arr[$i]->service->cwob[$k]->lname);
                                    $birthdate = $group_traveller_arr[$i]->service->cwob[$k]->birthdate;
                                    $meal = $group_traveller_arr[$i]->service->cwob[$k]->meal;
                                    ?>
                                    <tr>
                                        <td><?= ++$count ?></td>
                                        <td><?= 'ChildWithoutBed' ?></td>
                                ...
                                <?php } ?>
                                <?php
                                $group_traveller_arr[$i]->service->infant = ($group_traveller_arr[$i]->service->infant != '' || $group_traveller_arr[$i]->service->infant != null) ? $group_traveller_arr[$i]->service->infant : [];
                                for($k=0;$k<sizeof($group_traveller_arr[$i]->service->infant);$k++){
                                    $pass_name =  rawurldecode($group_traveller_arr[$i]->service->infant[$k]->honorofic.' '.$group_traveller_arr[$i]->service->infant[$k]->fname).' '.($group_traveller_arr[$i]->service->infant[$k]->lname);
                                    $birthdate = $group_traveller_arr[$i]->service->infant[$k]->birthdate;
                                    $meal = $group_traveller_arr[$i]->service->infant[$k]->meal;
                                    ?>
                                    <tr>
                                        <td><?= ++$count ?></td>
                                        <td><?= 'Infant' ?></td>
