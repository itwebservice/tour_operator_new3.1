...
if(($booking_id!="")){
    $sq_entry_date = mysqli_fetch_assoc(mysqlQuery("select * from b2c_sale where booking_id='$booking_id'"));
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'B2C Booking Refund')
...
$row_count++;
    ...
    while($row_refund = mysqli_fetch_assoc($sq_refund)){
      ...
      if($row_refund['clearance_status']=="Pending"){ 
        ...
        ->setCellValue('C'.$row_count, get_b2c_booking_id($row_refund['booking_id'],$year))
        ->setCellValue('D'.$row_count, $b2c_name)
...
header('Content-Disposition: attachment;filename="B2C Booking Refund('.date('d-m-Y H:i').').xls"');
