...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Activity Payment')
...
while($row_exc_payment = mysqli_fetch_assoc($sq_exc_payment)){
    ...
    $sq_paid_amount = $sq_paid_amount + $row_exc_payment['payment_amount'] + $row_exc_payment['credit_charges'];
     ...
     if($row_exc_payment ['currency_code'] ==''){
		$currency_code_3 = $sq_exc_info['currency_code'];
	  }
	  else{
		$currency_code_3= $row_exc_payment ['currency_code'];
	  }
    //Currency conversion
    $currency_amount1 = currency_conversion($currency,$currency_code_3,$row_exc_payment['payment_amount']+$row_exc_payment['credit_charges']);
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, get_exc_booking_payment_id($row_exc_payment['payment_id'],$year1))
        ->setCellValue('D'.$row_count, get_exc_booking_id($row_exc_payment['exc_id'],$year))
        ->setCellValue('E'.$row_count, $customer_name)
        ->setCellValue('F'.$row_count, date('d-m-Y', strtotime($row_exc_payment['payment_date'])))
        ->setCellValue('G'.$row_count, $row_exc_payment['payment_mode'])
        ->setCellValue('H'.$row_count, number_format($row_exc_payment['payment_amount'] + $row_exc_payment['credit_charges'],2).$currency_amount);
    ...
    ->setCellValue('F'.$row_count, 'Pending Clearance : '.number_format($sq_pending_amount,2))
...
header('Content-Disposition: attachment;filename="Activity Payment('.date('d-m-Y H:i').').xls"');
