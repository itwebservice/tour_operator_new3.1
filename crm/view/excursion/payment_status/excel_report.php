...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('C2', 'Activity Summary')
...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('W' . $row_count, "Incentive");
...
while ($row_exc = mysqli_fetch_assoc($sq_exc)) {
    ...
    $i = 0;
    $p_due_date = '';
    $sq_purchase_count = mysqli_num_rows(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Activity' and estimate_type_id='$row_exc[exc_id]' and delete_status='0'"));
    if ($sq_purchase_count == 0) {
        $p_due_date = 'NA';
    }
    $sq_purchase = mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Activity' and estimate_type_id='$row_exc[exc_id]' and delete_status='0'");
...
$row_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Activity' and estimate_type_id='$row_exc[exc_id]' and delete_status='0'"));
// currency_conversion
if($total_purchase ==''){
$currency_amount_3 =0;
}
...
$currency_amount_3= '';
    ...
    $sq_incentive = mysqli_fetch_assoc(mysqlQuery("select * from booker_sales_incentive where booking_id='$row_exc[exc_id]' and service_type='Excursion Booking'"));
     //Currency conversion
            $currency_amount1 = currency_conversion($currency,$row_exc['currency_code'],$total_bal);
            if($row_exc['currency_code'] !='0' && $currency != $row_exc['currency_code']){
              ...
              $currency_amount2 = currency_conversion($currency,$row_exc['currency_code'],$paid_amount);
	if($row_exc['currency_code'] !='0' && $currency != $row_exc['currency_code']){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H' . $row_count, get_date_user($row_exc['created_at']))
        ->setCellValue('I' . $row_count, number_format($row_exc['exc_issue_amount'], 2))
        ->setCellValue('J' . $row_count, number_format($row_exc['service_charge'] + $row_exc['markup'], 2))
        ...
        ->setCellValue('O' . $row_count, number_format($total_bal, 2) . $currency_amount)
        ->setCellValue('P' . $row_count, number_format($paid_amount, 2).$currency_amount_2)
        ->setCellValue('Q' . $row_count, number_format($bal, 2))
        ->setCellValue('R' . $row_count, $due_date)
        ->setCellValue('S' . $row_count, number_format($total_purchase, 2).$currency_amount_3)
        ->setCellValue('T' . $row_count, $vendor_name1)
        ->setCellValue('U' . $row_count, $branch_name)
        ->setCellValue('V' . $row_count, $emp_name)
        ->setCellValue('W' . $row_count, number_format($sq_incentive['incentive_amount'], 2));
...
header('Content-Disposition: attachment;filename="ActivitySummary(' . date('d-m-Y H:i') . ').xls"');
