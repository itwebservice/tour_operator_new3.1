		...
		submitHandler: function (form) {
			...
			for (var i = 0; i < rowCount; i++) {
				...
				if (row.cells[0].childNodes[0].checked) {
					var travel_date_temp = row.cells[2].childNodes[0];
					var travel_date1 = $(travel_date_temp).val();
					var arrval1 = row.cells[3].childNodes[0].value;
					...
					if (row.cells[12]) {
						var plane_id1 = row.cells[12].childNodes[0].value;
					}
					else {
						var plane_id1 = '';
					}
			...
			for (var i = 0; i < rowCount; i++) {
				...
				if (row.cells[0].childNodes[0].checked) {
					...
					if (row.cells[9]) {
						var trans_entry_id = row.cells[9].childNodes[0].value;
					}
					else {
						var trans_entry_id = '';
					}
					...
					if (data1 == 'yes') {
						...
						$.post(
							...
							infant_arr:infant_arr,
								...
								'arrval_arr[]': arrval_arr,class_arr:class_arr,
								...
								basic_amount: basic_amount,old_total:old_total,
								reflections: reflections, bsmValues: bsmValues, roundoff: roundoff, net_total: net_total, tcs_tax: tcs_tax, tcs: tcs,
								discount_in:discount_in,discount_amt:discount_amt,act_discount:act_discount,tds:tds
							},
							function (data) {
