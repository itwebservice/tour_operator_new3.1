$(function () {
	$('#frm_tab_4').validate({
		...
		submitHandler: function (form) {
			...
			for (var i = 0; i < rowCount; i++) {
				...
				if (row.cells[0].childNodes[0].checked) {
					var travel_date_temp = row.cells[2].childNodes[0];
					var travel_date1 = $(travel_date_temp).val();
					var arraval1 = row.cells[3].childNodes[0].value;
			...
			for (var i = 0; i < rowCount; i++) {
				...
				if (row.cells[0].childNodes[0].checked) {
					...
					meal_plan_hotel.push(meal_plan1);
					confirmation_no.push(confirmation_no1);
				}
			}
			//**Transport travel details starts here
			var transp_vehicle_arr = [];
			var transp_start_date = [];
			var transp_end_date = [];
			var trans_pickup_arr = [];
			var trans_drop_arr = [];
			var trans_pickuptype_arr = [];
			var trans_droptype_arr = [];
			var trans_count_arr = [];
			...
			for (var i = 0; i < rowCount; i++) {
				...
				if (row.cells[0].childNodes[0].checked) {
					...
					var vcount = row.cells[8].childNodes[0].value;
		...
		for (var i = 0; i < rowCount; i++) {
			...
			var p_travel_type = [];
			var bank_id_arr = [];
			var credit_amount_arr = [];
			var credit_charges_arr = [];
			var credit_card_details_arr = [];
												...
												if (result == 'yes') {
													...
													$.post(
														...
														infant_arr:infant_arr,
															...
															discount_in:discount_in,discount_amt:discount_amt,act_discount:act_discount,
															...
															'arraval_arr[]': arraval_arr,class_arr:class_arr,
															...
															basic_amount: basic_amount, bsmValues: bsmValues, roundoff: roundoff, credit_amount_arr: credit_amount_arr, credit_charges_arr: credit_charges_arr, credit_card_details_arr: credit_card_details_arr,tcs_tax:tcs_tax,tcs:tcs, package_type : package_type,tds:tds
														},
														function (data) {
															var msg1 = data.split('-');
