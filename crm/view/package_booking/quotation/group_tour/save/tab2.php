<form id="frm_tab2">
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-xs-12 mg_bt_10">
                ...
                <select class="form-control" style="width:100%" id="cmb_tour_name" name="cmb_tour_name" onchange="tour_group_reflect(this.id, false);" title="Destination Name"> 

					<option value="">*Destination Name</option>

					<?php
						$sq=mysqlQuery("select * from tour_master where active_flag = 'Active' order by tour_name asc");
			...
			success:function(result){
				...
				if(table.rows.length == 1){
					for(var k=1; k<table.rows.length; k++){
						document.getElementById("tbl_package_tour_quotation_dynamic_train").deleteRow(k);
					}
				}else{
				...
				if(train_arr.length <= 0){
					var row = table.rows[0];
					row.cells[5].childNodes[0].value = from_date + ' 00:00';
				...
				for(var i=0; i<train_arr.length; i++){
					...
					$('#'+row.cells[2].childNodes[0].id).append(option).trigger('change');
	...
	return str.charAt(0).toUpperCase() + str.slice(1);
		...
		$.ajax({
			...
			success:function(result){
				...
				if(cruise_arr.length <= 0){
					var row = table.rows[0];
					row.cells[2].childNodes[0].value = from_date + ' 00:00';
					row.cells[3].childNodes[0].value = to_date + ' 00:00';
				}
