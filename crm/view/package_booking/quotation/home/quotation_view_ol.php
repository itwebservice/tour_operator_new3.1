...
function findImageUrl($image_path, $is_new_quotation = false) {
    ...
    if ($is_new_quotation) {
        ...
        if (file_exists($itinerary_images_path)) {
            error_log("QUOTATION_VIEW_OL: Found image in itinerary_images folder (new quotation priority): " . $itinerary_images_path);
    ...
    if (file_exists($original_file_path)) {
        error_log("QUOTATION_VIEW_OL: Found image in original location: " . $original_file_path);
        return $original_url;
    }
    ...
    if (strpos($image_path_clean, 'uploads/quotation_images/') === 0) {
        ...
        if (file_exists($quotation_images_file_path)) {
            error_log("QUOTATION_VIEW_OL: Found image in quotation_images folder (database path): " . $quotation_images_file_path);
    ...
    if (file_exists($quotation_images_file_path)) {
        error_log("QUOTATION_VIEW_OL: Found image in quotation_images folder: " . $quotation_images_file_path);
        return $quotation_images_url;
    }
    ...
    if (file_exists($crm_uploads_path)) {
        error_log("QUOTATION_VIEW_OL: Found image in CRM uploads folder: " . $crm_uploads_path);
    ...
    if (file_exists($crm_quotation_images_path)) {
        error_log("QUOTATION_VIEW_OL: Found image in CRM uploads quotation_images folder: " . $crm_quotation_images_path);
    ...
    $itinerary_images_path = "../../../../../uploads/itinerary_images/" . basename($image_path_clean);
    if (file_exists($itinerary_images_path)) {
        error_log("QUOTATION_VIEW_OL: Found image in itinerary_images folder (fallback): " . $itinerary_images_path);
        $itinerary_images_url = $project_base_url . '/uploads/itinerary_images/' . basename($image_path_clean);
        return $itinerary_images_url;
    }

    error_log("QUOTATION_VIEW_OL: Image not found in any location: " . $image_path);
    return '';
}
			...
			<tr>
				...
				<td>
					...
					if (!empty($row_itinarary['day_image']) && trim($row_itinarary['day_image']) !== '' && trim($row_itinarary['day_image']) !== 'NULL') {
						...
						$daywise_image = findImageUrl($image_path, false); // quotation_view_ol is always for existing quotations
					}
