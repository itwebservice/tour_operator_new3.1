	...
	if($checked_arr[$i] === 'true'){
		...
		if($row_tariff_count > 0){
			...
			if($mealplan_tariff_switch == 'Yes'){
				$mean_plan_query = "and meal_plan='$meal_plan_arr[$i]'";
			}
			...
			while($row_tariff_master = mysqli_fetch_assoc($row_tariff_master1)){
				...
				for($i_date=0; $i_date<sizeof($checkDate_array)-1; $i_date++){
					...
					$blackdated_count = mysqli_num_rows(mysqlQuery("select * from hotel_blackdated_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') $mean_plan_query"));
					$day = date("l", strtotime($checkDate_array[$i_date]));
					$weekenddated_count = mysqli_num_rows(mysqlQuery("select * from hotel_weekend_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and day='$day' $mean_plan_query"));
					$qq = "select * from hotel_contracted_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') $mean_plan_query";
					$contracted_count = mysqli_num_rows(mysqlQuery($qq));
					if($blackdated_count>0){

						$sq_tariff = mysqli_fetch_assoc(mysqlQuery("select * from hotel_blackdated_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') $mean_plan_query"));
						$arr = array(
						...
						$sq_tariff = mysqli_fetch_assoc(mysqlQuery("select * from hotel_weekend_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and day='$day' $mean_plan_query "));
						$arr = array(
							'room_cost' => ($from_currency_rate / $to_currency_rate) * $sq_tariff['double_bed'],
							'child_with_bed' => ($from_currency_rate / $to_currency_rate) * $sq_tariff['child_with_bed'],
							'child_without_bed' =>($from_currency_rate / $to_currency_rate) *  $sq_tariff['child_without_bed'],
						...
						$sq_tariff = mysqli_fetch_assoc(mysqlQuery("select * from hotel_contracted_tarrif where pricing_id='$row_tariff_master[pricing_id]' and room_category = '$room_cat_arr[$i]' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') $mean_plan_query"));
						$arr = array(
							'room_cost' => ($from_currency_rate / $to_currency_rate) * $sq_tariff['double_bed'],
							'child_with_bed' => ($from_currency_rate / $to_currency_rate) * $sq_tariff['child_with_bed'],
							'child_without_bed' => ($from_currency_rate / $to_currency_rate) * $sq_tariff['child_without_bed'],
