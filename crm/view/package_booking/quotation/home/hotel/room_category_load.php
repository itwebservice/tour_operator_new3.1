...
while($row_hotel = mysqli_fetch_assoc($sq_hotel))
{
    ...
    if(!in_array($sq_cat['room_category'],$room_category_arr)){
        $selected = '';
        if(strtolower($sq_cat['room_category']) == 'deluxe room' || strtolower($sq_cat['room_category']) == 'deluxe'){
            $selected = 'selected';
            $deluxe_available = true;
        }
?>
	<option value="<?php echo $sq_cat['room_category'] ?>" <?php echo $selected; ?>><?php echo $sq_cat['room_category'] ?></option>
<?php	
array_push($room_category_arr,$sq_cat['room_category']);
    ...
    if(!in_array($sq_cat['room_category'],$room_category_arr)){
        $selected = '';
        if(!$deluxe_available && (strtolower($sq_cat['room_category']) == 'deluxe room' || strtolower($sq_cat['room_category']) == 'deluxe')){
            $selected = 'selected';
            $deluxe_available = true;
        }
?>
