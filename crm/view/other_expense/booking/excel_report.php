...
while($row_expense = mysqli_fetch_assoc($sq_expense)){
    ...
    $sq_paid = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from other_expense_payment_master where expense_type_id='$row_expense[expense_type_id]' and supplier_id='$row_expense[supplier_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
header('Content-Disposition: attachment;filename="Other Expense Booking('.date('d-m-Y H:i').').xls"');
