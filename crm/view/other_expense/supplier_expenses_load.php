...
<table class="table table-bordered table-hover" id="tbl_supplier_expense" style="margin: 0 !important; padding-bottom: 0 !important;">
	...
	<?php
	$count=1;
	$sq_supplier = mysqlQuery("select * from other_expense_master where supplier_id='$supplier_id'");
	while($row_supplier = mysqli_fetch_assoc($sq_supplier)){					

		$sq_supplier_p = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as payment_amount from other_expense_payment_master where supplier_id='$supplier_id' and expense_type_id='$row_supplier[expense_type_id]' and expense_id='$row_supplier[expense_id]'  and clearance_status!='Cancelled'"));

		$sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$row_supplier[expense_type_id]'"));		
