...
$sq_tourwise_id = mysqli_fetch_assoc(mysqlQuery("select id from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id = '$tour_group_id' and traveler_group_id = '$traveler_group_id' "));

 $tourwise_id = $sq_tourwise_id['id'];

 $sq = mysqlQuery("select total_travel_expense from tourwise_traveler_details where id='$tourwise_id'");
 while($row = mysqli_fetch_assoc($sq))
 {
 	$total_travel_expense = $row['total_travel_expense'];
 }

 $sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as total from payment_master where tourwise_traveler_id='$tourwise_id' and payment_for='traveling' "));
	...
	<div class="col-md-7">
		...
		<div class="table-responsive">
			<table id="tbl_travel_refund_amount" class="table table-bordered table-hover">
				...
				<tr class="text-center">
					<td colspan="2">
						<button class="btn btn-success" <?php echo $btn_status; ?> onclick="extra_travel_amount_refund_status_update()"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Save</button>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="table-responsive">
			...
			<table id="tbl_travel_refund_amount" class="table table-bordered table-hover">
				...
				<tbody>
					<?php 
					$sq_refund = mysqlQuery("select * from refund_travel_extra_amount where tourwise_traveler_id='$tourwise_id'");
