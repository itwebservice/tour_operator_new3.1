...
<div class="row">
	<div class="col-md-12  col-sm-10  col-xs-12"> <div class="table-responsive">
		<table class="table table-bordered table-hover mg_bt_0">
			...
			$count = 0;
			$disabled_count= 0;
			$sq_hotel_entries = mysqlQuery("select * from hotel_booking_entries where booking_id='$booking_id'");
			while($row_entry = mysqli_fetch_assoc($sq_hotel_entries)){
				$checked = "";
				...
				$sq_hotel_info = mysqli_fetch_assoc(mysqlQuery("select * from hotel_master where hotel_id='$row_entry[hotel_id]'"));
				?>
				<tr class="<?= $bg ?>">
					<td><?= ++$count ?></td>
		...
		<form id="frm_refund" class="mg_bt_150">
			...
			<div class="row">
				...
				<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
					...
					<?php }else{
						?>
						<option value="<?= $sq_hotel_info['tax_value'] ?>"><?= $sq_hotel_info['tax_value'] ?></option>
					<?php } ?>
					</select>
				</div>
				<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
...
function cancel_booking(){
	...
	if(len!=pass_count){
		...
		error_msg_alert('All the hotels have been already cancelled');
	}
	else
	{
