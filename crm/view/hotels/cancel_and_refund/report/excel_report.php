...
if(($booking_id!="")){
    $sq_entry_date = mysqli_fetch_assoc(mysqlQuery("select * from hotel_booking_master where booking_id='$booking_id' and delete_status='0'"));
...
$row_count++;
    ...
    while($row_refund = mysqli_fetch_assoc($sq_refund)){
        ...
        ->setCellValue('E'.$row_count, get_hotel_booking_refund_id($row_refund['refund_id'],$year1))
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('J'.$row_count, 'Total Refund: '.number_format(($total_refund - $sq_pending_amount - $sq_cancel_amount),2));
...
header('Content-Disposition: attachment;filename="Hotel Booking Refund('.date('d-m-Y H:i').').xls"');
