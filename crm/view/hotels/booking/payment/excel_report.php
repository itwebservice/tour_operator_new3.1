...
while($row_payment = mysqli_fetch_assoc($sq_payment))
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, get_hotel_booking_id($sq_booking['booking_id'],$year))
        ...
        ->setCellValue('I'.$row_count, $row_payment['payment_amount']+$row_payment['credit_charges'].$currency_amount);
	...
	$objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':I'.$row_count)->applyFromArray($borderArray);    
        ...
        ->setCellValue('H'.$row_count, 'Cancelled Amount : '.number_format($sq_cancel_amount,2))
...
header('Content-Disposition: attachment;filename="HotelPayment('.date('d-m-Y H:i').').xls"');
