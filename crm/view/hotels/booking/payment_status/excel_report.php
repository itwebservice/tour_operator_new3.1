...
while ($row_booking = mysqli_fetch_assoc($sq_booking)) {
    ...
    $row_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Hotel' and estimate_type_id='$row_booking[booking_id]' and delete_status='0'"));
// currency_conversion
...
$currency_amount_3= '';
    ...
    $currency_amount1 = currency_conversion($currency,$row_booking['currency_code'],$net_amount);
            ...
            $currency_amount2 = currency_conversion($currency,$row_booking['currency_code'],$paid_amount);
if($row_booking['currency_code'] !='0' && $currency != $row_booking['currency_code']){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('S' . $row_count, number_format($paid_amount, 2).$currency_amount_2)
        ->setCellValue('T' . $row_count, number_format($bal, 2))
        ->setCellValue('U' . $row_count, $due_date)
        ->setCellValue('V' . $row_count, number_format($total_purchase, 2).$currency_amount_3)
