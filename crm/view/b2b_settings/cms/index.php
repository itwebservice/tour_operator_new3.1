...
<div class="div_left type-02">
    <h5 class="div_left-title">General</h5>
    <ul class="nav nav-pills" id="index_page">
        ...
        <li role="presentation" class="dropdown">
            <a><i class="fa fa-image" aria-hidden="true"></i> 1- Banner Images</a>
        </li>
        <li role="presentation" class="dropdown">
        ...
        <li role="presentation" class="dropdown">
            <a><i class="fa fa-paste" aria-hidden="true"></i> 7- Call To Action</a>
        </li>
        <li role="presentation" class="dropdown">
...
<script type="text/javascript">
    $.post('cms/get_selected_section.php', {
        section_name: 1
    }, function(data) {
    ...
    ul.onclick = function(event) {
        ...
        var selected_item = selected.split('</i> ');
        var temp = selected_item[1].split('-');
        ...
        $('#section_data_form').validate({
            ...
            submitHandler: function(form) {
                ...
                if (section_name == '1') {
                    ...
                    // Why choose us
                    if (display_status == '') {
                        error_msg_alert("Please Select Display Status");
                        return false;
                    }
                    var images_array = new Array();
                    for (var j = 0; j < 5; j++) {
                        ...
                        if (j == 0) {
                            if (image_url == '') {
                                error_msg_alert("Please Select Image " + parseInt(j + 1));
                                return false;
                            }
                            if (title == '') {
                                error_msg_alert("Please Enter Title at row " + parseInt(j + 1));
                                return false;
                            }
                            if (description == '') {
                                error_msg_alert("Please Enter Description at row " + parseInt(j + 1));
                                return false;
                            }
                        }
                    ...
                    if (heading == '') {
                        error_msg_alert("Please Enter Heading");
                        return false;
                    }
                    ...
                    if (uploaded_count > 0) {
                        for (var j = 0; j < (uploaded_count); j++) {
                            ...
                            if (icon == '') {
                                error_msg_alert("Please Select Icon " + parseInt(j + 1));
                                return false;
                            }
                            if (title == '') {
                                error_msg_alert("Please Enter Title at row " + parseInt(j + 1));
                    ...
                    for (var i = 1; i <= 4; i++) {
                        ...
                        if (city_name != '' && hotel_name == '') {
                            error_msg_alert('Select Hotel name in row ' + i);
                            return false;
                        }
                        hotel_array.push({
                            'city_id': city_name,
                    ...
                    for (var i = 0; i < rowCount; i++) {
                        ...
                        if (row.cells[0].childNodes[0].checked) {
                            if (city == "") {
                                error_msg_alert("Select City at row " + (i + 1));
                                return false;
                            }
                            if (exc == "") {
                    ...
                    var title = $("#title").val();
                    if (image_url == '') {
                        error_msg_alert("Please Select Image");
                        return false;
                    }
                    if (title == '') {
                        error_msg_alert("Please Enter Title");
                        return false;
                    }
                    if (subtitle == '') {
                        error_msg_alert("Please Enter Subtitle");
                        return false;
                    }
                    ...
                    for (var i = 0; i < rowCount; i++) {
                        ...
                        if (row.cells[0].childNodes[0].checked) {
                            ...
                            if (hotel == "") {
                                error_msg_alert("Select Hotel at row " + (i + 1));
                                return false;
                            }
                            images_array.push({
                                'city_id': city,
                    ...
                    var flag1 = validate_char_size("description", 150);
                    if (!flag1) {
                        return false;
                    }
                    for (var i = 1; i <= 3; i++) {
                        ...
                        if (dest_id != '' && image_url == '') {
                            error_msg_alert('Please select Image at row ' + i);
                            return false;
                        }
                        if (dest_id != '') {
                    ...
                    if (activeTab[1] == 'hotels') {
                        ...
                        if (count > 6) {
                            error_msg_alert('Max 6 hotels can add,you can deselect some of them!');
                            return false;
                        }
                        for (var i = 0; i < rowCount; i++) {
                            var row = table.rows[i];
                    ...
                    //Best selling activities
                        ...
                        if (count > 6) {
                            error_msg_alert('Max 6 activities can add,you can deselect some of them!');
                            return false;
                        }
                        for (var i = 0; i < rowCount; i++) {
                            var row = table.rows[i];
                    ...
                    //Best selling tours
                        ...
                        if (count > 6) {
                            error_msg_alert('Max 6 tours can add,you can deselect some of them!');
                            return false;
                        }
                        for (var i = 0; i < rowCount; i++) {
                            var row = table.rows[i];
                    ...
                    //Best selling tours
                        ...
                        var flag1 = validate_char_size("copyright_text", 60);
                        if (!flag1) {
                            return false;
                        }
                        images_array.push({
                ...
                $.ajax({
                    ...
                    success: function(message) {
                        ...
                        if (data[0] == 'erorr') {
                            ...
                            } else if (section_name == '2' || section_name == '4' ||
                                section_name == '5' || section_name == '6' ||
                                section_name == '7' || section_name == '8' ||
                                section_name == '9' || section_name == '10') {
                                $.post('cms/get_selected_section.php', {
                                    section_name: section_name
                                }, function(data) {
                                    $('#section_data_form').html(data);
