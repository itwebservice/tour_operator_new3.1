...
if($visa_id != ''){
  ...
  $yr = explode("-", $date);
  $year = $yr[0];
  $invoice_id = get_visa_booking_id($visa_id,$year);
}
//$invoice_id = ($visa_id!="") ? get_visa_booking_id($visa_id): "";
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Visa Booking Refund')
...
while($row_refund_entry = mysqli_fetch_assoc($sq_refund_entries)){
  ...
  $objPHPExcel->getActiveSheet()
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, get_visa_booking_id($row_refund['visa_id'],$year))
        ->setCellValue('D'.$row_count, $traveler_name)
        ->setCellValue('E'.$row_count, get_visa_booking_refund_id($row_refund['refund_id'],$year1))
...
header('Content-Disposition: attachment;filename="Visa Booking Refund('.date('d-m-Y H:i').').xls"');
