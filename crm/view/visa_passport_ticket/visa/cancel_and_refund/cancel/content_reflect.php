...
<div class="row mg_tp_20 mg_bt_20">
	...
	<div class="col-md-10  col-md-offset-1 col-sm-12 col-xs-12">
		<div class="widget_parent-bg-img bg-img-red">
			...
			$sq_payment_info = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from visa_payment_master where visa_id='$visa_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
			$sq_refund_info = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from visa_refund_master where visa_id='$visa_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

			if ($sq_visa_info['visa_issue_amount'] == '') {
				$visa_amount = 0;
			} else {
				$visa_amount = $sq_visa_info['visa_issue_amount'];
			}
			if ($sq_visa_info['service_charge'] == '') {
				$service_charge = 0;
			} else {
				$service_charge = $sq_visa_info['service_charge'];
			}
			if ($sq_visa_info['service_tax_subtotal'] == '') {
				$subtotal_amount = 0;
			} else {
				$subtotal_amount = $sq_visa_info['service_tax_subtotal'];
			}
			$tax_show = '';
			$newBasic = $visa_amount;
			...
			if (isset($bsmValues[0]->service)) {   //inclusive service charge
				$newBasic = $visa_amount;
				$newSC = $service_tax_amount + $service_charge;
				$tax_name = '';
			} else {
				$tax_show = $name . ' ' . $currency_code . " " . $sq_visa_info['service_tax_subtotal'];
			...
			if (isset($bsmValues[0]->markup)) { //inclusive markup
				$newBasic = $visa_amount + $sq_visa_info['markup'] + $markupservice_tax_amount;
			} else {
				$newBasic = $visa_amount;
				$newSC = $service_charge + $sq_visa_info['markup'];
				$tax_show = $name . ' ' . $currency_code . ' ' . number_format($markupservice_tax_amount + $service_tax_amount, 2);
			}
			////////////Basic Amount Rules
			if (isset($bsmValues[0]->basic)) { //inclusive markup

				$newBasic = $visa_amount + $service_tax_amount;
				$tax_show = '';
			}
			begin_widget();
			$title_arr = array("Basic Sale", "Serv. Charge", " Tax", "Markup", "Markup Tax", "Roundoff", "Total Amount", "Paid Amount");
			$content_arr = array((float)($visa_amount), (float)($service_charge), (float)($service_tax_amount), (float)($sq_visa_info['markup']), (float)($markupservice_tax_amount), (float)($sq_visa_info['roundoff']), (float)($sq_visa_info['visa_total_cost']), (float)($sq_payment_info['sum']));
			$percent = ($sq_visa_info['visa_total_cost'] != '0') ? ((float)($sq_payment_info['sum']) / (float)($sq_visa_info['visa_total_cost'])) * 100 : 0;
			$percent = round($percent, 2);
			$label = "Visa Fee Paid In Percent";
			widget_element($title_arr, $content_arr, $percent, $label);
			end_widget(); ?>
			<input type="hidden" id="total_sale" name="total_sale" value="<?= $sq_visa_info['visa_total_cost'] ?>">
		...
		<div class="table-responsive">
			...
			<table class="table table-hover table-bordered mg_bt_0" style="margin: 0 !important;">
				...
				<tbody>
					...
					$count = 0;
					$disabled_count = 0;
					$sq_visa_entries = mysqlQuery("select * from visa_master_entries where visa_id='$visa_id'");
					...
					?>
						<tr class="<?= $bg ?>">
							...
							<td>
								<input type="checkbox" class="traveler_names" id="chk_entry_id_<?= $count ?>" name="chk_entry_id" <?= $checked ?> value="<?= $row_entry['entry_id'] ?>">
							</td>
						</tr>
					<?php
					}
...
<hr>
<?php
$sq_cancel_count = mysqli_num_rows(mysqlQuery("select * from visa_master_entries where visa_id='$visa_id' and status='Cancel'"));
...
<script>
	function cancel_booking() {
		...
		if (len != pass_count) {
			error_msg_alert('Please select all passengers for cancellation.');
			...
			$('#vi_confirm_box').vi_confirm_box({
				...
				callback: function(data1) {
					if (data1 == "yes") {
						...
						$.ajax({
							type: 'post',
							url: base_url + 'controller/visa_passport_ticket/visa/cancel/cancel_booking.php',
							data: {
				...
				$('#vi_confirm_box').vi_confirm_box({
					...
					callback: function(data1) {
						if (data1 == "yes") {
							...
							$.ajax({
								type: 'post',
								url: base_url + 'controller/visa_passport_ticket/visa/cancel/refund_estimate_update.php',
								data: {
									visa_id: visa_id,
