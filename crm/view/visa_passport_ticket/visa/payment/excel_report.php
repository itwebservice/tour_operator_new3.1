...
while($row_visa_payment = mysqli_fetch_assoc($sq_visa_payment)){
    ...
    $sq_paid_amount = $sq_paid_amount + $row_visa_payment['payment_amount'] + $row_visa_payment['credit_charges'];
      ...
      if($row_visa_payment ['currency_code'] ==''){
        $currency_code_3 = $sq_visa_info['currency_code'];
      }
      else{
        $currency_code_3= $row_visa_payment ['currency_code'];
      }

    $currency_amount1 = currency_conversion($currency,$currency_code_3,$row_visa_payment['payment_amount']+$row_visa_payment['credit_charges']);
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, get_visa_booking_payment_id($row_visa_payment['payment_id'],$year1))
        ->setCellValue('D'.$row_count, get_visa_booking_id($row_visa_payment['visa_id'],$year))
        ...
        ->setCellValue('H'.$row_count, number_format($row_visa_payment['payment_amount']+$row_visa_payment['credit_charges'],2).$currency_amount);
...
header('Content-Disposition: attachment;filename="VisaPayment('.date('d-m-Y H:i').').xls"');
