...
while ($row_visa = mysqli_fetch_assoc($sq_visa)) {
	...
	$i = 0;
	$p_due_date = '';
	$sq_purchase_count = mysqli_num_rows(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Visa' and estimate_type_id='$row_visa[visa_id]' and delete_status='0'"));
...
$currency_amount_3= '';
	...
	if ($app_invoice_format == 4)
		$url1 = BASE_URL . "model/app_settings/print_html/invoice_html/body/tax_invoice_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=&service_tax_per=&service_tax=$service_tax&net_amount=$net_amount&service_charge=$service_charge&total_paid=$total_paid&balance_amount=$balance_amount&sac_code=$sac_code&branch_status=$branch_status&visa_id=$visa_id&pass_count=$pass_count&credit_card_charges=$credit_card_charges";
	else
		$url1 = BASE_URL . "model/app_settings/print_html/invoice_html/body/visa_body_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=&service_tax_per=&service_tax=$service_tax&net_amount=$net_amount&service_charge=$service_charge&total_paid=$total_paid&balance_amount=$balance_amount&sac_code=$sac_code&branch_status=$branch_status&visa_id=$visa_id&credit_card_charges=$credit_card_charges";
	...
	$sq_incentive_count = mysqli_num_rows(mysqlQuery("select * from booker_sales_incentive where booking_id='$row_visa[visa_id]' and service_type='Visa Booking'"));
	...
	//Currency conversion
	$currency_amount1 = currency_conversion($currency,$row_visa['currency_code'],$total_bal);
	...
	$currency_amount2 = currency_conversion($currency,$row_visa['currency_code'],$total_paid);
	if($row_visa['currency_code'] !='0' && $currency != $row_visa['currency_code']){
	...
	$temp_arr = array("data" => array(
		...
		'<button class="btn btn-info btn-sm" id="packagev_btn-' . $row_visa['visa_id'] . '" onclick="visa_view_modal(' . $row_visa['visa_id'] . ');btnDisableEnable(this.id)" id="visa_view_modal_btn' . $row_visa['visa_id'] . '" data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
		...
		number_format($total_bal, 2) . $currency_amount,
		number_format($paid_amount, 2).$currency_amount_2,
		'<button class="btn btn-info btn-sm" id="paymentv_btn-' . $row_visa['visa_id'] . '" onclick="payment_view_modal(' . $row_visa['visa_id'] . ');btnDisableEnable(this.id)" id="payment_view_modal_btn' . $row_visa['visa_id'] . '"  data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
		number_format($bal, 2),
		$due_date,
		number_format($total_purchase, 2).$currency_amount_3,
		'<button class="btn btn-info btn-sm" id="supplierv_btn-' . $row_visa['visa_id'] . '" onclick="supplier_view_modal(' . $row_visa['visa_id'] . ');btnDisableEnable(this.id)" id="supplier_view_modal_btn' . $row_visa['visa_id'] . '" data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
		$branch_name,
		$emp_name,
