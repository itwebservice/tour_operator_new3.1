...
while ($row_ticket = mysqli_fetch_assoc($sq_ticket)) {
    ...
    if ($row_ticket['cancel_type'] == '1') {
        ...
        $bal = (($total_sale - (float)($cancel_estimate[0]->ticket_total_cost)) + $cancel_amount) - $paid_amount;
    } else {
...
$row_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Flight' and estimate_type_id='$row_ticket[ticket_id]' and delete_status='0'"));
// currency_conversion
if($total_purchase ==''){
	$currency_amount_3 =0;
}
...
$currency_amount_3= '';
    ...
    $cancel_estimate = json_decode($row_ticket['cancel_estimate']);
    $sales_return = ($row_ticket['cancel_type'] == 1 || $row_ticket['cancel_type'] == 2 || $row_ticket['cancel_type'] == 3) ? number_format((float)($cancel_estimate[0]->ticket_total_cost), 2) : 'NA';

    // currency conversion
	$currency_amount1 = currency_conversion($currency,$row_ticket['currency_code'],$total_bal);
	...
	$currency_amount2 = currency_conversion($currency,$row_ticket['currency_code'],$paid_amount);
	if($row_ticket['currency_code'] !='0' && $currency != $row_ticket['currency_code']){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('S' . $row_count, number_format($total_bal, 2).$currency_amount)
        ->setCellValue('T' . $row_count, number_format($paid_amount, 2).$currency_amount_2)
        ->setCellValue('U' . $row_count, $sales_return)
        ->setCellValue('V' . $row_count, number_format($bal, 2))
        ->setCellValue('W' . $row_count, $due_date)
        ->setCellValue('X' . $row_count, number_format($total_purchase, 2).$currency_amount_3)
