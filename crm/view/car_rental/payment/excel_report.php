...
while($row_payment = mysqli_fetch_assoc($sq_payment))
{
  ...
  $payment_id = get_car_rental_booking_payment_id($row_payment['payment_id'],$year1);
   ...
   global $currency;
				...
				// currency conversion
			$currency_amount1 = currency_conversion($currency,$row_payment['currency_code'],number_format($row_payment['payment_amount'] +
					$row_payment['credit_charges'],2));
	if($row_payment['currency_code'] !='0' && $currency != $row_payment['currency_code']){
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, get_car_rental_booking_id($row_payment['booking_id'],$year))
        ...
        ->setCellValue('H'.$row_count, number_format($row_payment['payment_amount'] + $row_payment['credit_charges'],2).$currency_amount);
  ...
  $objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':H'.$row_count)->applyFromArray($content_style_Array);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, 'Total Amount : '.number_format($total_paid_amt,2))
        ...
        ->setCellValue('G'.$row_count, 'Total Paid  :'.number_format(($total_paid_amt - $sq_pending_amount - $sq_cancel_amount),2));
...
header('Content-Disposition: attachment;filename="Car Rental Payment('.date('d-m-Y H:i').').xls"');
