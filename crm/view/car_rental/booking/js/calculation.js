...
function get_auto_values(booking_date, sub_total, payment_mode, service_charge, markup, type, charges_flag, amount_type, change = false) {
    ...
    if (charges_flag === 'true') {
        ...
        if (parseInt(markup_amount_rules_array.length) === 0) {
            ...
            if (isNaN(markup_cost)) markup_cost = 0.00;
            if (change && Number($('#' + markup).val()) != markup_cost && $('#' + markup).val() !== '0.00') {
    ...
    /////////////////Tax End///////////////////////
    if (type === 'save') calculate_total_fees(sub_total);
    else calculate_total_fees(sub_total, '1');
}
                    ...
                    case '1':
                        ...
                        if (customer !== '0' && customer !== '') {
                            $.ajax({
                                ...
                                'global': false,
                                'dataType': 'html',
                                'url': "get_customer_operator_state.php",
...
function calculate_charges(rules_array, type, basic_amount, markup_amount1) {
    ...
    if (rules_array.length) {
        ...
        switch (apply_on) {
            case "1":
                //Per pax
                total_pax = (total_pax === '') ? 0 : total_pax;
                var service_fee = (rules_array[0].fee_type === 'Flat') ? parseFloat(rules_array[0].fee) * (parseInt(total_pax)) : (parseFloat(target_amount) * parseFloat(rules_array[0].fee) / 100) * (parseInt(total_pax));
                return service_fee;
                break;
            case '6':
            case '2':
