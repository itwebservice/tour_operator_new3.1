...
while ($row_booking = mysqli_fetch_assoc($sq_booking)) {
    ...
    if ($balance_amt >= 0) {
        $total_balance = $total_balance + $balance_amt;
    } else {
        $total_refund = $total_refund + abs($balance_amt);
    }
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('J' . $row_count, number_format((float)($row_booking['service_charge']) + (float)($row_booking['markup']), 2))
