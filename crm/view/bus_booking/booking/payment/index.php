...

{
    ...
    window.location = 'payment/excel_report.php?customer_id=' + customer_id + '&booking_id=' + booking_id +
        '&payment_from_date=' + payment_from_date + '&payment_to_date=' + payment_to_date + '&financial_year_id=' +
        financial_year_id + '&payment_mode=' + payment_mode + '&cust_type=' + cust_type + '&company_name=' +
...
function cash_bank_receipt_generate() {
    ...
    var url = base_url + "view/bank_receipts/bus_booking_payment/cash_bank_receipt.php?payment_id_arr=" +
...
function cheque_bank_receipt_generate() {
    ...
    var url = base_url + "view/bank_receipts/bus_booking_payment/cheque_bank_receipt.php?payment_id_arr=" +
...
function whatsapp_send_r(booking_id, payment_amount, base_url) {
    $.post(base_url + 'controller/bus_booking/payment/receipt_whatsapp_send.php', {
        booking_id: booking_id,
    ...
    $('#vi_confirm_box').vi_confirm_box({
        callback: function(data1) {
            if (data1 == "yes") {
                var branch_status = $('#branch_status').val();
                var base_url = $('#base_url').val();
                $.post(base_url + 'controller/bus_booking/payment/payment_delete.php', {
                    payment_id: payment_id
                }, function(data) {
