			...
			<table class="table table-bordered bg_white cust_table" id="exc_payment_list" style="margin:20px 0 !important">
				...
				<tbody>
					...
					while ($row_exc_payment = mysqli_fetch_assoc($sq_exc_payment)) {
						...
						if ($row_exc_payment['clearance_status'] == "Cleared") {
							$bg = 'success';
						}
						if ($row_exc_payment['clearance_status'] == "") {
							$bg = '';
						}
						...
						$total_paid = currency_conversion($currency, $sq_exc_info['currency_code'], (float)($row_exc_payment['payment_amount']) + (float)($row_exc_payment['credit_charges']));
