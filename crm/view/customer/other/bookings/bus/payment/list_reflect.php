...
<table class="table table-bordered bg_white cust_table" id="tbl_list" style="margin:20px 0 !important">
	...
	<tbody>
		...
		while($row_payment = mysqli_fetch_assoc($sq_payment)){
			...
			$url1 = BASE_URL."model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=yes&outstanding=$outstanding&table_name=bus_booking_payment_master&customer_field=booking_id&in_customer_id=$row_payment[booking_id]&status=$row_payment[status]&branch_admin_id=$sq_bus_info[branch_admin_id]";
			?>
			<tr class="<?= $bg?>">				
				<td><?= $count ?></td>				
