...
$sq_location = mysqli_fetch_assoc(mysqlQuery("select * from b2b_transfer_master where entry_id='$entry_id'"));
?>
<form id="frm_update">
    <input type="hidden" id="entry_id" name="entry_id" value="<?= $sq_location['entry_id'] ?>">

    <div class="modal fade" id="update_modal" data-backdrop="static" data-keyboard="false" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <select id="vehicle_type1" name="vehicle_type1" style="width:100%" title="Vehicle Type"
                                data-toggle="tooltip" class="form-control" required>
                                <option value="<?= $sq_location['vehicle_type'] ?>"><?= $sq_location['vehicle_type'] ?>
                                </option>
                                <?php get_vehicle_types(); ?>
                            </select>
                        </div>
                        <div class="col-sm-4">
                            <input type="text" id="vehicle_name1" name="vehicle_name1"
                                onchange="locationname_validate(this.id);" placeholder="*Vehicle Name"
                                title="Vehicle Name" value="<?= $sq_location['vehicle_name'] ?>" required>
                        </div>
                        <div class="col-sm-4">
                            <input type="number" id="seating_capacity1" name="seating_capacity1" max_length="20"
                                placeholder="*Seating Capacity" title="Seating Capacity"
                                value="<?= $sq_location['seating_capacity'] ?>" required>
                        </div>
                    </div>
                    <div class="row mg_tp_10">
                        <div class="col-sm-4">
                            <select class="<?= $active_inactive_flag ?>" name="active_flag" id="active_flag"
                                title="Status">
                    ...
                    <div class="row mg_tp_20">
                        <div class="col-sm-12">
                            ...
                            <textarea class="feature_editor" name="canc_policy1" id="canc_policy1"
                                style="width:100% !important"
                                rows="2"><?= $sq_location['cancellation_policy'] ?></textarea>
                        </div>
                    </div>
                    <div class="row text-center mg_tp_30">
                        <div class="col-md-12">
                            <button class="btn btn-sm btn-success" id="update"><i
...
function load_images(entry_id)
{
    ...
    $.ajax({
	type:'post',
	url: 'master/get_transfer_img.php',
...
function delete_image(image_id){
    ...
    $.ajax({
        type:'post',
        url: base_url+'controller/b2b_transfer/delete_image.php',
