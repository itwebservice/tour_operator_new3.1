    ...
    <div class="modal-content">
      ...
      <div class="modal-body">       
          ...
          <div class="row mg_bt_20">
              ...
              <div class="col-md-3 col-sm-6 col-xs-12">
                  <select name="currency_code" id="currency_code1" title="Currency" style="width:100%" data-toggle="tooltip" disabled required>
                      <?php
                      $sq_currencyq = mysqli_fetch_assoc(mysqlQuery("select id,currency_code from currency_name_master where id='$sq_tariff[currency_id]'"));
                      ?>
          ...
          <div class="row mg_bt_10">
            ...
            <div class="col-md-12 text-right text_center_xs">
                <?php
                $sq_tariff_count = mysqli_num_rows(mysqlQuery("select * from b2b_transfer_tariff_entries where tariff_id='$tariff_id'"));
                if($sq_tariff_count == 0){
                  include_once 'get_tariff_rows.php'; 
                }
                else{
                  ?>
                    <button type="button" class="btn btn-excel btn-sm" onclick="addRow('table_transfer_tarrif','2')" title="Add row"><i class="fa fa-plus"></i></button>
                </div>
            </div>
            <div class="row mg_bt_10">
            <div class="col-md-12">
                <div class="table-responsive">
                    ...
                    while($row_tariffentries = mysqli_fetch_assoc($sq_tariffentries)){
                      ...
                      ?>
                      <tr>
                          <td><input class="css-checkbox" id="chk_transfer<?= $count ?>-u" type="checkbox" checked><label class="css-label" for="chk_transfer<?= $count ?>-u"> </label></td>
                          ...
                          <td><select name="pickup_from" id="pickup_from<?= $count ?>-u" data-toggle="tooltip" style="width:150px;" title="Pickup Location" class="form-control app_minselect2"><!-- Pickup -->
                            ...
                            if($row_tariffentries['pickup_type'] == 'city'){
                              ...
                              $row = mysqli_fetch_assoc(mysqlQuery("select hotel_id,hotel_name from hotel_master where hotel_id='$row_tariffentries[pickup_location]'"));
                              $html = '<optgroup value="hotel" label="Hotel Name"><option value="'.$row['hotel_id'].'">'.$row['hotel_name'].'</option></optgroup>';
                            }
                            else{
                              $row = mysqli_fetch_assoc(mysqlQuery("select airport_name, airport_code, airport_id from airport_master where airport_id='$row_tariffentries[pickup_location]'"));
                          ...
                          <td><select name="drop_to" id="drop_to<?= $count ?>-u" style="width:155px;" data-toggle="tooltip" title="Drop-off Location" class="form-control app_minselect2"><!-- Drop-off -->
                            ...
                            if($row_tariffentries['drop_type'] == 'city'){
                              ...
                              $row = mysqli_fetch_assoc(mysqlQuery("select hotel_id,hotel_name from hotel_master where hotel_id='$row_tariffentries[drop_location]'"));
                              $html = '<optgroup value="hotel" label="Hotel Name"><option value="'.$row['hotel_id'].'">'.$row['hotel_name'].'</option></optgroup>';
                            }
                            else{
                              $row = mysqli_fetch_assoc(mysqlQuery("select airport_name, airport_code, airport_id from airport_master where airport_id='$row_tariffentries[drop_location]'"));
                              $airport_nam = clean($row['airport_name']);
                              $airport_code = clean($row['airport_code']);
                              $pickup = $airport_nam." (".$airport_code.")";
                              $html = '<optgroup value="airport" label="Airport Name"><option value="'.$row['airport_id'].'">'.$pickup.'</option></optgroup>';
...
$(function(){
  $('#frm_tariff_update').validate({
    ...
    submitHandler:function(form){
      ...
      if(service_tax == '0'){ error_msg_alert('Select Tax(%)'); 
        $('#tariff_update').prop('disabled',false);return false;}
