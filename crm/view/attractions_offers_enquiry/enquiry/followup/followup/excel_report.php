...
$query = "
    SELECT en.enquiry_id, 
           ...
           e.first_name, 
           e.last_name
    FROM enquiry_master AS en
    LEFT JOIN enquiry_master_entries AS ef 
        ON en.enquiry_id = ef.enquiry_id
    LEFT JOIN emp_master AS e 
        ON en.assigned_emp_id = e.emp_id
    WHERE ef.followup_type != ''
    AND ef.followup_date = (
        SELECT MAX(followup_date) 
        FROM enquiry_master_entries 
        WHERE enquiry_id = en.enquiry_id
    )";
...
if ($financial_year_id != "") {
    $query .= " AND en.financial_year_id = '$financial_year_id'";
}
...
while($row_followup = mysqli_fetch_assoc($sq_enquiries)){

    $sq_enq = mysqli_fetch_assoc(mysqlQuery("select * from enquiry_master where enquiry_id='$row_followup[enquiry_id]'"));
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B'.$row_count, $emp_name)
...
header('Content-Disposition: attachment;filename="Enquiry_Followup('.date('d-m-Y H:i').').xls"');
