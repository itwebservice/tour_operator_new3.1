...
while($row_customer = mysqli_fetch_assoc($sq_customer)){
    $sq_credit = mysqli_fetch_assoc(mysqlQuery("select * from b2b_creditlimit_master where register_id='$row_customer[register_id]' order by entry_id desc"));
    $sq_currency = mysqli_fetch_assoc(mysqlQuery("select currency_code from currency_name_master where id='$row_customer[currency]'"));
    $sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$row_customer[city]'"));
    $sq_country = mysqli_fetch_assoc(mysqlQuery("select * from country_list_master where country_id='$row_customer[country]'"));
    $country = ($sq_country['country_name'] != "") ? $sq_country['country_name'].'('.$sq_country['country_code'].')' : 'NA';
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G'.$row_count, $sq_currency['currency_code'])
        ...
        ->setCellValue('Q'.$row_count, $sq_city['city_name'])
        ->setCellValue('R'.$row_count, $country)
        ->setCellValue('S'.$row_count, $sq_credit['credit_amount'])
        ->setCellValue('T'.$row_count, $row_customer['deposite'])
        ->setCellValue('U'.$row_count, ($row_customer['approval_status']!='')?$row_customer['approval_status']:'Pending');
...
header('Content-Disposition: attachment;filename="B2B Customers Report('.date('d-m-Y H:i').').xls"');
