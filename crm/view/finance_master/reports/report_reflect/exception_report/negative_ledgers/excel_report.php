...
if($ledger_name != ''){
  $sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$ledger_name'"));
}
...
$sq_ledger_info = mysqlQuery($query);
while($row_ledger = mysqli_fetch_assoc($sq_ledger_info))
{
  ...
  $sq_le_name = mysqlQuery($query1);  
  while($row_finance = mysqli_fetch_assoc($sq_le_name))
  {
...
header('Content-Disposition: attachment;filename="Negative Ledgers Report('.date('d-m-Y H:i').').xls"');
