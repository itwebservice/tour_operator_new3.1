...
$transaction_bal = 0;
if(($from_date!="" && $to_date!="")){
  $sq_credit = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from bank_cash_book_master where payment_date<'$from_date' and payment_type='Bank' and payment_side='Credit' and  clearance_status!='Cancelled'"));
  $sq_debit = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from bank_cash_book_master where payment_date<'$from_date' and payment_type='Bank' and payment_side='Debit' and  clearance_status!='Cancelled'"));
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, "Payment ID")
...
while($row = mysqli_fetch_assoc($sq)){
  ...
  $sq_bank_info = mysqli_fetch_assoc(mysqlQuery("select * from bank_master where bank_id='$row[bank_id]'"));
  ...
  if($row['payment_amount'] != 0){
    ...
    $emp_name = ($row['emp_id'] != 0) ? $sq_emp['first_name'].' '.$sq_emp['last_name']: 'Admin';
        ...
        ->setCellValue('I'.$row_count, ($row['bank_id'] != '0') ? $sq_bank_info['bank_name'].':'.$sq_bank_info['branch_name'] : 'NA')
...
header('Content-Disposition: attachment;filename="Bank_Book_Register('.date('d-m-Y H:i').').xls"');
