...
while ($row_booking = mysqli_fetch_assoc($sq_booking)) {
  ...
  while ($row_package = mysqli_fetch_assoc($sq_pacakge)) {
    ...
    if (strtotime($till_date1) < strtotime($due_date)) {
      ...
      if ($total_days > '30' && $total_days <= '60') {
        $group2  += $pending_amt;
      }
      if ($total_days > '60' && $total_days <= '90') {
        $group3  += $pending_amt;
      }
      if ($total_days > '90' && $total_days <= '120') {
        $group4 += $pending_amt;
      }
      if ($total_days > '120' && $total_days <= '180') {
        $group5 += $pending_amt;
      }
      if ($total_days > '180' && $total_days <= '360') {
        $group6 += $pending_amt;
      }
      if ($total_days > '360') {
        $group7  += $pending_amt;
...
while ($row_booking = mysqli_fetch_assoc($sq_booking)) {
  ...
  while ($row_package = mysqli_fetch_assoc($sq_pacakge)) {
    ...
    } else if ($row_package['cancel_type'] == '2' || $row_package['cancel_type'] == '3') {
      $cancel_estimate = json_decode($row_package['cancel_estimate']);
      $pending_amt = (($booking_amt - (float)($cancel_estimate[0]->ticket_total_cost)) + $cancel_est) - $total_paid;
    } else {
      $pending_amt = $booking_amt - $total_paid;
    }
...
header('Content-Disposition: attachment;filename="Receivables Ageing Report(' . date('d-m-Y H:i') . ').xls"');
