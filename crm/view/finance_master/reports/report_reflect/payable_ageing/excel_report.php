...
while ($row_supplier = mysqli_fetch_assoc($sq_supplier)) {
  ...
  while ($row_package = mysqli_fetch_assoc($sq_pacakge)) {
    ...
    if ($row_package['purchase_return'] == '1') {
      ...
      $pending_amt = (($booking_amt - (float)($cancel_estimate[0]->net_total)) + $cancel_est) - $total_paid;
    } else {
...
header('Content-Disposition: attachment;filename="Payables Ageing Report(' . date('d-m-Y H:i') . ').xls"');
