...
if($asset_name != ''){
  $sq_asset_name = mysqli_fetch_assoc(mysqlQuery("select * from fixed_asset_master where entry_id='$asset_name'"));
}
...
{
  $sq_asset = mysqli_fetch_assoc(mysqlQuery("select * from fixed_asset_master where entry_id ='$row_query[asset_id]'"));
  $sq_depr = mysqli_fetch_assoc(mysqlQuery("select sum(depr_till_date) as depr_till_date from fixed_asset_entries where asset_id ='$row_query[asset_id]' and asset_ledger ='$row_query[asset_ledger]'"));
  $closing_c_amount = $row_query['purchase_amount'] - $sq_depr['depr_till_date'];

  $sq_latest = mysqli_fetch_assoc(mysqlQuery("select * from fixed_asset_entries where asset_id ='$row_query[asset_id]' and asset_ledger ='$row_query[asset_ledger]' order by entry_id desc"));
...
header('Content-Disposition: attachment;filename="Fixed_Asset_Register('.date('d-m-Y H:i').').xls"');
