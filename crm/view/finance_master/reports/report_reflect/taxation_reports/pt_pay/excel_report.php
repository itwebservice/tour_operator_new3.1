...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('E'.$row_count, "Gross Salary")

        ->setCellValue('F'.$row_count, "PT Deducted");
...
while($row_query = mysqli_fetch_assoc($sq_query))
{
  $sq_emp = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$row_query[emp_id]'"));
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('E'.$row_count, $row_query['gross_salary'])

        ->setCellValue('F'.$row_count, $row_query['pt']);
...
header('Content-Disposition: attachment;filename="PT Payable('.date('d-m-Y H:i').').xls"');
