...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('C' . $row_count, "Purchase/Expense_ID")
    ->setCellValue('D' . $row_count, "Purchase_Date")
...
while ($row_query = mysqli_fetch_assoc($sq_query)) {
    $tds_on_amount = (float)($row_query['basic_cost']) + (float)($row_query['non_recoverable_taxes']) + (float)($row_query['service_charge']) + (float)($row_query['other_charges']);
    ...
    if ($row_query['tds'] != '0') {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C' . $row_count, $row_query['estimate_id'])
            ->setCellValue('D' . $row_count, get_date_user($row_query['purchase_date']))

            ->setCellValue('E' . $row_count, $supp_name)

            ->setCellValue('F' . $row_count, ($supp_pan_no == '') ? 'NA' : strtoupper($supp_pan_no))

            ->setCellValue('G' . $row_count, number_format($tds_on_amount, 2))

            ->setCellValue('H' . $row_count, number_format($row_query['tds'], 2));

        // $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$row_count, strtoupper);
...
while ($row_query = mysqli_fetch_assoc($sq_query)) {
    ...
    if ($row_query['tds'] != '0') {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B' . $row_count, $count++)
            ->setCellValue('C' . $row_count, get_other_expense_booking_id($row_query['expense_id'], $year))

            ->setCellValue('D' . $row_count, get_date_user($row_query['created_at']))

            ->setCellValue('E' . $row_count, ($sq_supp['vendor_name'] == '') ? $sq_exp['ledger_name'] : $sq_supp['vendor_name'])

            ->setCellValue('F' . $row_count, $pan_no)
