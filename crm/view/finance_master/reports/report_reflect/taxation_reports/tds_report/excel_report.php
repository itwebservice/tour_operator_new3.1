...
while ($row_query = mysqli_fetch_assoc($sq_query)) {
  ...
  $yr = explode("-", $date);
  $year = $yr[0];
  $pass_count = mysqli_num_rows(mysqlQuery("select * from package_travelers_details where booking_id='$row_query[booking_id]'"));
  $cancle_count = mysqli_num_rows(mysqlQuery("select * from package_travelers_details where booking_id='$row_query[booking_id]' and status='Cancel'"));
  ...
  if ($pass_count != $cancle_count) {
    ...
    $tds_on_amount = (float)($row_query['net_total']) + (float)($row_query['tds']);
    if ($row_query['tds'] != '0') {
      ...
      $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C' . $row_count, get_package_booking_id($row_query['booking_id'], $year))
        ->setCellValue('D' . $row_count, get_date_user($row_query['booking_date']))
