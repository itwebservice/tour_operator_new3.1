...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, "Year")

        ->setCellValue('D'.$row_count, "User ID")
...
while($row_query = mysqli_fetch_assoc($sq_query))
{
  $sq_emp = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$row_query[emp_id]'"));
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C'.$row_count, $row_query['year'])
        ->setCellValue('D'.$row_count, $row_query['emp_id'])
...
header('Content-Disposition: attachment;filename="Provident Fund Payable('.date('d-m-Y H:i').').xls"');
