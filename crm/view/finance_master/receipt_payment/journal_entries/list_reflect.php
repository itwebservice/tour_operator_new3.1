...
$array_s = array();
$temp_arr = array();
$query = "select * from journal_entry_master where delete_status='0' ";
...
$count = 0;
$total_cr = 0;
$delete_btn_class = isset($delete_flag) ? $delete_flag : '';
...
while ($row_journal = mysqli_fetch_assoc($sq_journal)) {
	...
	$ledger_id = isset($sq_journal_entry['ledger_id']) ? $sq_journal_entry['ledger_id'] : 0;
	$entry_type = isset($sq_journal_entry['type']) ? $sq_journal_entry['type'] : '';
	$sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$ledger_id'"));
	$ledger_name = isset($sq_ledger['ledger_name']) ? $sq_ledger['ledger_name'] : '';
	...
	$debit_amount = isset($sq_journal_debit['amount']) ? floatval($sq_journal_debit['amount']) : 0;
	$total_cr += $debit_amount;
	$temp_arr = array(
		(int)(++$count),
		...
		<button class="' . $delete_btn_class . ' btn btn-danger btn-sm" onclick="delete_entry(' . $row_journal['entry_id'] . ')" title="Delete Entry"><i class="fa fa-trash"></i></button>'
	);
