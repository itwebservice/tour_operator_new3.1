...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('A'.$row_count, "Group Id")
...
while($row_gr = mysqli_fetch_assoc($sq_group)){
    ...
    if($count <= '111'){ 
        ...
        $sq_head = mysqli_fetch_assoc(mysqlQuery("select * from head_master where head_id='$sq_group2[head_id]'"));
        $group_name = $sq_group1['subgroup_name'];
    }

    $objPHPExcel->setActiveSheetIndex(0)

        ->setCellValue('A'.$row_count, $row_gr['subgroup_id'])
...
header('Content-Disposition: attachment;filename="Account Group Report('.date('d-m-Y H:i').').xls"');
