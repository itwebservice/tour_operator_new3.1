...
while ($row_booking = mysqli_fetch_assoc($sq_package)) {
    ...
    //    $sq_paid_amount1 = mysqli_fetch_assoc(mysqlQuery("select currency_code from payment_master where tourwise_traveler_id='$row_booking[id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

	//   $currency_amount2 = currency_conversion( $sq_paid_amount1['currency_code'],$row_booking['currency_code'],floatval($sq_paid_amount['sum']));
	// if($sq_paid_amount1['currency_code'] !='0' && $row_booking['currency_code'] != $sq_paid_amount1['currency_code']){
...
$row_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Group Tour' and estimate_type_id='$row_booking[tour_group_id]' and delete_status='0'"));
// currency_conversion
if($total_purchase ==''){
	$currency_amount_3 =0;
}
...
$currency_amount_3= '';
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('R' . $row_count, number_format($sq_paid_amount['sum'], 2).$currency_amount_2)
        ...
        ->setCellValue('U' . $row_count, number_format($total_purchase, 2).$currency_amount_3)
