<?php
$sq_plane_details_count = mysqli_num_rows(mysqlQuery("select * from plane_master where tourwise_traveler_id='$tourwise_id'"));
if($sq_plane_details_count==0){
    ...
    <div class="row">
        <div class="col-xs-12 text-right">
            <button type="button" class="btn btn-excel" title="Add Row" onclick="addRow('tbl_plane_travel_details_dynamic_row');event_airport('tbl_plane_travel_details_dynamic_row',4,5)"><i class="fa fa-plus"></i></button>
    ...
    <table id="tbl_plane_travel_details_dynamic_row" name="tbl_plane_travel_details_dynamic_row" class="table table-bordered table-hover no-marg pd_bt_51">
        <?php
        $count_p = 0;
        $sq_plane_details = mysqlQuery("select * from plane_master where tourwise_traveler_id='$tourwise_id'");
        while($row_plane_details = mysqli_fetch_assoc($sq_plane_details)){
            ...
            <tr>
                ...
                <td><input type="text" id="<?php echo 'txt_plane_date-'.$count_p.'p' ?>" name="<?php echo 'txt_plane_date-'.$count_p.'p' ?>" placeholder="Departure Date & Time" title="Departure Date & Time" value="<?php echo date("d-m-Y H:i", strtotime($row_plane_details['date'])) ?>" class="app_datetimepicker" onchange="get_to_datetime(this.id,'txt_arravl<?= $count_p?>p');validate_travelingDates(this.id);" style="width:156px"/></td>
                <td><input type="text" id="<?php echo 'txt_arravl'.$count_p.'p' ?>" name="<?php echo 'txt_arravl-'.$count_p.'p' ?>" placeholder="Arrival date & time" title="Arrival date & time" class="app_datetimepicker" value="<?php echo date("d-m-Y H:i", strtotime($row_plane_details['arraval_time'])) ?>" style="width:156px" onchange="validate_travelingDates(this.id);"/></td>
                ...
                <td><select name="plane_class-<?$count_p?>p" id="plane_class-<?$count_p?>p" title="Class" style="width: 200px;">
                    <option value="<?= $row_plane_details['class'] ?>"><?= $row_plane_details['class'] ?></option>
                        <?php get_flight_class_dropdown(); ?>
                    </select></td>
                <td><input type="text" id="<?php echo 'txt_plane_seats-'.$count_p.'p' ?>" name="<?php echo 'txt_plane_seats-'.$count_p.'p' ?>" placeholder="Total Seats" title="Total Seats" maxlength="2"   value="<?php echo $row_plane_details['seats'] ?>" style="width:110px"/></td>
        ...
        <script>
            $("#txt_arravl"+<?= $count_p ?>+'p').datetimepicker({ format: "d-m-Y H:i"  });
            $("#txt_plane_date-"+<?= $count_p ?>+'p').datetimepicker({ format: "d-m-Y H:i"  });
...
function generating_plane_date(){
    ...
    {
        $( "#txt_plane_date"+i+'p').datetimepicker({ format: "d-m-Y H:i"  });
        $( "#txt_arravl"+i+'p').datetimepicker({ format: "d-m-Y H:i"  });
    }             
}
