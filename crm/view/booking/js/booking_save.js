...
$('#frm_tab_3').validate({
	...
	submitHandler: function (form) {
		// for avoid double click event
		var button = $('#btn_save_booking'); 
		// Button reference
		...
		button.prop('disabled', true).text('Save');
		...
		var total_payment = parseFloat(payment_amount1);
...
function save_booking_details() {
	...
	var m_adolescence = [];
	var m_passport_no = [];
	var m_passport_issue_date = [];
	var m_passport_expiry_date = [];
	...
	var train_travel_date = [];
	var train_from_location = [];
	var train_to_location = [];
	var train_train_no = [];
	var train_travel_class = [];
	var train_travel_priority = [];
	var train_amount = [];
	var train_seats = [];
	...
	var from_city_id_arr = [];
	var to_city_id_arr = [];
	var plane_travel_date = [];
	var plane_from_location = [];
	var plane_to_location = [];
	var plane_train_no = [];
	var plane_amount = [];
	var plane_seats = [];
	var plane_company = [];
	var arravl_arr = [];
	var flight_class_arr = [];
	...
	for (var i = 0; i < rowCount; i++) {
		...
		if (row.cells[0].childNodes[0].checked) {
			...
			var travel_date_temp = row.cells[2].childNodes[0];
			var travel_date1 = $(travel_date_temp).val();
			var arravl1 = row.cells[3].childNodes[0].value;
			var plane_from_location1 = row.cells[4].childNodes[0].value;
			var to_location1 = row.cells[5].childNodes[0].value;
			var company1 = row.cells[6].childNodes[0].value;
			var class1 = row.cells[7].childNodes[0].value;
			var seats1 = row.cells[8].childNodes[0].value;
			var amount1 = row.cells[9].childNodes[0].value;
			var from_city_id1 = row.cells[10].childNodes[0].value;
			var to_city_id1 = row.cells[11].childNodes[0].value;
	...
	var cruise_dept_date_arr = [];
	var cruise_arrival_date_arr = [];
	var route_arr = [];
	var cabin_arr = [];
	var sharing_arr = [];
	var cruise_seats_arr = [];
	var cruise_amount_arr = [];
	...
	var gcurrency_code = $('#gcurrency_code').val();
	...
	var single_person = $('#txt_single_person_seats').val();
	var single_person_cost = $('#txt_single_person_expense').val();
	//**Form information 
					...
					$.post(
						...
						function (data) {
							...
							if (traveling_payment == true || traveling_payment == false) {
								...
								var contact_no = $('#txt_m_mobile_no').val();
										...
										if (result == 'yes') {
											...
											$.post(
												...
												'controller/group_tour/booking/booking_details_complete_save.php',
												{
													...
													'plane_company[]': plane_company,flight_class_arr:flight_class_arr,
													...
													basic_amount: basic_amount, roundoff: roundoff, bsmValues: bsmValues, total_discount: total_discount, credit_charges: credit_charges, credit_card_details: credit_card_details,currency_code:gcurrency_code,tcsvalue:tcsvalue,tcs:tcs,single_person:single_person,single_person_cost:single_person_cost
												},
												function (data) {
													var msg1 = data.split('-');
													booking_save_message(msg1[0]);
													$('#btn_save_booking').button('reset');
													window.open(base_url + 'view/vendor/dashboard/estimate/estimate_save_modal.php?type=Group Tour&amount=' + basic_amount + '&tour_id=' + msg1[1] + '&group_id=' + msg1[2]);
													setTimeout(() => {
														if ($('#whatsapp_switch').val() == 'on')
															whatsapp_send(tour_id, customer_id, tour_group_id, emp_id);
													}, 1000);
...
//        var tcsamount=parseFloat(parseFloat(basic_amount)+parseFloat(service_tax_amount1))*parseFloat(tcs_tax)/100;
