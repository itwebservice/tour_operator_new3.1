  ...
  $('#frm_vendor_payment_save1').validate({
      ...
      submitHandler:function(form){
        ...
        if(parseFloat(total_purchase) == '0') { 
          $('#payment_save').prop('disabled',false);error_msg_alert("Atleast one booking is required!");
              return false; }
        if(parseFloat(total_purchase) < parseFloat(advance_nullify)){ 
          $('#payment_save').prop('disabled',false);error_msg_alert("Amount to be nullify should be less or equal to the Total Purchase"); return false; }
        ...
        if(payment_mode=='Advance'){
          ...
          if(parseFloat(advance_amount) < parseFloat(payment_amount)){
            $('#payment_save').prop('disabled',false);error_msg_alert("Payment amount to be nullify should not be more than Advance"); return false; }
          if(parseFloat(advance_amount) < parseFloat(advance_nullify)){ 
            $('#payment_save').prop('disabled', false);error_msg_alert("Amount to be nullify should not be more than Advance amount"); return false; }
        }
        ...
        if(parseFloat(advance_nullify)<'0'){ 
          $('#payment_save').prop('disabled',false);error_msg_alert("Amount to be nullify should be greater than 0"); return false; };

        var total_payment_amount = parseFloat(payment_amount) + parseFloat(advance_nullify);
        if(parseFloat(total_payment_amount) > parseFloat(total_purchase)){ 
          $('#payment_save').prop('disabled',false);error_msg_alert("Total Payment should not be more than Purchase"); return false; }
