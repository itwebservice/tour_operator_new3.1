...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('J'.$row_count, "Mode")
        ->setCellValue('K'.$row_count, "Bank Name")
        ->setCellValue('L'.$row_count, "Cheque No/ID");
 ...
 $total_amount1 = (float)($row_payment['payment_amount'] ?? 0) + (float)($row_payment['credit_charge_amount'] ?? 0);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('B'.$row_count, ++$count)
         ->setCellValue('C'.$row_count,get_vendor_payment_id($payment_id, $year))
        ->setCellValue('D'.$row_count, ($row_payment['estimate_type'] =='')? 'NA': $row_payment['estimate_type'] )
	...
	$objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':L'.$row_count)->applyFromArray($borderArray);    
        ...
        ->setCellValue('I'.$row_count, "Total Amount : ".number_format($total_payment, 2))
        ->setCellValue('J'.$row_count, "Pending Clearance : ".number_format($sq_pending_amount, 2))
        ->setCellValue('K'.$row_count, "Total Cancel : ".number_format($sq_cancel_amount, 2))
        ->setCellValue('L'.$row_count, "Total Paid : ".number_format(($total_payment - $sq_pending_amount - $sq_cancel_amount), 2));
...
header('Content-Disposition: attachment;filename="Supplier Payment('.date('d-m-Y H:i').').xls"');
