		...
		while($row_estimate = mysqli_fetch_assoc($sq_estimate)){
			...
			<tr class="<?= $bg ?>">
				...
				<?php if($row_estimate['cancel_est_flag'] == '0'){ ?>				
					<td>
						<button class="btn btn-info btn-sm" class="from-control" title="Calculate Refund Estimate" onclick="save_modal(<?= $row_estimate['estimate_id'] ?>,<?= $row_estimate['cancel_est_flag'] ?>)" id="update_estimate-<?= $row_estimate['estimate_id'] ?>"><i class="fa fa-pencil-square-o"></i></button>
					</td>
				<?php }
				else{ ?>
					<td>
						<button class="btn btn-info btn-sm" class="from-control" title="View Cancellation Estimate" onclick="save_modal(<?= $row_estimate['estimate_id'] ?>,<?= $row_estimate['cancel_est_flag'] ?>)" id="update_estimate-<?= $row_estimate['estimate_id'] ?>"><i class="fa fa-eye"></i></button>
					</td>
				<?php } ?>			
			</tr>
			<?php
