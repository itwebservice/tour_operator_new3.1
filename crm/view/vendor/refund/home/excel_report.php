...
if($estimate_id!="") {
    $query = mysqli_fetch_assoc(mysqlQuery("select purchase_date from vendor_estimate where estimate_id='$estimate_id'"));
    $date = $query['purchase_date'];
    $yr = explode("-", $date);
    $year = $yr[0];
    $invoice_id = get_vendor_estimate_id($estimate_id,$year);
}else{
    $invoice_id = '';
}
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Supplier Estimate Refund')
            ->setCellValue('B3', 'Supplier Costing')
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, "Refund Date")
...
header('Content-Disposition: attachment;filename="Supplier Estimate Refund('.date('d-m-Y H:i').').xls"');
