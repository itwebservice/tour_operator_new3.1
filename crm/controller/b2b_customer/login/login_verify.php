...
$row_count = mysqli_num_rows(mysqlQuery("select * from b2b_login where username='$username' and password='$password' and status='1' and agent_code='$agent_code'"));
if($row_count>0){

	$sq = mysqli_fetch_assoc(mysqlQuery("select * from b2b_login where username='$username' and password='$password' and status='1' and agent_code='$agent_code'"));
	$register_id = $sq['register_id'];

	if($sq['agent_flag'] == '1'){
		$sq_customer = mysqli_fetch_assoc(mysqlQuery("select * from b2b_registration where register_id='$sq[user_id]'"));
	}else{
		$sq_customer1 = mysqli_fetch_assoc(mysqlQuery("select * from b2b_users where id='$sq[user_id]'"));
		$sq_customer = mysqli_fetch_assoc(mysqlQuery("select * from b2b_registration where register_id='$sq_customer1[register_id]'"));
	}

	$_SESSION['b2b_agent_code'] = $agent_code;
	$_SESSION['b2b_username'] = $username;
	$_SESSION['b2b_password'] = $password;
	$_SESSION['register_id'] = $sq_customer['register_id']; 
	$_SESSION['company_name'] = $sq_customer['company_name'];
	$_SESSION['customer_id'] = $sq_customer['customer_id'];
	$_SESSION['agent_flag'] = $sq['agent_flag'];
	$_SESSION['user_id'] = $sq['user_id'];
	global $currency;
