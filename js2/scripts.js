...
$(document).ready(function () {
	...
	$(document).on("click",".c-checkSquare .filterCheckbox",function() {
		//e.preventDefault();
		if ($(this).hasClass('st-checked')) {
			$(this).removeClass('st-checked');
            callBy=$(this).data('field');
            
			setFilter($(this).data('field'), $(this).data('value'), false);
		}
		else {
            ...
            console.log(callBy);
             console.log('b');
			setFilter($(this).data('field'), $(this).data('value'), true);
		}
	});
...
function name_validate(id) {
    ...
    if (/^[A-z ]+$/.test(pass1)) {
        ...
        error_msg_alert("Please enter valid name",base_url);
...
function enq_to_action_page(type, item_id, enq_data) {
    ...
    if (type == '4') {
		...
		var input_name = 'result' + item_id;
		var transfer_type = '';
		$('input[name=' + input_name + ']:checked').each(function () {
			transfer_type = ($(this).val());
		});
		if (transfer_type == '') {
			error_msg_alert('Please select at least one Transfer Type!',base_url);
			return false;
		}
        const result = transfer_type.split('-').slice(0, -2).join('-');
        enq_data.push(result);
    ...
    if (type == '3') {
		...
		var room_type_arr = new Array();
		for (var i = 1; i <= rooms; i++) {
			var input_name = 'result_day' + item_id+ i;
			$('input[name=' + input_name + ']:checked').each(function () {
				room_type_arr.push($(this).val());
			});
		}
		if (room_type_arr.length == 0) {
			error_msg_alert('Please select at least one Room!',base_url);
			return false;
		}
        enq_data.push(room_type_arr);
