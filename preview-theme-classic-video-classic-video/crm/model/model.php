...
$username = "root";
$password = "";
$db_name = "tourthemestore_classic_video";

global $conn;
$conn = new mysqli($servername, $username, $password, $db_name);

define('BASE_URL', 'http://localhost/preview-theme-classic-video/crm/');
// >>>>>>> Stashed changes
...
$email_icon = BASE_URL . 'images/email.png';
$phone_icon = BASE_URL . 'images/call.png';
...
{ // extending email file
  ...
  public function app_email_master($to, $content, $subject, $acc_status = '')
  {
    ...
    if ($app_smtp_status == "Yes") {
      ...
      // Set shorter timeout (10 seconds) for faster email sending
      $mail->Timeout = 10;
    }
    ...
    $mail->MsgHTML($body);
    if (!$mail->Send()) {
      // Don't echo errors if output buffering is active (for OTP responses)
      if (!ob_get_level()) {
