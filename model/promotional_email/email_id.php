...
public function email_id_save()
{
	...
	$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(email_id_id) as max from sms_email_id"));
	$email_id_id = $sq_max['max'] + 1;

	$sq_email_id_count = mysqli_num_rows(mysqlQuery("select * from sms_email_id where email_id='$email_id'"));
	...
	{
		$sq_email_id = mysqlQuery("insert into sms_email_id ( email_id_id, branch_admin_id, email_id, created_at ) values ( '$email_id_id', '$branch_admin_id', '$email_id', '$created_at' )");
		if($sq_email_id){
...
public function email_id_update()
{
	...
	$sq_email_id_count = mysqli_num_rows(mysqlQuery("select * from sms_email_id where email_id='$email_id' and email_id_id!='$email_id_id'"));
	if($sq_email_id_count>0){
		echo "error--Sorry, This Email ID already exists!";
		exit;
	}

	$sq_email_id = mysqlQuery("update sms_email_id set email_id='$email_id' where email_id_id='$email_id_id'");
	if($sq_email_id){
...
public function email_id_group_assign()
{
	...
	$branch_admin_id = $_SESSION['branch_admin_id'];
	$sq_email_count = mysqli_num_rows(mysqlQuery("SELECT * FROM `email_group_entries` WHERE email_group_id='$email_group_id'"));
	...
	if($sq_email_count <= '24'){
		...
		for($i=0; $i<sizeof($email_id_id_arr); $i++){
			...
			$sq_count = mysqli_num_rows(mysqlQuery("select * from email_group_entries where email_group_id='$email_group_id' and email_id_id=$email_id_id"));
			$email_id_id = str_replace("'","",$email_id_id);

			if($sq_count == 0){
				$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(id) as max from email_group_entries"));
				$id = $sq_max['max'] + 1;
				$sq_entry = mysqlQuery("insert into email_group_entries ( id, email_group_id, branch_admin_id, email_id_id ) values( '$id', '$email_group_id', '$branch_admin_id', '$email_id_id' )");
...
public function fetch_email_id_from_system()
{
	...
	while($row_group_tour = mysqli_fetch_assoc($sq_group_tour)){
		...
		if($email_id!=''){
			...
			$sq_email_count = mysqli_num_rows(mysqlQuery("select * from sms_email_id where email_id='$email_id'"));
			if($sq_email_count==0){
	...
	while($row_package_tour = mysqli_fetch_assoc($sq_package_tour)){
		...
		$email_id = $row_package_tour['email_id'];
		if($email_id!=''){
			...
			if($sq_email_count==0){
				...
				$sq_mobile_no = mysqlQuery("insert into sms_email_id (  email_id_id, branch_admin_id, email_id, created_at ) values ('$email_id_id', '$branch_admin_id', '$email_id', '$created_at' )");
				if(!$sq_mobile_no){
					echo "Email ID  saved successfully!";
					exit;
				}
...
public function email_id_from_csv_save()
{
	...
	if(empty($handle) === false) {
	    while(($data = fgetcsv($handle, 1000, ",")) !== FALSE){
			...
			if(!empty($email_id))
			{
				$sq_mobile_no_count = mysqli_num_rows(mysqlQuery("select * from sms_email_id where email_id='$data[0]'"));
