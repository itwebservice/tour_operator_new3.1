...
while ($row_visa = mysqli_fetch_assoc($sq_visa)) {
  ...
  if ($pass_count != $cancel_count) {
    $misc_amount = $misc_amount + $row_visa['misc_total_cost'];
    $sq_misc_booking++;
  }
}
...
$sq_b2c_booking = 0;
$b2c_amount = 0;
$sq_pass = mysqlQuery("select * from b2c_sale where DATE(created_at) = '$cur_date'");
while ($row_pass = mysqli_fetch_assoc($sq_pass)) {

  if ($row_pass['status'] != 'Cancel') {
    $costing_data = json_decode($row_pass['costing_data']);
    $net_total = (float)($costing_data[0]->net_total);
    $b2c_amount = $b2c_amount + $net_total;
    $sq_b2c_booking++;
  }
}
// //B2B booking
// $sq_b2c_booking = 0;
// $b2c_amount = 0;
// $sq_pass = mysqlQuery("select * from b2b_booking_master where DATE(created_at) = '$cur_date'");
// while($row_pass = mysqli_fetch_assoc($sq_pass)){
...
while ($row_air = mysqli_fetch_assoc($sq_air)) {
  ...
  if ($row_air['cancel_type'] == '0') {
    ...
    $air_amount = $row_air['ticket_total_cost'] - (float)($cancel_estimate[0]->ticket_total_cost);
    $sq_air_booking++;
  }
}
...
$total_sale_amount = $group_amount + $package_amount + $visa_amount + $air_amount + $train_amount + $bus_amount + $car_amount + $forex_amount + $hotel_amount + $exc_amount + $misc_amount + $b2c_amount;
...
while ($row_hotel = mysqli_fetch_assoc($sq_hotel)) {
  if ($row_hotel['purchase_return'] == '0') {
    ...
    $p_hotel += $row_hotel['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_dmc = mysqli_fetch_assoc($sq_dmc)) {
  if ($row_dmc['purchase_return'] == '0') {
    ...
    $p_dmc += $row_dmc['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_tr = mysqli_fetch_assoc($sq_trans)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $p_transport += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_tr = mysqli_fetch_assoc($sq_trans)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $cancel_estimate = json_decode($row_tr['cancel_estimate']);
    $p_car += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_visa = mysqli_fetch_assoc($sq_visa)) {
  if ($row_visa['purchase_return'] == '0') {
    ...
    $p_visa += $row_visa['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
//Cruise Vendor
...
while ($row_cruise = mysqli_fetch_assoc($sq_cruise)) {
  if ($row_cruise['purchase_return'] == '0') {
    ...
    $p_cruise += $row_cruise['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_tr = mysqli_fetch_assoc($sq_trans)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $cancel_estimate = json_decode($row_tr['cancel_estimate']);
    $p_ticket += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_exc = mysqli_fetch_assoc($sq_exc)) {
  if ($row_exc['purchase_return'] == '0') {
    ...
    $exc_ticket += $row_exc['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
//Train Vendor
...
while ($row_tr = mysqli_fetch_assoc($sq_train)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $cancel_estimate = json_decode($row_tr['cancel_estimate']);
    $p_train += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_tr = mysqli_fetch_assoc($sq_trans)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $cancel_estimate = json_decode($row_tr['cancel_estimate']);
    $p_ins += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}
...
while ($row_tr = mysqli_fetch_assoc($sq_trans)) {
  if ($row_tr['purchase_return'] == '0') {
    ...
    $cancel_estimate = json_decode($row_tr['cancel_estimate']);
    $p_other += $row_tr['net_total'] - (float)($cancel_estimate[0]->net_total);
  }
}

$total_purchase_amount = $p_hotel + $p_transport + $p_train + $p_ticket + $p_ins + $p_car + $p_other + $p_visa + $p_dmc + $exc_ticket + $p_cruise;

//*************** Office Expense *****************//
$exp_query = "select sum(total_fee) as exp_paid from other_expense_master where expense_date = '$cur_date'";
$row_exp_count = mysqli_num_rows(mysqlQuery($exp_query));
$row_exp = mysqli_fetch_assoc(mysqlQuery($exp_query));
            ...
            <table class="table table-bordered no-marg" id="tbl_emp_list" style="padding-bottom: 0 !important;">
              ...
              <tbody>
                <tr>
                  ...
                  <td><?= $sq_misc_booking ?></td>
                  <td><?= $sq_b2c_booking ?></td>
                </tr>
                <tr>
                  ...
                  <td><?= number_format($misc_amount, 2) ?></td>
                  <td><?= number_format($b2c_amount, 2) ?></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="adolence_info">
            <ul class="main_block">
              <li class="col-md-4"><span>Total Sale Amount : </span><?php echo number_format($total_sale_amount, 2); ?></li>
              <li class="col-md-4"><span>Total Purchase Amount : </span><?php echo number_format($total_purchase_amount, 2); ?></li>
              <li class="col-md-4"><span>Other Expense : </span><?php echo ($row_exp_count > 0) ? number_format((float)($row_exp['exp_paid']), 2) : 0; ?></li>
            </ul>
          </div>
        </div>
      </div>
