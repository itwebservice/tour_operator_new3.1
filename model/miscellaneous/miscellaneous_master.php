...
class miscellaneous_master
{
	public function miscellaneous_master_save()
	{
		...
		$sq_visa = mysqlQuery("insert into miscellaneous_master (misc_id, customer_id,branch_admin_id,financial_year_id, misc_issue_amount, service_charge, markup, service_tax_markup, service_tax_subtotal, misc_total_cost, created_at, due_date,emp_id,narration,service, reflections , roundoff , bsm_values,invoice_pr_id,currency_code) values ('$misc_id', '$customer_id', '$branch_admin_id','$financial_year_id', '$misc_issue_amount', '$service_charge', '$markup', '$service_tax_markup', '$service_tax_subtotal', '$misc_total_cost', '$balance_date', '$due_date', '$emp_id','$narration','$services','$reflections' , '$roundoff' , '$bsmValues','$invoice_pr_id','$currency_code')");
	...
	public function miscellaneous_master_update()
	{
		...
		if (!$sq_visa) {
			...
			global $transaction_master;
			if ((float)($old_total) != (float)($misc_total_cost)) {
