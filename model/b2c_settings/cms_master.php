...
class cms_master{
	...
	public function save(){
		...
		if($sq_settings == '0'){
			...
			if($section == '26'){
				$sq_setting = mysqlQuery("insert into  b2c_settings (setting_id,  services) values('1','$data')");
			}

			if($section == '29'){
				$sq_setting = mysqlQuery("insert into  b2c_settings (setting_id,  about_us) values('1','$data')");
			}
			...
			if($section == '26'){
				$sq_setting = mysqlQuery("update b2c_settings set services = '$data' where setting_id='1'");
			}

			if($section == '29'){
				$sq_setting = mysqlQuery("update b2c_settings set about_us = '$data' where setting_id='1'");
			}
		}
		if($section == '8'){
		...
		if($section == '25'){
			...
			$entry_id = $data[$j]->entry_id;
			$position = addslashes($data[$j]->position);
			$location = addslashes($data[$j]->location);
			$job_type = addslashes($data[$j]->job_type);

			
			$job_description = addslashes($data[$j]->job_description);
			$skills = addslashes($data[$j]->skills);
			$benefits = addslashes($data[$j]->benefits);

			if($entry_id==''){
				$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as entry_id from b2c_career"));
				$max_entry_id = $sq_max['entry_id'] + 1;
				$sq_setting = mysqlQuery("INSERT INTO `b2c_career`(`entry_id`, `position`, `location`, `job_type`,`job_description`,`skills`,`benefits`) VALUES ('$max_entry_id','$position','$location','$job_type','$job_description','$skills','$benefits')");
				if($sq_setting){
					echo "success--B2C CMS settings saved.";
					exit;
				}
				...
				$active_flag = ($data[$j]->active_flag == 'Inactive') ? '1' : '0';
				$sq_setting = mysqlQuery("update `b2c_career` set position='$position',location='$location' ,job_type='$job_type', job_description='$job_description',skills='$skills', benefits='$benefits', active_flag='$active_flag' where entry_id='$entry_id'");
				if($sq_setting){
					echo "success--B2C CMS settings updated.";
					exit;
				}
		...
		if($section == '27'){
			...
			$entry_id = $data[$j]->entry_id;
			$tname = addslashes($data[$j]->tname);
			$image = $data[$j]->image;
			$designation = addslashes($data[$j]->designation);

			if($entry_id==''){
				$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as entry_id from b2c_team_details"));
				$max_entry_id = $sq_max['entry_id'] + 1;
				$sq_setting = mysqlQuery("INSERT INTO `b2c_team_details`(`entry_id`, `tname`, `designation`, `image`) VALUES ('$max_entry_id','$tname','$designation','$image')");
				if($sq_setting){
					echo "success--B2C CMS settings saved.";
					exit;
				}
				...
				$active_flag = ($data[$j]->active_flag == 'Inactive') ? '1' : '0';
				$sq_setting = mysqlQuery("update `b2c_team_details` set tname='$tname',designation='$designation',image='$image', active_flag='$active_flag' where entry_id='$entry_id'");
				if($sq_setting){
					echo "success--B2C CMS settings updated.";
					exit;
				}
		...
		if($section == '28'){
			...
			if($entry_id==''){
				$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as entry_id from b2c_awards"));
				$max_entry_id = $sq_max['entry_id'] + 1;
				$sq_setting = mysqlQuery("INSERT INTO `b2c_awards`(`entry_id`, `title`, `image`) VALUES ('$max_entry_id','$title','$image')");
				if($sq_setting){
					echo "success--B2C CMS settings saved.";
					exit;
				}
				...
				// $active_flag = ($data[$j]->active_flag == 'Inactive') ? '1' : '0';
				$sq_setting = mysqlQuery("update `b2c_awards` set title='$title',image='$image' where entry_id='$entry_id'");
				if($sq_setting){
					echo "success--B2C CMS settings updated.";
					exit;
				}
