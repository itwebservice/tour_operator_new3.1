  ...
  function enquiry_master_save(){
    ...
    $landline_no = isset($_POST["landline_no"]) ? $_POST["landline_no"] : '';
    $country_code = isset($_POST['country_code']) ? $_POST['country_code'] : '';
    $email_id = isset($_POST["email_id"]) ? $_POST["email_id"] : '';
    $location = isset($_POST["location"]) ? $_POST["location"] : '';
    $assigned_emp_id = isset($_POST['assigned_emp_id']) ? $_POST['assigned_emp_id'] : 1;
    $enquiry_specification = isset($_POST["enquiry_specification"]) ? $_POST["enquiry_specification"] : ''; 
    $enquiry_date = isset($_POST["enquiry_date"]) ? $_POST["enquiry_date"] : ''; 
    $followup_date = isset($_POST["followup_date"]) ? $_POST["followup_date"] : '';
    $reference_id = isset($_POST['reference_id']) ? $_POST['reference_id'] : '';
    $enquiry_content = isset($_POST['enquiry_content']) ? $_POST['enquiry_content'] : '';
    $branch_admin_id = isset($_POST['branch_admin_id']) ? $_POST['branch_admin_id'] : '';
    $financial_year_id = $_SESSION['financial_year_id'];
    $customer_name = isset($_POST['customer_name']) ? $_POST['customer_name'] : '';
    $user_id = isset($_POST['user_id']) ? $_POST['user_id'] : 0;
    $by = isset($_POST['by']) ? $_POST['by'] : ''; //enquiries saved by customer 
    ...
    if($by == 'cust'){
      ...
      $sq_enquiry = mysqlQuery("insert into enquiry_master (enquiry_id, login_id,branch_admin_id,financial_year_id, enquiry_type,enquiry, name, mobile_no, landline_no, country_code,email_id,location, assigned_emp_id, enquiry_specification, enquiry_date, followup_date, reference_id, enquiry_content,customer_name,user_id ,tour_name) values ('$enquiry_id', '$login_id', '$branch_admin_id','$financial_year_id', '$enquiry_type','$enquiry', '$name', '$mobile_no', '$landline_no', '$country_code','$email_id','$location', '$assigned_emp_id', '$enquiry_specification', '$enquiry_date', '$followup_date', '$reference_id', '$enquiry_content','$customer_name','$user_id', '$destination_name')");
      ...
      $sq_followup = mysqlQuery("insert into enquiry_master_entries(entry_id, enquiry_id, followup_reply,  followup_status,  followup_type, followup_date, followup_stage, created_at) values('$entry_id', '$enquiry_id', '', 'New','', '$followup_date','$enquiry', '$enquiry_date')");
  ...
  function send_enquiry_mail($enquiry_id, $email_id, $name, $tour_name1, $enquiry_specification,$assigned_emp_id,$mobile_no,$update_flag){
    ...
    $enq_content = isset($enqDetails['enquiry_content']) ? json_decode($enqDetails['enquiry_content']) : [];
    foreach($enq_content as $rows){
      if(isset($rows->name)){
        switch($rows->name){
          ...
          case 'total_single_person' : $tour_sp = $rows->value;break;
        }
      }
    }
    ...
    $ass_email_id = isset($sq_emp['email_id']) ? $sq_emp['email_id'] : '';
    $ass_emp = isset($sq_emp['first_name']) ? $sq_emp['first_name'].' '.$sq_emp['last_name'] : '';
    $ass_mobile = isset($sq_emp['mobile_no']) ? $sq_emp['mobile_no'] : '';
    }
    $content = '
      <tr>
          <table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
            ...
            if($enqDetails['enquiry_type'] == 'Package Booking' || $enqDetails['enquiry_type'] == 'Group Booking'){
              ...
              <tr><td style="text-align:left;border: 1px solid #888888;">Total Single Person</td>   <td style="text-align:left;border: 1px solid #888888;">'.$tour_sp.'</td></tr>';
            }
            $content  .= '</table>
              </tr>';
          ...
          if($enqDetails['enquiry_type'] == 'Package Booking'){
            ...
            $dest_id = isset($sq_dest['dest_id']) ? $sq_dest['dest_id'] : 0;
            $sq_package_count = mysqli_num_rows(mysqlQuery("select * from custom_package_master where dest_id = '$dest_id' and total_nights='$total_nights'"));
            if($sq_package_count > 0){

              $query_p = "select * from custom_package_master where dest_id = '$dest_id' and total_nights='$total_nights'";
            }else{
              
              $sq_package_count = mysqli_num_rows(mysqlQuery("select * from custom_package_master where dest_id = '$dest_id' limit 0,2"));
              if($sq_package_count > 0){
                $query_p = "select * from custom_package_master where dest_id = '$dest_id' limit 0,3";
              }
            }
            if($sq_package_count > 0){
  ...
  function send_emp_enquiry_mail($enquiry_id, $assigned_emp_id,$name)
  {
    ...
    $sq_reference = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `references_master` where reference_id = ".$sq_enquiry_details['reference_id']));
    $reference = isset($sq_reference['reference_name']) ? $sq_reference['reference_name'] : '';
    $Cust_name = $sq_enquiry_details['name'];
    $enquiry_type = isset($sq_enquiry_details['enquiry_type']) ? $sq_enquiry_details['enquiry_type'] : '';
    $assigned_emp_id = $sq_enquiry_details['assigned_emp_id'];
    $enq_content = isset($sq_enquiry_details['enquiry_content']) ? json_decode($sq_enquiry_details['enquiry_content']) : [];
    $tour_name = '';
    $tour_adults = '';
    $travel_from_date = '';
    $travel_to_date = '';
    ...
    $content .= '
            ...
            <tr><td style="text-align:left;border: 1px solid #888888;">Reference </td>   <td style="text-align:left;border: 1px solid #888888;">'.$reference.'</td></tr>
...
Email ID : ".$ass_email_id
    ...
    $whatsapp_msg .= '%0aThank%20you,%0a'.$app_name;
  ...
  function enquiry_master_update()
  {
    ...
    $enquiry_date = isset($_POST['enquiry_date']) ? $_POST['enquiry_date'] : '';
    $followup_date = isset($_POST['followup_date']) ? $_POST['followup_date'] : '';
    $reference_id = isset($_POST['reference']) ? $_POST['reference'] : '';
    ...
    $enquiry_type = isset($_POST['enquiry_type']) ? $_POST['enquiry_type'] : '';
    $name = isset($_POST['name']) ? $_POST['name'] : '';
    $customer_dropdown = isset($_POST['customer_dropdown']) ? $_POST['customer_dropdown'] : '';
    $user_id = isset($_POST['user_id']) ? $_POST['user_id'] : 0;
    $enquiry_date = date('Y-m-d', strtotime($enquiry_date));
    $followup_date = date('Y-m-d H:i', strtotime($followup_date));
  ...
  public function followup_reply_save()
  {
    ...
    $q = "select * from enquiry_master_entries where enquiry_id='$enquiry_id'";
    $sq_entery_count = mysqli_fetch_assoc(mysqlQuery($q));
    $sq_followup_add = true;
    ...
    if($sq_entery_count['followup_reply']==""){
      $sq_followup = mysqlQuery("update enquiry_master_entries set followup_reply='$followup_reply',followup_date='$followup_date',followup_type='$followup_type',followup_status='$followup_status', created_at='$created_at', followup_stage='$followup_stage',quotation_id='$quotation_id' where enquiry_id='$enquiry_id'");
    }
    else{
      $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from enquiry_master_entries"));
      $entry_id1 = $sq_max['max'] + 1;
      $sq_followup_add = mysqlQuery("insert into enquiry_master_entries(entry_id, enquiry_id, followup_reply, followup_status, followup_type, followup_date, followup_stage, created_at,quotation_id) values('$entry_id1', '$enquiry_id', '$followup_reply', '$followup_status','$followup_type', '$followup_date', '$followup_stage', '$created_at','$quotation_id')");
