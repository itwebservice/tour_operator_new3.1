    ...
    <div class="col-md-12 passengerPanel endPagecenter mg_bt_10">
      ...
      if ($sq_quotation['costing_type'] == 1) { ?>
        ...
        while ($sq_costing = mysqli_fetch_assoc($sq_costing1)) {
          ...
          $quotation_cost = (float)($quotation_cost) + (float)($travel_cost) + (float)($other_cost);
          $quotation_cost = ceil($quotation_cost);
          $currency_amount1 = currency_conversion($currency, $sq_quotation['currency_code'], $quotation_cost);
                  ...
                  <table class="table table-bordered no-marg tableTrnasp" id="tbl_emp_list">
                    ...
                    <tbody>
                      ...
                      if ($sq_plane_count > 0) { ?>
                        <tr>
                          <td><?= 'Flight' ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['flight_acost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['flight_ccost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['flight_icost'])) ?></td>
                        </tr>
                      <?php }
                      if ($sq_train_count > 0) { ?>
                        <tr>
                          <td><?= 'Train' ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['train_acost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['train_ccost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['train_icost'])) ?></td>
                        </tr>
                      <?php }
                      if ($sq_cruise_count > 0) { ?>
                        <tr>
                          <td><?= 'Cruise' ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['cruise_acost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['cruise_ccost'])) ?></td>
                          <td><?= currency_conversion($currency, $sq_quotation['currency_code'], (float)($sq_quotation['cruise_icost'])) ?></td>
                        </tr>
                      <?php } ?>
                    </tbody>
                  </table>
