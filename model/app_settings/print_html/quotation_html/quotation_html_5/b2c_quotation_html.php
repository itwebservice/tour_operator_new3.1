...
if ($type == '1') {
  ...
  while ($row_tariff = mysqli_fetch_assoc($sq_tariff)) {
    ...
    $total_cost1 = ($sq_quotation['adults'] * (float)($row_tariff['cadult'])) + ($sq_quotation['chwob'] * (float)($row_tariff['ccwob'])) + ($sq_quotation['chwb'] * (float)($row_tariff['ccwb'])) + ($sq_quotation['infant'] * (float)($row_tariff['cinfant'])) + ($sq_quotation['extra_bed'] * (float)($row_tariff['cextra']));
  }
  if ($total_cost1 == '0') {
...
// PDF for Group Tour
  ...
  if ($total_pax > 1) {
    $adult_cost_total = intval($sq_quotation['adults']) * (float)($sq_pckg['adult_cost']);
    $child_without_cost_total = intval($sq_quotation['chwob']) * (float)($sq_pckg['child_without_cost']);
    $child_with_cost_total = intval($sq_quotation['chwb']) * (float)($sq_pckg['child_with_cost']);
    $with_bed_cost_total = intval($sq_quotation['extra_bed']) * (float)($sq_pckg['with_bed_cost']);
    $infant_cost_total = intval($sq_quotation['infant']) * (float)($sq_pckg['infant_cost']);
  } else {
    $adult_cost_total = intval($sq_quotation['adults']) * (float)($sq_pckg['single_person_cost']);
  }
      ...
      <div class="transportDetails_costing">
        <div class="table-responsive">
          <table class="table table-bordered tableTrnasp no-marg" style="width:100% !important;" id="tbl_emp_list">
            <tr>
              <td style="text-align:left;border: 1px solid #888888;font"><b>Package Type</b></td>
              <td style="text-align:left;border: 1px solid #888888;"><?= $sq_quotation['package_type'] ?></td>
            </tr>
            <tr>
              <td style="text-align:left;border: 1px solid #888888;"><b>Travel Date</b></td>
              <td style="text-align:left;border: 1px solid #888888;"><?= get_date_user($sq_quotation['travel_from_date']) . ' To ' . get_date_user($sq_quotation['travel_to_date']) ?></td>
            </tr>
            <tr>
              <td style="text-align:left;border: 1px solid #888888;"><b>Other Specification</b></td>
              <td style="text-align:left;border: 1px solid #888888;"><?= ($sq_quotation['specification'] != '') ? $sq_quotation['specification'] : 'NA' ?></td>
            </tr>
          </table>
        </div>
      </div>
