...
class b2b_sale{
    ...
    function success_save(){
        ...
        $sq_save = mysqlQuery("INSERT INTO `b2b_booking_master`(`booking_id`, `customer_id`,`financial_year_id`, `fname`, `lname`, `email_id`, `country_id`, `contact_no`, `sp_request`, `cart_checkout_data`, `coupon_code`, `traveller_details`,`timing_slots`, `created_at`,`user_id`,`agent_flag`) VALUES ('$booking_id','$customer_id','$financial_year_id','$fname','$lname','$email_id','$country_id','$phone_contact','$sp_request','$cart_checkout_data','$coupon_code','$traveller_details','$timing_slots','$used_at','$user_id','$agent_flag')");

        if($sq_save){
            mysqlQuery("UPDATE `b2b_login` SET `cart_data`='',`request_id`='0' WHERE user_id='$user_id' and agent_flag='$agent_flag'");
            ...
            $total_hotel_cost = 0;$total_transfer_cost = 0;$total_activity_cost = 0;$total_tour_cost = 0;$total_ferry_cost = 0;$total_group_cost = 0;
            $total_hotel_tax = 0; $total_transfer_tax = 0; $total_activity_tax = 0; $total_tour_tax = 0;$total_ferry_tax = 0;$total_group_tax = 0; 
            $total_cost = 0;
            ...
            for($i=0;$i<sizeof($group_list_arr);$i++){
                //Applied Tax
                $services = ($group_list_arr[$i]->service!='') ? $group_list_arr[$i]->service : [];
                for($j=0;$j<count(array($services));$j++){
                    $tax_arr = explode(',',$services->service_arr[$j]->taxation);
                    ...
                    $total_group_cost += $room_cost1;
                    $total_group_tax += $tax_amount1;
                    $total_cost += $total_amount1;
                }
            }
            if($coupon_code != ''){
    ...
    public function finance_save($booking_id,$total_cost,$used_at,$payment_mode,$payment_amount1,$customer_id,$bank_id1,$total_hotel_cost,$total_transfer_cost,$total_activity_cost,$total_tour_cost,$hotel_tax_ledger,$transfer_tax_ledger,$activity_tax_ledger,$tour_tax_ledger,$total_hotel_tax,$total_transfer_tax,$total_activity_tax,$total_tour_tax,$discount,$total_ferry_cost,$total_ferry_tax,$ferry_tax_ledger,$total_group_cost,$total_group_tax,$group_tax_ledger)
    {
        ...
        if($total_group_cost != 0){
            ...
            $module_entry_id = $booking_id;
            $transaction_id = "";
            $payment_amount = $total_group_cost;
            $payment_date = $booking_date;
            $payment_particular = get_sales_particular(get_b2b_booking_id($booking_id,$yr1), $booking_date, $total_group_cost, $customer_id);
            $ledger_particular = get_ledger_particular('To','B2B Sales');
            $gl_id = 59;
            $payment_side = "Credit";
            $clearance_status = "";
        ...
        //Group Tour Tax Amount
        if($total_group_tax != 0){
            $group_tax_ledgers = explode('+',$group_tax_ledger);
            $total_group_tax1 = $total_group_tax / 2;
            if(sizeof($group_tax_ledgers) == 1){
                // Credit
                $module_name = "B2B Booking";
                $module_entry_id = $booking_id;
                $transaction_id = "";
                $payment_amount = $total_group_tax;
                $payment_date = $booking_date;
                $payment_particular = get_sales_particular(get_b2b_booking_id($booking_id,$yr1), $booking_date, $total_group_tax, $customer_id);    
                $ledger_particular = get_ledger_particular('By','Cash/Bank');
                $gl_id = $group_tax_ledgers[0];
                $payment_side = "Credit";
                $clearance_status = "";
                $transaction_master->transaction_save($module_name, $module_entry_id, $transaction_id, $payment_amount, $payment_date, $payment_particular, $gl_id,'', $payment_side, $clearance_status, $row_spec,$branch_admin_id,$ledger_particular,'INVOICE');
            }
            else{
                for($i=0;$i<sizeof($group_tax_ledgers);$i++){
                    // Credit
                    $module_name = "B2B Booking";
                    $module_entry_id = $booking_id;
                    $transaction_id = "";
                    $payment_amount = $total_group_tax1;
                    $payment_date = $booking_date;
                    $payment_particular = get_sales_particular(get_b2b_booking_id($booking_id,$yr1), $booking_date, $total_group_tax1, $customer_id);    
                    $ledger_particular = get_ledger_particular('By','Cash/Bank');
                    $gl_id = $group_tax_ledgers[$i];
                    $payment_side = "Credit";
                    $clearance_status = "";
                    $transaction_master->transaction_save($module_name, $module_entry_id, $transaction_id, $payment_amount, $payment_date, $payment_particular, $gl_id,'', $payment_side, $clearance_status, $row_spec,$branch_admin_id,$ledger_particular,'INVOICE');
                }
    ...
    {    
        ...
        if(sizeof($group_list_arr)>0){
            $group_content .= '<tr>
            <table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
            <tr><td colspan="2">Group Tour Information</td></tr>
            </table>
            </tr>';
            for($i=0;$i<sizeof($group_list_arr);$i++){
                ...
                for($j=0;$j<count(array($services));$j++){
                    ...
                    $group_content .= '
                        <tr>
                            <table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
                            <tr><td style="text-align:left;border: 1px solid #888888;"> Tour Name</td>   <td style="text-align:left;border: 1px solid #888888;">'.$group_list_arr[$i]->service->service_arr[$j]->tour_name.'</td></tr>
                            <tr><td style="text-align:left;border: 1px solid #888888;"> Travel-Date</td>   <td style="text-align:left;border: 1px solid #888888;" >'.$tour_group[1].'</td></tr>
                            </table>
                        </tr>';
                }
            }
        }
        $content .= $hotel_content.$transfer_content.$activity_content.$tours_content.$ferry_content.$group_content;
