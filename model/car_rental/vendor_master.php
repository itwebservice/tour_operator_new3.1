...
public function vendor_save()
{
	...
	$sq_vendor_count = mysqli_num_rows(mysqlQuery("select * from car_rental_vendor where vendor_name='$vendor_name'"));
	...
	if($sq_vendor)
	{
		...
		for($i=0; $i<sizeof($vehicle_no_arr); $i++){
			...
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(vehicle_id) as max from car_rental_vendor_vehicle_entries"));
...
public function vendor_update()
{
	...
	$sq_vendor_count = mysqli_num_rows(mysqlQuery("select * from car_rental_vendor where vendor_name='$vendor_name' and vendor_id!='$vendor_id'"));
	...
	if($sq_vendor)
	{
	    ...
	    if($GLOBALS['flag']){
	      ...
	      echo "Vehicle Supplier has been successfully updated.";
