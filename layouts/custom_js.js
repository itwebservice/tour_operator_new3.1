$(document).ready(function() {
        ...
        function tours_page_currencies(current_page_url) {
            ...
            currency_icon_lisr.forEach(function(item) {
                // Skip currency icons for percentage offers
                var discountText = item.closest('.discount-text');
                if (discountText) {
                    var offerFlagEl = discountText.querySelector('.offer-currency-flag');
                    if (offerFlagEl && offerFlagEl.innerHTML.trim() === 'percentage') {
                        // This is a percentage offer, skip setting the currency icon and ensure it's hidden
            ...
            if (current_page_url != base_url + 'view/tours/tours-listing.php') {
                ...
                if (offer_price_list !== null && offeramt_Classlist[0] !== undefined) {
                    offer_price_list.map((tour) => {
                        if (tour.flag == 'no' && tour.id != 'PERCENT' && tour.package_id) {
                            // Find matching DOM element by package_id (element ID format: offer-currency-price{package_id})
                            var offerEl = document.getElementById('offer-currency-price' + tour.package_id);
                            if (offerEl && offerEl.style.display != 'none' && offerEl.style.visibility != 'hidden') {
                                // Double-check it's not a percentage offer by checking the flag element
                                var discountText = offerEl.closest('.discount-text');
                                if (discountText) {
                                    var offerFlagEl = discountText.querySelector('.offer-currency-flag');
                                    if (offerFlagEl && offerFlagEl.innerHTML.trim() === 'percentage') {
                                        // This is a percentage offer, ensure it stays hidden
                                        offerEl.style.display = 'none';
                                        offerEl.style.visibility = 'hidden';
                                        offerEl.innerHTML = '';
                                        return;
                                    }
                                }
                                // Use same formula as hotel offers
                                var currency_rates = get_currency_rates(tour.id, currency_id).split('-');
                                var to_currency_rate = parseFloat(currency_rates[0]);
                                var from_currency_rate = parseFloat(currency_rates[1]);
                                var offer_amount = parseFloat(tour.amount) || 0;
                                if (from_currency_rate > 0 && offer_amount > 0) {
                                    offerEl.innerHTML = parseFloat(from_currency_rate * offer_amount).toFixed(2);
                                }
                            }
                        }
                    });
                ...
                // Ensure all percentage offers stay hidden
                var allOfferPriceEls = document.querySelectorAll(".offer-currency-price");
                allOfferPriceEls.forEach(function(offerEl) {
                    var discountText = offerEl.closest('.discount-text');
                    if (discountText) {
                        var offerFlagEl = discountText.querySelector('.offer-currency-flag');
                        if (offerFlagEl && offerFlagEl.innerHTML.trim() === 'percentage') {
                ...
                //Tour Original Price (Old Price)
                var original_price_list = document.querySelectorAll(".original-currency-price");
                var original_price_id = document.querySelectorAll(".original-currency-id");

                if (original_price_list.length > 0 && original_price_id.length > 0) {
                    for (var i = 0; i < original_price_list.length; i++) {
                        if (original_price_list[i].innerHTML.trim() != '') {
                            var orig_id = original_price_id[i].innerHTML;
                            var orig_amount = parseFloat(original_price_list[i].innerHTML);
                            var currency_rates = get_currency_rates(orig_id, currency_id).split('-');
                            var to_currency_rate = currency_rates[0];
                            var from_currency_rate = currency_rates[1];
                            original_price_list[i].innerHTML = parseFloat(from_currency_rate * orig_amount).toFixed(2);
                        }
                    }
                }
            }
        ...
        function hotel_page_currencies() {
            ...
            if (offer_price_list !== null && offeramt_Classlist[0] !== undefined) {
                offer_price_list.map((tour, i) => {
                    var iconEl = offeramt_Classlist[i].previousElementSibling;
                    if (tour.flag === 'percentage') {
                        offeramt_Classlist[i].innerHTML = tour.amount;
                        if (iconEl && iconEl.classList.contains('currency-icon')) {
                            iconEl.classList.add('c-hide');
                        }
                    } else {
                        var currency_rates = get_currency_rates(tour.id, currency_id).split('-');
                        var to_currency_rate = currency_rates[0];
                        ...
                        if (iconEl && iconEl.classList.contains('currency-icon')) {
                            iconEl.classList.remove('c-hide');
                        }
                    }
                });
            }
        ...
        const metaData = window.metaTagsData || [];
        const getPageMeta = (pageName) => {
            return metaData.find(item => item.page === pageName);
        };
        const metas = getPageMeta(window.currentPageType || '');
        console.log(metas);
        if (metas) {
