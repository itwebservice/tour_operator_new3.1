    ...
    <div class="row">
        ...
        <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
            <select name="visa_id_filter" id="visa_id_filter" style="width:100%" title="Booking ID">
				...
				$query .= " and financial_year_id = '" . $_SESSION['financial_year_id'] . "' order by misc_id desc";
				...
				?>
                <option value="<?= $row_visa['misc_id'] ?>">
                    <?= get_misc_booking_id($row_visa['misc_id'], $year) . ' : ' . $sq_customer['first_name'] . ' ' . $sq_customer['last_name'] ?>
                </option>
                <?php
				}
				?>
...
function business_rule_load() {
    get_auto_values('balance_date', 'visa_issue_amount', 'payment_mode', 'service_charge', 'markup', 'save', 'true',
        'service_charge');
}
...
function calculate_total_amount(offset = '') {
    ...
    visa_issue_amount = ($('#basic_show' + offset).html() == '&nbsp;') ? visa_issue_amount : parseFloat($(
        '#basic_show' + offset).text().split(' : ')[1]);
    ...
    var total_amount = parseFloat(visa_issue_amount) + parseFloat(service_charge) + parseFloat(service_tax_amount) +
        parseFloat(markup) + parseFloat(markupservice_tax_amount);


    total_amount = total_amount.toFixed(2);
    // var roundoff = Math.round(total_amount) - total_amount;
    $('#roundoff' + offset).val('0.00');

    $('#visa_total_cost' + offset).val(parseFloat(total_amount) );
    // + parseFloat(roundoff));
...
function excel_report() {
    ...
    window.location = 'home/excel_report.php?customer_id=' + customer_id + '&misc_id=' + misc_id + '&from_date=' +
...
function whatsapp_send(emp_id, customer_id, booking_date, base_url, contact_no, email_id) {
    $.post(base_url + 'controller/miscellaneous/whatsapp_send.php', {
        emp_id: emp_id,
...
function delete_entry(misc_id) {
    $('#vi_confirm_box').vi_confirm_box({
        callback: function(data1) {
            if (data1 == "yes") {
                var branch_status = $('#branch_status').val();
                var base_url = $('#base_url').val();
                $.post(base_url + 'controller/miscellaneous/miscellaneous_master_delete.php', {
                    booking_id: misc_id
                }, function(data) {
