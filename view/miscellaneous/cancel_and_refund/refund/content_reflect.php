...
$sq_refund_amount1 = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from miscellaneous_refund_master where misc_id='$misc_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from miscellaneous_refund_master where misc_id='$misc_id'"));
$sq_pend_pay = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from miscellaneous_refund_master where misc_id='$misc_id' and clearance_status='Pending'"));
$sq_canl_pay = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from miscellaneous_refund_master where misc_id='$misc_id' and clearance_status='Cancelled'"));
...
$remaining = $refund_amount - $sq_refund_amount1['sum'];
                ...
                <div class="stat_content main_block">
                    ...
                    <span class="main_block content_span" data-original-title="" title="">
                        <span class="stat_content-tilte pull-left" data-original-title="" title="">Pending Refund
                            Amount</span>
                        <span class="stat_content-amount pull-right" data-original-title=""
                            title=""><?php echo number_format($remaining_show, 2); ?></span>
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="panel panel-default panel-body">
                <div class="row  text-center mg_tp_10">
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        ...
                        <input type="text" id="refund_amount" name="refund_amount" placeholder="*Refund Amount"
                            title="Refund Amount"
                    ...
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        <input type="text" id="refund_date" name="refund_date" placeholder="*Refund Date"
                            title="Refund Date" value="<?= date('d-m-Y') ?>">
                    </div>
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                ...
                <div class="row mg_bt_10">
                    <div class="col-xs-12">
                        <select name="entry_id" id="entry_id" multiple>
                            <?php
                            $sq_visa_entries = mysqlQuery("select * from miscellaneous_master_entries where misc_id='$misc_id' and status='Cancel'");
        ...
        <div class="table-responsive">
            ...
            <table class="table table-hover table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
                ...
                <tbody>
                    ...
                    $count = 0;
                    $bg;
                    $sq_visa_refund = mysqlQuery($query);
                    while ($row_visa_refund = mysqli_fetch_assoc($sq_visa_refund)) {

                        $count++;

                        $sq_visa_info = mysqli_fetch_assoc(mysqlQuery("select * from miscellaneous_master where misc_id='$row_visa_refund[misc_id]' and delete_status='0'"));
                        $date = $sq_visa_info['created_at'];
                        $yr = explode("-", $date);
                        $year = $yr[0];

                        $traveler_name = "";
                        $sq_refund_entries = mysqlQuery("select * from miscellaneous_refund_entries where refund_id='$row_visa_refund[refund_id]'");
                        ...
                        if ($row_visa_refund['clearance_status'] == 'Pending') {
                            $bg = "warning";
                        } else if ($row_visa_refund['clearance_status'] == 'Cancelled') {
                            $bg = "danger";
                        } else if ($row_visa_refund['clearance_status'] == 'Cleared') {
                            $bg = "success";
                        } else {
                            $bg = "";
                        }
                ...
                <tfoot>
                    ...
                    <tr class="active">
                        <th colspan="2" class="info">Refund : <?= number_format((float)($sq_pay['sum']), 2) ?></th>
                        <th colspan="2" class="warning">Pending : <?= number_format((float)($sq_pend_pay['sum']), 2) ?></th>
                        <th colspan="2" class="danger">Cancelled : <?= number_format((float)($sq_canl_pay['sum']), 2) ?></th>
                        <th colspan="2" class="success">Total_Refund :
                            <?= number_format(((float)($sq_pay['sum']) - (float)($sq_pend_pay['sum']) - (float)($sq_canl_pay['sum'])), 2); ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
                    ...
                    callback: function(data1) {
                        if (data1 == "yes") {
                            ...
                            $.ajax({
                                type: 'post',
                                url: base_url +
                                    'controller/miscellaneous/refund/refund_save.php',
                                data: {
                                    misc_id: misc_id,
