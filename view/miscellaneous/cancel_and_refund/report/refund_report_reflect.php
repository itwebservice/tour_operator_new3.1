...
<table class="table table-hover" id="tbl_refund" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_refund = mysqli_fetch_assoc($sq_refund)){
			...
			$sq_entry_year = mysqli_fetch_assoc(mysqlQuery("select * from miscellaneous_master where misc_id='$row_refund[misc_id]' and delete_status='0'"));
			...
			<tr class="<?= $bg;?>">
				...
				<td><?= $traveler_name ?></td>
				<td><?= get_misc_booking_refund_id($row_refund['refund_id'],$year1); ?></td>
	...
	<tfoot>
		<tr class="active">
			...
			<th colspan="3" class="text-right success">Total_Refund : <?= ($sq_paid_amount=='')?number_format(0,2):number_format($sq_paid_amount,2); ?></th>
		</tr>
	</tfoot>
</table>
