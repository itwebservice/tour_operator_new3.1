...
$query = mysqli_fetch_assoc(mysqlQuery("SELECT banner_images FROM `b2c_settings` where setting_id='1'"));
$images = ($query['banner_images'] != '' && $query['banner_images'] != 'null') ? json_decode($query['banner_images']) : [];
?>
<form id="section_banner">
    ...
    <div class="row mg_bt_20">
        <div class="col-md-4">
            <select class="form-control" style="width:100%" name="banner_count" id="banner_count"
                onchange="banner_images_reflect(this.id);" title="Select No of Images" data-toggle="tooltip">
    ...
    <div class="row mg_bt_20">
        ...
        <div class="col-md-3">
            <div class="gallary-single-image mg_bt_20" style="height:100px;max-height: 100px;overflow:hidden;">
                ...
                <img src="<?php echo $download_url; ?>" id="<?php echo $images[$i]->banner_count; ?>" width="100%"
                    height="100%">
                <span class="img-check-btn"><button type="button" class="btn btn-danger btn-sm"
                        onclick="delete_image('<?php echo $images[$i]->banner_count; ?>')" title="Delete Image"
                        data-toggle="tooltip"><i class="fa fa-times" aria-hidden="true"></i></button></span>
            </div>
        </div>
        <?php } ?>
    </div>
...
function banner_images_reflect(banner_count) {
    ...
    if (banner_count != '') {
        ...
        if (parseInt(total_upload_count) <= 5) {
            $.post('banners/get_banner_images_section.php', {
                banner_uploaded_count: banner_uploaded_count,
