...
{
    $qry = "SELECT * FROM airline_master INNER JOIN ticket_trip_entries on airline_master.airline_id = ticket_trip_entries.airline_id inner join ticket_master on ticket_trip_entries.ticket_id = ticket_master.ticket_id INNER JOIN customer_master on ticket_master.customer_id = customer_master.customer_id where customer_master.customer_id='$customer'";
    $res = mysqlQuery($qry);
    $html = '';
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_ticket_booking_id($db['ticket_id'], $year) . '</td>
            <td scope="col">Airline</td>
            <td scope="col">' .dateModify( $db['created_at']) . '</td>
            <td scope="col">' . $db['ticket_total_cost'] . '</td>
            <td scope="col">' . $db['ticket_total_cost'] - (int) $db['adult_fair'] -(int) $db['children_fair'] . '</td> </tr>';
    }
    echo  $html;
}
function car_rental($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_car_rental_booking_id($db['booking_id'], $year) . '</td>
...
function visa($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_visa_booking_id($db['visa_id'],$year) . '</td>
...
function bus($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_bus_booking_id($db['booking_id'],$year) . '</td>
            <td scope="col">Bus</td>
            <td scope="col">' . dateModify($db['created_at']) . '</td>
            <td scope="col">' . $db['total_cost'] . '</td>
            <td scope="col">' . $db['total_cost'] - $db['net_total'] . '</td> </tr>';
...
function excursion($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_exc_booking_id($db['exc_id'],$year) . '</td>
...
function miscellaneous($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_misc_booking_id($db['misc_id'],$year) . '</td>
...
function hotel($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_hotel_booking_id($db['booking_id'],$year) . '</td>
...
function train($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_train_ticket_booking_id($db['train_ticket_id'],$year) . '</td>
...
function tourwise($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_group_booking_id($db['id'],$year) . '</td>
            <td scope="col">Tourwise</td>
            <td scope="col">' .dateModify( $db['form_date']) . '</td>
            <td scope="col">' . $db['total_cost'] . '</td>
            <td scope="col">' . $db['total_cost'] - $db['net_total'] . '</td> </tr>';
    }
    echo  $html;
...
function package($id)
{
    ...
    while ($db = mysqli_fetch_array($res)) {
        ...
        $html .= '<tr><td>' . get_package_booking_id($db['booking_id'],$year) . '</td>
            <td scope="col">Package</td>
            <td scope="col">' . dateModify($db['booking_date']) . '</td>
            <td scope="col">' . $db['total_cost'] . '</td>
            <td scope="col">' . $db['total_cost'] - $db['net_total'] . '</td> </tr>';
    }
    echo  $html;
