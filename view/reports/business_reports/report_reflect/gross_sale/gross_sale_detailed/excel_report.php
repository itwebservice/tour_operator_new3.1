...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Gross sale Detailed');
...
$row_count = 11;
       ...
       $objPHPExcel->setActiveSheetIndex(0)
        ...
        while($row_package = mysqli_fetch_assoc($sq_package)){
            ...
            $sq_total_member = mysqli_num_rows(mysqlQuery("select booking_id from package_travelers_details where booking_id = '$row_package[booking_id]' AND status!='Cancel'"));
            ...
            $sq_incentive = mysqli_fetch_assoc(mysqlQuery("select * from booker_incentive_package_tour where booking_id='$row_package[booking_id]'"));
            $sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum from package_payment_master where booking_id='$row_package[booking_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
            ...
            $i=0;
            $p_due_date = '';
            $sq_purchase_count = mysqli_num_rows(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Package Tour' and estimate_type_id='$row_package[booking_id]' and vendor_estimate.delete_status='0'"));
            if($sq_purchase_count == 0){  $p_due_date = 'NA'; }
            $sq_purchase = mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Package Tour' and estimate_type_id='$row_package[booking_id]' and vendor_estimate.delete_status='0'");
            while($row_purchase = mysqli_fetch_assoc($sq_purchase)){
                $purchase_amt = $row_purchase['net_total'] - $row_purchase['refund_net_total'];
                $total_purchase = $total_purchase + $purchase_amt;
            }
            $sq_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Package Tour' and estimate_type_id='$row_package[booking_id]' and vendor_estimate.delete_status='0'"));     
