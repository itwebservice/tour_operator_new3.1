...
if($from_date!="" && $to_date!=""){
    $sq_high_count = mysqli_num_rows(mysqlQuery("select * from enquiry_master_entries where enquiry_id in(select enquiry_id from enquiry_master where enquiry='Strong' and status!='Disabled' and enquiry_date between '$from_date1' and '$to_date1' ) and followup_status='Dropped' and status!='False' "));
    ...
    $sq_drop_type = mysqli_fetch_assoc(mysqlQuery($query2));
    $enq_type = isset($sq_drop_type['enquiry_type']) ? $sq_drop_type['enquiry_type'] : 'NA';
    ...
    $sq_eq_emp = mysqli_fetch_assoc(mysqlQuery($query));
    $assigned_emp_id = isset($sq_eq_emp['assigned_emp_id']) ? $sq_eq_emp['assigned_emp_id'] : 0;

    $sq_emp2 = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$assigned_emp_id'"));
    $emp_name = ($assigned_emp_id != 0) ? $sq_emp2['first_name'].' '.$sq_emp2['last_name'] : 'NA';
    ...
    $sq_eq_emp1 = mysqli_fetch_assoc(mysqlQuery($query1));
    $sq_emp1 = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$assigned_emp_id'"));
    $emp_name = ($assigned_emp_id != 0) ? $sq_emp1['first_name'].' '.$sq_emp1['last_name'] : 'NA';
}
?>
<input type="hidden" id="branch_status1" value="<?= $branch_status ?>">
            ...
            <div class="widget_parent">
                <div class="stat_content main_block">
                    ...
                    <span class="main_block content_span" data-original-title="" title="">
                        ...
                        <span class="stat_content-amount pull-right" data-original-title="" title=""> <?= $enq_type ?></span>
                    </span> 
                </div>   
            </div>
        </div>
            ...
            <div class="widget_parent">
                <div class="stat_content main_block">
                    <span class="main_block content_span" data-original-title="" title="">
                        ...
                        <span class="stat_content-amount pull-right" data-original-title="" title=""><?= $emp_name ?></span>
                    </span>
                    <span class="main_block content_span" data-original-title="" title="">
...
function enquiry_list_reflect()
{
  ...
  $.get( "report_reflect/sales_projection/analysis/enquiry_list_reflect.php" , { emp_id : emp_id , enquiry_type : enquiry_type , from_date : from_date , to_date : to_date , enquiry_status : enquiry_status,branch_status:branch_status} , function ( data ) {
