    ...
    <div class="widget_parent-bg-img bg-img-red">
      ...
      for ($i = 0; $i < sizeof($group_list_arr); $i++) {
        ...
        for ($j = 0; $j < count(array($services)); $j++) {
          ...
          $group_total += $room_cost1;
          $grouptax_total += $tax_amount1;
        }
      }

      $servie_total = $servie_total + $hotel_total + $transfer_total + $activity_total + $tours_total + $ferry_total + $group_total;

      $main_total += $servie_total;
      $main_tax_total += $tax_total + $trtax_total + $acttax_total + $tourstax_total + $ferrytax_total + $grouptax_total;
      ...
      $content_arr = array(number_format($main_total, 2), number_format($main_tax_total, 2), number_format($offer_amount, 2), number_format((float)($final_total) - $offer_amount, 2), number_format($sq_payment_info['sum'], 2));
          ...
          ?>
              ...
              ?>
                <tr class="<?= $bg ?>">
                  <td><?= ++$count ?></td>
                  <td>Group Tour</td>
                  <td><?= $cart_checkout_data[$i]->service->service_arr[0]->tour_name ?></td>
                </tr>
          <?php
              }
            }
  ...
  <form id="frm_refund" class="mg_bt_150">
    ...
    <div class="row">
      <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
        <input type="number" name="cancel_amount" id="cancel_amount" class="text-right" placeholder="*Cancel amount(Tax Incl)" title="Cancel amount(Tax Incl)" onchange="validate_balance(this.id);calculate_total_refund()" value="<?= $sq_b2b_info['cancel_amount'] ?>">
      </div>
      <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
        <select title="Select Tax" id="tax_value" name="tax_value" class="form-control" onchange="calculate_total_refund();">
          <?php
          if ($sq_b2b_info['estimate_flag'] == 0) { ?>
            <option value="">*Select Tax</option>
            <?php get_tax_dropdown('Income') ?>
          <?php } else {
          ?>
            <option value="<?= $sq_b2b_info['tax_value'] ?>"><?= $sq_b2b_info['tax_value'] ?></option>
          <?php } ?>
        </select>
      </div>
      <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
        <input type="text" title="Tax Subtotal" id="tour_service_tax_subtotal" name="tour_service_tax_subtotal" value="<?= $sq_b2b_info['tax_amount'] ?>" readonly>
        <input type="hidden" id="ledger_posting" />
      </div>
      <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
        <input type="number" name="cancel_amount_exc" id="cancel_amount_exc" class="text-right" placeholder="*Cancellation Charges" title="Cancellation Charges" onchange="validate_balance(this.id);calculate_total_refund()" value="<?= $sq_b2b_info['cancel_amount_exc'] ?>">
      </div>
      <div class="col-md-3 col-sm-6 col-xs-12 mg_tp_10">
        <input type="number" name="total_refund_amount" id="total_refund_amount" class="amount_feild_highlight text-right" placeholder="Total Refund" title="Total Refund" readonly value="<?= $sq_b2b_info['total_refund_amount'] ?>">
      </div>
    </div>
    ...
    $('#vi_confirm_box').vi_confirm_box({
      ...
      callback: function(data1) {
        if (data1 == "yes") {
          ...
          $.post(base_url + 'controller/b2b_customer/cancel/cancel_booking.php', {
