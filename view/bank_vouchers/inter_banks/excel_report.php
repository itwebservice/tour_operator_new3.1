...
if($dbank_id!=""){
	$sq_cust = mysqli_fetch_assoc(mysqlQuery("select * from bank_master where bank_id='$dbank_id'"));
    $name_str1 = $sq_cust['bank_name'].' ('.$sq_cust['branch_name'].')';
}
else{
	$name_str1 = "";
}
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B3', 'Creditor Bank')
            ->setCellValue('C3', $name_str)
            ->setCellValue('B4', 'Debitor Bank')
            ->setCellValue('C4', $name_str1)
...
header('Content-Disposition: attachment;filename="Inter-Bank_Transfer('.date('d-m-Y H:i').').xls"');
