...
$sq = mysqli_fetch_assoc( mysqlQuery("select * from refund_traveler_cancelation where refund_id='$refund_id'") );
...
$sq_traveler = mysqli_fetch_assoc( mysqlQuery( "select m_honorific, first_name, last_name from travelers_details where traveler_id='$traveler_id'" ) );

$traveler_name = $sq_traveler['m_honorific'].' '.$sq_traveler['first_name'].' '.$sq_traveler_query['last_name'];
$first_time = true;

$sq = mysqlQuery("select traveler_id from refund_traveler_cancalation_entries where refund_id='$refund_id'");
while($row = mysqli_fetch_assoc($sq))
{
	$traveler_name_sq = mysqli_fetch_assoc(mysqlQuery("select m_honorific, first_name, last_name from travelers_details where traveler_id='$row[traveler_id]'"));
	if($first_time == true)
	{	
		$first_time = false;
