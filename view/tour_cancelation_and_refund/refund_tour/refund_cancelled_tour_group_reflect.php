...
$sq = mysqlQuery("select id from tourwise_traveler_details where tour_id = '$tour_id' and tour_group_id = '$tour_group_id' and traveler_group_id = '$traveler_group_id' ");
if($row = mysqli_fetch_assoc($sq))
{
    $tourwise_id = $row['id'];
}

$sql = mysqlQuery("SELECT SUM(amount) as total FROM payment_master where tourwise_traveler_id ='$tourwise_id' and payment_for = 'Travelling' ");
$row = mysqli_fetch_array($sql);
$traveling_amount_paid = $row['total'];

$sql = mysqlQuery("SELECT SUM(amount) as total FROM payment_master where tourwise_traveler_id ='$tourwise_id' and payment_for = 'Tour' ");
$row = mysqli_fetch_array($sql);
...
<table id="tbl_canceled_tour_details" class="table table-bordered table-hover"> 
        ...
        <tr class="text-center">
           <td colspan="2">
               <button id="btn_refund_tour_fee" class="btn btn-success"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Save</button>
           </td>
        </tr>
...
<div class="col-md-6 col-sm-6">
    <?php
    $count = mysqli_num_rows(mysqlQuery("select * from refund_tour_cancelation where tourwise_traveler_id='$tourwise_id' "));
    if($count>0)
    {
    ?>
       <div class="table-responsive">
...
$('#frm_refund').validate({
    ...
    submitHandler:function(form){
        ...
        $('#btn_refund_tour_fee').prop('disabled',true);
            ...
            callback: function(data1){
                    if(data1=="yes"){
                        ...
                        $.post( 
                            ...
                            function(data) {
                                msg_alert(data);  
                                $('#btn_refund_tour_fee').prop('disabled',false);
