...
$year_arr = array();
$sq = mysqlQuery("select id, traveler_group_id,customer_id,form_date from tourwise_traveler_details where tour_id = '$tour_id' and tour_group_id = '$tour_group_id' and delete_status='0' ");
while($row = mysqli_fetch_assoc($sq))
{
  $pass_count = mysqli_num_rows(mysqlQuery("select * from travelers_details where traveler_group_id='$row[traveler_group_id]'"));
  $cancelpass_count = mysqli_num_rows(mysqlQuery("select * from travelers_details where traveler_group_id='$row[traveler_group_id]' and status='Cancel'"));
