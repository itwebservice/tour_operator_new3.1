...
while ($row_payment = mysqli_fetch_assoc($sq_payment)) {
	...
	if ($row_payment['amount'] != 0) {
		...
		$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum ,sum(credit_charges) as sumc ,currency_code from package_payment_master where clearance_status!='Cancelled' and booking_id='$row_payment[booking_id]'"));

		$total_sale = $sq_booking['net_total'] + $sq_pay['sumc'];
		$total_pay_amt = $sq_pay['sum'] + $sq_pay['sumc'];;
		$pass_count = mysqli_num_rows(mysqlQuery("select * from package_travelers_details where booking_id='$sq_booking[booking_id]'"));
		$cancle_count = mysqli_num_rows(mysqlQuery("select * from package_travelers_details where booking_id='$sq_booking[booking_id]' and status='Cancel'"));
		$status = '';
		if ($pass_count == $cancle_count) {
			$sq_esti_c = mysqli_num_rows(mysqlQuery("select cancel_amount from package_refund_traveler_estimate where booking_id='$sq_booking[booking_id]'"));
			$sq_esti = mysqli_fetch_assoc(mysqlQuery("select cancel_amount from package_refund_traveler_estimate where booking_id='$sq_booking[booking_id]'"));
			$canc_amount = ($sq_esti_c > 0) ? $sq_esti['cancel_amount'] : 0;
		...
		$receipt_type = ($row_payment['payment_for'] == 'Travelling') ? "Travel Receipt" : "Package Tour Receipt";

		$pay_id= $row_payment['payment_id'];

		// $url1 = BASE_URL . "model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=$branch_status&outstanding=$outstanding&tour=$tour&table_name=package_payment_master&customer_field=booking_id&in_customer_id=$row_payment[booking_id]&currency_code=$sq_booking[currency_code]&status=$row_payment[status]";
		...
		$url1 = BASE_URL."model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=$branch_status&outstanding=$outstanding&tour=$tour&table_name=package_payment_master&customer_field=booking_id&in_customer_id=$row_payment[booking_id]&currency_code=$currency_code_2&status=$row_payment[status]&pay_id=$pay_id&net_amt=$net_amt";
		...
		if ($row_payment['payment_mode'] == 'Credit Note' || ($row_payment['payment_mode'] == 'Credit Card' && $row_payment['clearance_status'] == "Cleared")) {
			...
			$edit_btn = '<button id="update_button-' . $row_payment['payment_id'] . '" class="btn btn-info btn-sm" data-toggle="tooltip" onclick="update_modal(' . $row_payment['payment_id'] . ')" title="Update Details"><i class="fa fa-pencil-square-o"></i></button>';
		...
		$currency_amount1 = currency_conversion($currency,$currency_code_1,$row_payment['amount']+$row_payment['credit_charges']);
