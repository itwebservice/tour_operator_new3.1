...
$sq_tours = mysqlQuery("select * from custom_package_program where package_id = '$package_id'");
?>
      ...
      <div class="row">
          ...
          <table style="width:100%" id="package_program_list" name="package_program_list" class="table mg_bt_0 table-bordered">
              ...
              <?php while($row_tours = mysqli_fetch_assoc($sq_tours)){?>
                <tr>
                <td width="27px;" style="padding-right: 10px !important;"><input class="css-checkbox mg_bt_10 labelauty" id="chk_program<?= $count?>" type="checkbox" checked style="display: none;"><label for="chk_program1" style="margin-top: 55px;"><span class="labelauty-unchecked-image"></span><span class="labelauty-checked-image"></span></label></td>
                <td width="50px;"><input maxlength="15" value="<?= $count?>" type="text" name="username" placeholder="Sr. No." class="form-control mg_bt_10" disabled="" style="margin-top: 35px;"></td>
                <td class="col-md-2 no-pad" style="padding-left: 5px !important;"><input type="text" id="special_attaraction<?= $count?>-u" onchange="validate_spaces(this.id);validate_spattration(this.id);" name="special_attaraction" class="form-control mg_bt_10" placeholder="Special Attraction" style="margin-top: 35px;" title="Special Attraction" value="<?= $row_tours['attraction']?>"></td>
                ...
                <td class="col-md-5 no-pad" style="max-width: 594px;overflow: hidden;position: relative;"><textarea id="day_program<?= $count?>-u" name="day_program" class="form-control mg_bt_10 day_program" placeholder="*Day<?= $count?>-u Program" title="Day-wise Program" onchange="validate_spaces(this.id); validate_dayprogram(this.id);" style=" height:900px;" rows="3" ><?= $row_tours['day_wise_program']?></textarea><span class="style_text"><span class="style_text_b" data-wrapper="**" style="font-weight: bold; cursor: pointer;" title="Bold text">B</span><span class="style_text_u" data-wrapper="__" style="cursor: pointer;" title="Underline text"><u>U</u></span></span>
                </td>
                <td class="col-md-1/2 no-pad" style="padding-left: 5px !important;"><input type="text" id="overnight_stay<?= $count?>-u" name="overnight_stay" onchange="validate_spaces(this.id);validate_onstay(this.id);" class="form-control mg_bt_10" placeholder="Overnight Stay" style="margin-top: 35px;" title="Overnight Stay" value="<?= $row_tours['stay']?>"></td>
                <td class="col-md-1/2 no-pad" style="padding-left: 5px !important;"><select id="meal_plan<?= $count?>" title="" style="margin-top: 35px;" name="meal_plan" class="form-control mg_bt_10" data-original-title="Meal Plan">
                        |<?php if($row_tours['meal_plan'] !=''){ ?>
                          <option value="<?= $row_tours['meal_plan']?>"><?= $row_tours['meal_plan']?></option>
                        <?php } ?>
                        <?php get_mealplan_dropdown(); ?>
                        </select></td>
                <td class='col-md-1 pad_8'><button type="button" style="margin-top: 35px;" class="btn btn-info btn-iti btn-sm" id="itinerary<?php echo $count; ?>" title="Add Itinerary" onClick="add_itinerary('dest_name2','special_attaraction<?= $count?>-u','day_program<?= $count?>-u','overnight_stay<?= $count?>-u','Day-<?=$count?>')"><i class="fa fa-plus"></i></button>
                </td>
                </tr>
              <?php $count++; } ?>
              </tbody>
