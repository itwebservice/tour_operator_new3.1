        ...
        <div class="table-responsive">
            <table id="tbl_package_hotel_infomration" class="table table-bordered table-hover table-striped" style="width: 1475px;">
                <tr>
                    ...
                    <td><select id="city_name1" name="city_name1" style="width:150px" title="Select City Name" class="form-control city_name" onchange="load_hotel_list(this.id); " data-add-new-option="true">
                        </select></td>
                    <?php if($room_category_switch == 'No' ){?>
                    <td><select id="hotel_name1" name="hotel_name1" class="form-control app_select2" style="width:170px" title="Select Hotel Name" onchange="get_auto_values('txt_booking_date','total_basic_amt','payment_mode','service_charge','markup','save','true','service_charge','discount');" data-add-new-option="true">
                            <option value="">*Hotel Name</option>
                        </select></td>
                    <?php }else {?>
                    <td><select id="hotel_name1" name="hotel_name1" class="form-control app_select2" style="width:170px" title="Select Hotel Name" onchange="hotel_type_load_cate(this.id);get_auto_values('txt_booking_date','total_basic_amt','payment_mode','service_charge','markup','save','true','service_charge','discount');" data-add-new-option="true">
                            <option value="">*Hotel Name</option>
                        </select></td>
                    <?php }?>
...
<script>
    ...
    $(function () {
        $('#hotel_name1').select2({ width: '170px', minimumResultsForSearch: 0 });
        $('#txt_catagory1').select2({ width: '140px', minimumResultsForSearch: 0 });
        if (typeof initHotelSelectAddNew === 'function') {
            initHotelSelectAddNew('#hotel_name1');
        } else {
            setTimeout(function () {
                if (typeof initHotelSelectAddNew === 'function') {
                    initHotelSelectAddNew('#hotel_name1');
