        ...
        <table id="tbl_flight_quotation_dynamic_plane_update" name="tbl_flight_quotation_dynamic_plane_update" class="table table-bordered pd_bt_51 no-marg">
			...
			if($sq_plane_count==0){
				...
				<tr>
					...
					<td><select id="airline_name-1" class="app_select2 form-control"  title="Airline Name" name="airline_name-1" style="width: 200px;" data-add-new-option="true">
				...
				$offset = "_u";
				$count = 0;
				$sq_q_plane = mysqlQuery("select * from flight_quotation_plane_entries where quotation_id='$quotation_id'");
				while($row_q_plane = mysqli_fetch_assoc($sq_q_plane)){
					...
					<tr>
						...
						<td><select id="airline_name-<?= $offset.$count ?>_d" class="app_select2 form-control" style="width:160px" title="Airline Name" name="airline_name-<?= $offset.$count ?>_d" data-add-new-option="true">
			            ...
			            <td><input type="text" id="txt_dapart-<?= $offset.$count ?>_d" name="txt_dapart-<?= $offset.$count ?>_d" style="width:160px" class="app_datetimepicker" placeholder="*Departure Date & Time" title="Departure Date & Time" onchange="get_to_datetime(this.id,'txt_arrval-<?= $offset.$count ?>_d')" value="<?= get_datetime_user($row_q_plane['dapart_time']) ?>" /></td>
			            <td><input type="text" style="width:160px" id="txt_arrval-<?= $offset.$count ?>_d" name="txt_arrval-<?= $offset.$count ?>_d" class="app_datetimepicker" onchange="validate_validDatetime('txt_dapart-<?= $offset.$count ?>_d','txt_arrval-<?= $offset.$count ?>_d')" placeholder="*Arrival Date Time" title="Arrival Date Time" value="<?= get_datetime_user($row_q_plane['arraval_time']) ?>" /></td>
		            ...
		            <script>
		            	$('#txt_arrval-<?= $offset.$count ?>_d, #txt_dapart-<?= $offset.$count ?>_d').datetimepicker({ format:'d-m-Y H:i' });
...
$('select[id^="airline_name-"]').each(function() {
    $(this).select2();
});
