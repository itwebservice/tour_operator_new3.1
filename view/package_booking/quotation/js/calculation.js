function get_auto_values(
	...
	if (discount_amt === '' || discount_amt === undefined) discount_amt = 0;
	...
	if (type === 'save') {
		...
		var discount_in = $('#discount_in-' + offset).val();
		// var discount_amt = $('#discount_amt-' + offset).val();
	}
	else {
		var service_tax_subtotal = 'service_tax_subtotal-' + offset;
	...
	if (type === 'save') quotation_cost_calculate(sub_total);
	else quotation_cost_calculate1(sub_total);
}
...
function get_tax_charges(
	...
	var offset = basic_amountid.split('-');
	
	if (type === 'save') {
		var service_tax_subtotal = 'service_tax_subtotal-' + offset[1];
		var service_tax_markup = 'service_tax_markup-' + offset[1];
		var tds = 'tds';
				...
				if (calculation_mode !== '"Exclusive"') {
					if (tax_for === 'service_charge') {
						if (target_amount === 'Service Charge') {
							...
							$('#service_show-' + offset[1]).html('Inclusive Amount : <span>' + new_service_charge + '</span>');
							...
							$('#markup_show-' + offset[1]).html('Inclusive Amount : <span>' + new_service_charge + '</span>');
							$('#markup_show').html('Inclusive Amount : <span>' + new_service_charge + '</span>');
						}
						else if (target_amount === 'Basic') {
							// $('#' + basic_amountid).val(new_service_charge);
							$('#basic_show-' + offset[1]).html('Inclusive Amount : <span>' + new_service_charge + '</span>');
					...
					if (tax_for === 'service_charge') {
						if (target_amount === 'Service Charge') {
							...
							$('#' + markupid).val(new_service_charge);
							$('#markup_show-' + offset[1]).html('&nbsp;');
							$('#markup_show').html('&nbsp;');
						}
						else if (target_amount === 'Basic') {
							$('#' + basic_amountid).val(new_service_charge);
							$('#basic_show-' + offset[1]).html('&nbsp;');
							$('#basic_show').html('&nbsp;');
						}
						$('#' + service_tax_subtotal).val(applied_taxes);
						$('#hotel_taxes').val(ledger_posting);
						...
						condition.forEach((cond) => {
							...
							switch (condition) {
								...
								case '5':
									...
									break;
								case '8':
									if (value == 'Bus' || value == 'All') flag = true;
									break;
								case '3':
									switch (for1) {
										case '!=':
...
function calculate_charges(rules_array, type, basic_amount, markup_amount1) {
	...
	if (rules_array.length) {
		...
		if (type === 'save') {
			...
			var child_wob = $('#children_without_bed').val();
