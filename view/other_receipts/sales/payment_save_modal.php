...
<div class="modal fade" id="s_payment_save_modal" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
      ...
      <div class="modal-body">
            ...
            <legend>Receipt Details</legend>
              ...
              <div class="row mg_bt_10">                      
                ...
                <div class="col-md-4">
                  <input type="text" id="payment_amount" name="payment_amount" class="form-control" placeholder="*Amount" title="Amount" onchange="validate_balance(this.id);get_credit_card_charges('identifier','payment_mode','payment_amount','credit_card_details','credit_charges');">
                </div>
              </div>
              <div class="row mg_bt_10">
              ...
              <div class="row">
               ...
               <div class="col-md-4">
                            <select name="currency_code" id="currency_code1" title="Currency"  class="form-control" style="width:100% !important;" data-toggle="tooltip" required>
                                <?php
                ...
                <div class="col-md-8 col-sm-9">
                  <span style="color: red;line-height: 35px;" data-original-title="" title="" class="note">Note : Please make sure Date, Amount, mode, Creditor bank entered properly.</span>
                </div>
              </div>
...
$('#cust_id_s,#currency_code1').select2();
...
$(function(){
  $('#frm_sale_payment_save1').validate({
      ...
      submitHandler:function(form){
          ...
          if(typeof credit_note_amount === 'undefined') { credit_note_amount = '0'; }
          if(advance_nullify == "" || typeof advance_nullify === 'undefined') { advance_nullify = '0'; }

          if(payment_mode=="Credit Note" && credit_note_amount != ''){
            if(parseFloat(payment_amount) > parseFloat(credit_note_amount)) { 
          ...
          if(parseFloat(advance_nullify) > 0 || parseFloat(advance_nullify) != '' || parseFloat(advance_nullify) != 0){
            ...
            if(parseFloat(payment_amount)>0){;
              $('#payment_save').prop('disabled',false);
              error_msg_alert("Please release payment either from Advances or Receipt Details!");
              return false; 
            }
          ...
          if(parseFloat(advance_amount) < parseFloat(advance_nullify)){ 
           ...
           if(parseFloat(total_purchase) < parseFloat(payment_amount)) {
              error_msg_alert("Amount  should not be more than Total Outstanding ");
              $('#payment_save').prop('disabled',false);
              return false; 
          }
