...
$query = mysqli_fetch_assoc(mysqlQuery("select * from hotel_availability_request where request_id='$request_id'"));

$cart_data = ($query['cart_data']!='') ? json_decode($query['cart_data']) : [];
$response = ($query['response']!='') ? json_decode($query['response']) : [];
?>
      ...
      <div class="modal-content">
          ...
          <input type="hidden" value='<?= json_encode($cart_data) ?>' id="cart_data"/>
              ...
              for($i=0;$i<sizeof($cart_data);$i++){
                ...
                if(isset($item->status) && $item->status == 'Available') $ava_status = 'checked'; else $ava_status='';
                if(isset($item->status) && $item->status == 'Not Available') $nava_status = 'checked'; else $nava_status='';
              ?>
                <div class="row no-gutters">
                  <div class="col-md-12">
                    ...
                    <?php if(isset($item->confirmation_no) && $item->confirmation_no!=''){?><p style="margin-bottom: 0px;"><b>Confirmation No</b>: <?= $item->confirmation_no ?></p><?php }?>
                    <?php if(isset($item->updated_by) && $item->updated_by!=''){?><p style="margin-bottom: 0px;"><b>Updated By</b>: <?= $item->updated_by ?></p><?php }?>
                    <?php if(isset($item->note) && $item->note!=''){?><p class="no-pad"><b>Note</b>: <?= $item->note ?></p><?php } ?>
                    
                    <div class="row mg_tp_10"><div class='col-md-12'>
                      <div id="optional_hotel_section<?=$i?>">
                        <?php if(isset($item->status) && $item->status == 'Not Available' && sizeof($item->options)>0){ ?>
                        ...
                        <?php }
                        else if(isset($item->status) && $item->status == 'Not Available' && sizeof($item->options)==0){?>
                          <div class="panel panel-default panel-body app_panel_style feildset-panel mg_tp_30">
                              <legend>Select Similar Hotel Options</legend>
                              <div class="row text-right mg_bt_10">
                                  <div class="col-xs-12">
