...
$sq_email_no = mysqlQuery($query1);
	
while($row_email_no = mysqli_fetch_assoc($sq_email_no)){
	$count++;
	
	$group_name = "";
	$sq_group_entries = mysqlQuery("select * from email_group_entries where email_id_id='$row_email_no[email_id_id]'");
	while($row_group_entry = mysqli_fetch_assoc($sq_group_entries)){
		$email_group_id = $row_group_entry['email_group_id'];
		
		$sq_sms_group = mysqli_fetch_assoc(mysqlQuery("select * from email_group_master where email_group_id='$email_group_id'"));
