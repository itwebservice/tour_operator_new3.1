...
if($supplier_type!=""){
    $sq_supp = mysqli_fetch_assoc(mysqlQuery("select * from other_vendors where vendor_id='$supplier_type'"));   
}
if($expense_type!=""){
    $sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$expense_type'"));
}
// Add some data
...
$count = 0;
    ...
    $sq_paid = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from other_expense_payment_master where clearance_status!='Cancelled'"));
    $sq_canc = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as cancel from other_expense_payment_master where clearance_status='Cancelled'"));
    $sq_pend = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as pending from other_expense_payment_master where clearance_status='Pending'"));
    
    if($supplier_type!=""){
        $sq_paid = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from other_expense_payment_master where clearance_status!='Cancelled' and supplier_id='$supplier_type'"));
        $sq_canc = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as cancel from other_expense_payment_master where clearance_status='Cancelled' and supplier_id='$supplier_type'"));
        $sq_pend = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as pending from other_expense_payment_master where clearance_status='Pending' and supplier_id='$supplier_type'"));
    }
    if($expense_type!=""){
        $sq_paid = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from other_expense_payment_master where clearance_status!='Cancelled' and expense_type_id='$expense_type'"));
        $sq_canc = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as cancel from other_expense_payment_master where clearance_status='Cancelled' and expense_type_id='$expense_type'"));
        $sq_pend = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as pending from other_expense_payment_master where clearance_status='Pending' and expense_type_id='$expense_type'"));
    }
...
header('Content-Disposition: attachment;filename="Other Expense Payment('.date('d-m-Y H:i').').xls"');
