...
$sq_payment = mysqli_fetch_assoc(mysqlQuery("select * from other_expense_payment_master where payment_id='$payment_id'"));

$sq_est = mysqli_fetch_assoc(mysqlQuery("select sum(total_fee) as net_total from other_expense_master where expense_type_id='$sq_payment[expense_type_id]' and supplier_id='$sq_payment[supplier_id]'"));

$sq_paid = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as payment_amount from other_expense_payment_master where expense_type_id='$sq_payment[expense_type_id]' and supplier_id='$sq_payment[supplier_id]'"));
...
$sq_expense = mysqli_fetch_assoc(mysqlQuery("select * from other_vendors where vendor_id='$sq_payment[supplier_id]'"));
$sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$sq_payment[expense_type_id]'"));
      ...
      <div class="modal-body">
        ...
        <input type="hidden" id="old_bank_id" name="old_bank_id" value="<?= $sq_bank['bank_id'] ?>">
          ...
          <div class="row text-center">
              <div class="col-md-12">
                <button class="btn btn-sm btn-success" id="payment_update"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Update</button>
              </div>
          </div>
...
$(function(){
  $('#frm_vendor_payment_update').validate({
      ...
      submitHandler:function(form){
            ...
            $('#payment_update').prop('disabled',true);
            ...
            if(payment_amount > payment_old_value){
              ...
              if(parseFloat(balance_paying) > parseFloat(balance_amount)) { error_msg_alert('Payment should not be more than expense amount!');
              $('#payment_update').prop('disabled',false); return false; }
            }              
