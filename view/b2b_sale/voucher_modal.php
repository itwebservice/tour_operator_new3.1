...
$booking_data = mysqli_fetch_assoc(mysqlQuery("SELECT * from b2b_booking_master WHERE booking_id = '$booking_id'"));
...
<?php
      $timing_slots_exc = mysqli_fetch_assoc(mysqlQuery("select * from  excursion_master_tariff where entry_id = ".$cart_checkout_data[$i]->service->service_arr[0]->id));
