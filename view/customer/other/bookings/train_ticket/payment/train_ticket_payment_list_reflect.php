...
<div class="row mg_tp_20">
	<div class="col-md-12">
		<div class="table-responsive">
			<table class="table table-bordered bg_white cust_table" id="table_train" style="margin:20px 0 !important;">
				<thead>
				...
				<tbody>
					...
					while ($row_train_ticket_payment = mysqli_fetch_assoc($sq_train_ticket_payment)) {
						...
						$count++;
						$sq_train_ticket_info = mysqli_fetch_assoc(mysqlQuery("select * from train_ticket_master where train_ticket_id='$row_train_ticket_payment[train_ticket_id]' and delete_status='0'"));
						...
						$url1 = BASE_URL . "model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=yes&outstanding=$outstanding&table_name=train_ticket_payment_master&customer_field=train_ticket_id&in_customer_id=$row_train_ticket_payment[train_ticket_id]&status=$row_train_ticket_payment[status]&branch_admin_id=$sq_train_ticket_info[branch_admin_id]";
					...
					?>
						...
						<tr class="<?= $bg ?>">
							...
							<td class="success"><?= number_format($row_train_ticket_payment['payment_amount'] + $row_train_ticket_payment['credit_charges'], 2) ?></td>
							<td>
				...
				<tfoot>
					<tr class="active">
						...
						<th colspan="2" class="active text-right danger">Cancelled : <?= number_format($sq_cancel_amount, 2) ?></th>
