  ...
  <div class="row">
      <div class="col-sm-4 col-sm-offset-4">
          <select id="cmb_traveler_id" name="cmb_traveler_id" title="Passenger Name" style="width:100%;" onchange="traveler_id_proof_info_reflect()" class="form-control" title="Passenger">
              ...
              <?php
                  $query = "select * from tourwise_traveler_details where customer_id='$customer_id' and delete_status='0' and tour_group_status != 'Cancel' order by traveler_group_id desc";
                  $sq_tourwise_traveler_det = mysqlQuery($query);
                  while($row_tourwise_traveler_details = mysqli_fetch_assoc( $sq_tourwise_traveler_det ))
                  {
                    $pass_count = mysqli_num_rows(mysqlQuery("select traveler_id from travelers_details where traveler_group_id='$row_tourwise_traveler_details[traveler_group_id]'"));
                    $cancelpass_count = mysqli_num_rows(mysqlQuery("select traveler_id from travelers_details where traveler_group_id='$row_tourwise_traveler_details[traveler_group_id]' and status='Cancel'"));
                    if($pass_count != $cancelpass_count){
                      $yr = explode("-", $row_tourwise_traveler_details['form_date']);
                      $year = $yr[0];
                      $sq_travelers_details = mysqlQuery("select traveler_id, m_honorific, first_name, last_name from travelers_details where traveler_group_id='$row_tourwise_traveler_details[traveler_group_id]' and status='Active' "); 
...
function traveler_id_proof_info_reflect()
{
  ...
  $.post('bookings/group_booking/id_proof/traveler_id_proof_info_reflect.php', { traveler_id : traveler_id }, function(data){
