...
$query = "select * from refund_traveler_cancelation where tourwise_traveler_id in (select id from tourwise_traveler_details where customer_id='$customer_id' and delete_status='0')";
...
<table class="table table-bordered table-hover mg_bt_0 bg_white" id="group_table3" style="margin: 20px 0;">
    ...
    $sq_paid_amount=0;
    $Total_payment=0;
    $date; $paid_amt = 0; $footer_paid_total = 0; $footer_pending_total = 0; $footer_canc_total = 0;$sq_can_amount = 0;
    $sq_refund = mysqlQuery($query);
    while($row_refund = mysqli_fetch_assoc($sq_refund))
    {
        $count++;
        $sq_booking_c = mysqli_num_rows(mysqlQuery("select * from tourwise_traveler_details where id='$row_refund[tourwise_traveler_id]' and delete_status='0'"));
        if($sq_booking_c != 0){
            $sq_booking = mysqli_fetch_assoc(mysqlQuery("select * from tourwise_traveler_details where id='$row_refund[tourwise_traveler_id]' and delete_status='0'"));
            ...
            else if($row_refund['clearance_status']=="Cleared"){ $bg='success';
                $bg = '';
