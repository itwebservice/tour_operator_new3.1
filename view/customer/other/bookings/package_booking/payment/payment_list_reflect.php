        ...
        <tbody>
          ...
          while ($row = mysqli_fetch_assoc($sq)) {
            ...
            if ($pass_count == $cancle_count) {
              $sq_esti_count = mysqli_num_rows(mysqlQuery("select booking_id from package_refund_traveler_estimate where booking_id='$row[booking_id]'"));
              $sq_esti = mysqli_fetch_assoc(mysqlQuery("select cancel_amount from package_refund_traveler_estimate where booking_id='$row[booking_id]'"));
              $canc_amount = ($sq_esti_count > 0) ? $sq_esti['cancel_amount'] : 0;
            ...
            } else if ($row['clearance_status'] == "Cleared" && ($row['payment_mode'] == "Cheque" || $row['payment_mode'] == "Credit Card")) {
              $bg = 'success';
            } else {
              $bg = '';
            }
