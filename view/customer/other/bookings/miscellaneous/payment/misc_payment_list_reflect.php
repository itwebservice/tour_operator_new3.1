...
<table class="table table-bordered bg_white cust_table" id="miscellaneous_payment_list" style="margin:20px 0 !important">
	...
	<tbody>
		...
		$sq_miscellaneous_payment = mysqlQuery($query);
		

		$sq_cancel_pay=mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from miscellaneous_payment_master where clearance_status='Cleared'"));
	
		$sq_pend_pay=mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from miscellaneous_payment_master where clearance_status='Pending'"));

		while($row_miscellaneous_payment = mysqli_fetch_assoc($sq_miscellaneous_payment)){
			...
			$sq_customer_info = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_miscellaneous_info[customer_id]'"));
			$bg='';
			...
			$url1 = BASE_URL."model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=yes&outstanding=$outstanding&table_name=miscellaneous_payment_master&customer_field=misc_id&in_customer_id=$row_miscellaneous_payment[misc_id]&status=$row_miscellaneous_payment[status]&branch_admin_id=$sq_miscellaneous_info[branch_admin_id]";
	...
	<tfoot>
		<tr class="active">
			...
			<th colspan="2" class="danger text-right">Cancelled : <?= number_format($sq_cancel_amount,2) ?></th>
