...
$sq_exc = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master_tariff where entry_id='$entry_id'"));
$images_url = '';
															...
															<div class="col-md-12">
																<div class="table-responsive">
																	<table id="table_exc_tarrif_offer" name="table_exc_tarrif_offer" class="table table-bordered no-marg pd_bt_51" style="width:100%">
																		<?php
																		$sq_ocount = mysqli_num_rows(mysqlQuery("select * from excursion_master_offers where exc_id='$entry_id'"));
																		if ($sq_ocount == 0) {
																		?>
																			<tr>
																				...
																				<td><input type="text" id="from_date" class="form-control" name="from_date" placeholder="Valid From" title="Valid From" value="<?= date('d-m-Y') ?>" style="width: 120px;" onchange="get_to_date(this.id,'to_date')" /></td>
																		...
																		<?php } else { ?>
																			<?php $count = 1;
																			$sq_offer = mysqlQuery("select * from excursion_master_offers where exc_id='$entry_id'");
																			while ($row_offer = mysqli_fetch_assoc($sq_offer)) { ?>
																				<tr>
																					...
																					<td><input type="text" id="from_date-u" <?= $count ?> class="form-control" name="from_date-u" placeholder="Valid From" title="Valid From" value="<?= get_date_user($row_offer['from_date']) ?>" style="width: 120px;" onchange="get_to_date(this.id,'to_date-u<?= $count ?>')" /></td>
																					<td><input type="text" id="to_date-u<?= $count ?>" class="form-control" name="to_date" placeholder="Valid To " title="Valid To" onchange="validate_validDate('from_date-u<?= $count ?>','to_date-u<?= $count ?>')" value="<?= get_date_user($row_offer['to_date']) ?>" style="width: 120px;" /></td>
																				...
																				<script>
																					$('#from_date-u<?= $count ?>,#to_date-u<?= $count ?>').datetimepicker({
																						timepicker: false,
																						format: 'd-m-Y'
																					});
									...
									<div class="panel panel-default main_block">
										<div class="panel-heading main_block" role="tab" id="heading3">
											<div class="Normal collapsed main_block" role="button" data-toggle="collapse"
												...
												aria-controls="collapse3" id="collapsed1">
												<div class="col-md-12"><span><em style="margin-left: 15px;"><?php echo "Additional information"; ?></em></span>
												</div>
											</div>
										</div>
...
<script>
	...
	$('#to_date,#from_date,#to_date_c,#from_date_c,#from_date_basic,#to_date_basic').datetimepicker({
		timepicker: false,
		format: 'd-m-Y'
	});
	...
	function upload_user_pic_attch1() {
	...
	function upload_img() {
		...
		var upload_url = $('#photo_upload_url_i1').val();
		var exc_entry_id = $('#exc_entry_id').val();
		$.ajax({
			type: 'post',
			url: base_url + 'controller/b2b_excursion/img_update.php',
			data: {
				upload_url: upload_url,
				exc_entry_id: exc_entry_id
			},
	...
	$(function() {
		$('#frm_b2b_exc_update').validate({
			...
			submitHandler: function(form) {
				...
				var off_days = '';
				$('#off_days').find('option:selected').each(function() {
					off_days += $(this).attr('value') + ',';
				});
				off_days = off_days.substring(0, off_days.length - 1);

				if (city_id == '') {
					error_msg_alert('Select City in Activity Information!');
					return false;
				}
				if (service_name == '') {
					error_msg_alert('Enter Activity in Activity Information!');
					return false;
				}
				...
				if (bfrom_date_array.length == 0) {
					error_msg_alert('Please Enter Basic Costing in Costing Information!');
					return false;
				}
				...
				$.ajax({
					...
					data: {
						...
						taxation: JSON.stringify(taxation),
						active_flag: active_flag,
						transfer_option_array: transfer_option_array,
						bfrom_date_array: bfrom_date_array,
						bto_date_array: bto_date_array,
						...
						offer_in_array: offer_in_array,
						coupon_code_array: coupon_code_array,
						offer_array: offer_array,
						agent_type_array: agent_type_array,
						offer_entryid_array: offer_entryid_array,
						off_days: off_days,
