...
<div class="row text-right mg_bt_10">
    <div class="col-md-12">
        ...
        <button class="btn btn-info btn-sm ico_left mg_bt_10" data-toggle="modal" data-target="#payment_save_modal"><i
    ...
    <div class="row">
        ...
        <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
            <input type="text" id="payment_from_date_filter" name="payment_from_date_filter" placeholder="From Date"
                class="form-control" title="From Date"
                onchange="validate_validDate('payment_from_date_filter','payment_to_date_filter');get_to_date(this.id,'payment_to_date_filter');">
        </div>
        <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
            <input type="text" id="payment_to_date_filter" class="form-control" name="payment_to_date_filter"
...
function payment_list_reflect() {
    ...
    $.post('../payment/payment_list_reflect.php', {
        ...
        pagination_load(data, columns, true, true, 10, 'car_r_book', true);
        $('.loader').remove();
    });
}
...
function cash_bank_receipt_generate() {
    ...
    var url = base_url + "view/bank_receipts/car_rental_payment/cash_bank_receipt.php?payment_id_arr=" +
...
function cheque_bank_receipt_generate() {
    ...
    var url = base_url + "view/bank_receipts/car_rental_payment/cheque_bank_receipt.php?payment_id_arr=" +
...
function excel_report() {
    ...
    var payment_to_date = $('#payment_to_date_filter').val();
    var branch_status = $('#branch_status').val();
    window.location = '../payment/excel_report.php?booking_id=' + booking_id + '&payment_from_date=' +
        payment_from_date + '&payment_to_date=' + payment_to_date + '&financial_year_id=' + financial_year_id +
        '&payment_mode=' + payment_mode + '&branch_status=' + branch_status;
}

function whatsapp_send_r(booking_id, payment_amount, base_url) {
    $.post(base_url + 'controller/car_rental/payment/receipt_whatsapp_send.php', {
        booking_id: booking_id,
    ...
    $('#vi_confirm_box').vi_confirm_box({
        callback: function(data1) {
            if (data1 == "yes") {
                var branch_status = $('#branch_status').val();
                var base_url = $('#base_url').val();
                $.post(base_url + 'controller/car_rental/payment/payment_delete.php', {
                    payment_id: payment_id
                }, function(data) {
