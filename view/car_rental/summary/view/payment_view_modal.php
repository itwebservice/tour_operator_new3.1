          ...
          <div class="profile_box">
                <div class="table-responsive">
                    <table  class="table table-bordered no-marg">
                      ...
                      <?php 
                      $count = 0;
                      $sq_query = mysqlQuery("SELECT * FROM car_rental_payment WHERE booking_id = '$booking_id' and payment_amount!='0'");
                      while($row_entry = mysqli_fetch_assoc($sq_query))
                      {
                          ...
                          $bg='warning';
                           ...
                           $pkg_query=mysqli_fetch_assoc(mysqlQuery("select * from car_rental_booking  where booking_id='$booking_id'"));

                            $currency_amount2 = currency_conversion($currency,$pkg_query['currency_code'],$row_entry['payment_amount']);

                            if($pkg_query['currency_code'] !='0' && $currency != $pkg_query['currency_code'] && $quotation_cost != $currency_amount2){
                          ...
                          <tr class="<?= $bg ?>">
                              ...
                              <td><?php echo number_format($row_entry['payment_amount'],2).$currency_amount; ?></td>
                          </tr>  
                          <?php
                        }
                      ?>
