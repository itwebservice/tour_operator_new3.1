...
if($booking_id!=""){
  $sq_car_rental_info = mysqli_fetch_assoc(mysqlQuery("select * from bus_booking_master where booking_id='$booking_id' and delete_status='0'"));
...
$row_count++;
    ...
    while($row_refund = mysqli_fetch_assoc($sq_refund)){
      ...
      $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H'.$row_count, 'Pending : '.number_format($pending_total,2))
        ->setCellValue('I'.$row_count, 'Cancelled : '.number_format($cancelled_total,2))
        ->setCellValue('J'.$row_count, 'Total Refund : '.number_format(($total_refund - $pending_total - $cancelled_total),2));
...
header('Content-Disposition: attachment;filename="Bus Booking Refund('.date('d-m-Y H:i').').xls"');
