...
<div class="row text-right mg_bt_10">
    <div class="col-xs-12">
        ...
        <button class="btn btn-info btn-sm ico_left mg_bt_10" id="btn_save_modal" onclick="save_modal()"><i class="fa fa-plus" title="Bus Booking"></i>&nbsp;&nbsp;Bus</button>
    </div>
</div>
    ...
    <div class="row">
        ...
        <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
            ...
            <select name="booking_id_filter" id="booking_id_filter" style="width:100%" title="Booking ID">
                ...
                <?php get_bus_booking_dropdown($role, $branch_admin_id, $branch_status, $emp_id, $role_id, $financial_year_id) ?>
        ...
        <div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10">
            ...
            <input type="text" id="from_date_filter" name="from_date_filter" placeholder="From Date" title="From Date"
                onchange="get_to_date(this.id,'to_date_filter');">
...
function business_rule_load() {
    get_auto_values('balance_date', 'basic_cost', 'payment_mode', 'service_charge', 'markup', 'save', 'true', 'basic',
        'basic');
}
var columns = [{
        title: "Invoice_No"
...

{
    ...
    $.post('booking/list_reflect.php', {
        ...
        pagination_load(data, columns, true, true, 10, 'bus_book', true);
        $('.loader').remove();
    });
...

{
    ...
    window.location = 'booking/excel_report.php?customer_id=' + customer_id + '&booking_id=' + booking_id +
        '&from_date=' + from_date + '&to_date=' + to_date + '&cust_type=' + cust_type + '&company_name=' +
...

{
    ...
    var cust_type = $('#cust_type_filter').val();
    var branch_status = $('#branch_status').val();
    $.post('booking/company_name_load.php', {
        cust_type: cust_type,
        branch_status: branch_status
    }, function(data) {
...
function calculate_total_amount(offset = '') {
    ...
    if (service_tax_markup == "") {
        service_tax_markup = 0;
    }
    if (service_charge == "") {
        service_charge = 0;
    }
    ...
    var total_amount = parseFloat(basic_cost) + parseFloat(service_charge) + parseFloat(service_tax_amount) +
...

{
    ...
    $.get("booking/get_customer_dropdown.php", {
        cust_type: cust_type,
        company_name: company_name,
        branch_status: branch_status
    }, function(data) {
...
function whatsapp_send(emp_id, customer_id, booking_date, base_url, contact_no, email_id) {
    $.post(base_url + 'controller/bus_booking/booking/whatsapp_send.php', {
        emp_id: emp_id,
    ...
    $('#vi_confirm_box').vi_confirm_box({
        callback: function(data1) {
            if (data1 == "yes") {
                var branch_status = $('#branch_status').val();
                var base_url = $('#base_url').val();
                $.post(base_url + 'controller/bus_booking/booking/booking_delete.php', {
                    booking_id: booking_id
                }, function(data) {
