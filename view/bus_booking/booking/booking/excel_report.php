...
$count = 0;
        ...
        while($row_booking = mysqli_fetch_assoc($sq_booking)){
            $query = mysqli_fetch_assoc(mysqlQuery("SELECT sum(payment_amount) as sum,sum(credit_charges) as sumc from bus_booking_payment_master where booking_id='$row_booking[booking_id]' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
...
$paid_amount = ($paid_amount == '')?'0':$paid_amount;
			...
			$sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("SELECT sum(`credit_charges`) as sumc from bus_booking_payment_master where booking_id='$row_booking[booking_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
header('Content-Disposition: attachment;filename="Bus Booking('.date('d-m-Y H:i').').xls"');
