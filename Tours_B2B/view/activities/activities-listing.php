        ...
        if ($off_day) {
          ...
          if ($sq_count > 0) {
            ...
            while (($row_tariff_master  = mysqli_fetch_assoc($sq_tariff_master))) {
              ...
              array_push($all_orgcosts_array, array('orgamount' => (float)($total_cost1), 'id' => $currency_id));
              ...
              array_push($actual_ccosts_array, (float)($total_cost1));
            }
...
<script>
  ...
  function get_price_filter_data_result(final_arr) {
    var base_url = $('#base_url').val();
    $.post(base_url + 'Tours_B2B/view/activities/activity_results.php', {
      final_arr: final_arr
    }, function(data) {
