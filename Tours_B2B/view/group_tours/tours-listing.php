...
<div class="c-containerDark">
  ...
  <div class="container">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-12">
                      ...
                      <div class="form-group">
                        ...
                        <div id="seats_availability" class="m20-top"></div>
    ...
    <div class="row">
      ...
      if ($tour_group_id != '') {
        $query .= " and tour_id in(select tour_id from tour_groups where group_id = '$tour_group_id')";
      }
      ...
      if ($query != '') {
        ...
        while (($row_query  = mysqli_fetch_assoc($sq_query))) {
          ...
          if ($tour_group_id != '') {
            $query_dates = "select * from tour_groups where group_id = '$tour_group_id'";
          } else {
            $query_dates = "select * from tour_groups where tour_id = '$row_query[tour_id]'";
          }
          $sq_dates = mysqlQuery($query_dates);
          while ($row_dates = mysqli_fetch_assoc($sq_dates)) {

            $from_date = date("d-m-Y", strtotime($row_dates['from_date']));
            $to_date = date("d-m-Y", strtotime($row_dates['to_date']));

            $val = (int)date_diff(date_create(date("d-m-Y")), date_create($to_date))->format("%R%a");
            if ($val <= 0)  continue; // skipping the ended group tours (only used group quotation)

            $total_seats = $row_dates['capacity'];
            $seats_booked = $bk_seats1->booked_seats($row_dates['tour_id'], $row_dates['group_id']);
            $b2c_seats_booked = $bk_seats1->b2c_booked_seats($row_dates['tour_id'], $row_dates['group_id']);
            $b2b_seats_booked = $bk_seats1->b2b_booked_seats($row_dates['tour_id'], $row_dates['group_id']);

            $available_seats = $total_seats - $seats_booked - $b2c_seats_booked - $b2b_seats_booked;
            if ($available_seats <= 0)  continue; //Skipping if seats not available

            array_push($travel_date_array, array(
              'group_id' => $row_dates['group_id'],
              'total_seats' => $total_seats,
              'available_seats' => $available_seats,
              'tour_date' => date('d-m-Y', strtotime($from_date)) . ' To ' . date('d-m-Y', strtotime($to_date))
            ));
          }

          if (!empty($travel_date_array)) {
            //Group Costing
            ...
            array_push($tours_result_array, array(
              ...
              'pp_adult_cost' => $row_query['adult_cost'],
              'pp_child_without_cost' => $row_query['child_without_cost'],
              'pp_child_with_cost' => $row_query['child_with_cost'],
              'pp_with_bed_cost' => $row_query['with_bed_cost'],
              'pp_infant_cost' => $row_query['infant_cost'],
              'pp_extrabed_cost' => $with_bed_cost_total,
...
<script>
  ...
  // Get currency changed values in group tour result
  ...
  function get_price_filter_data_result(final_arr) {
    var base_url = $('#base_url').val();
    $.post(base_url + 'Tours_B2B/view/group_tours/tours_results.php', {
      final_arr: final_arr
    }, function(data) {
  ...
  function getSliderValue() {
    ...
    if (ranges[0] != '' && ranges[1] != '' && ranges[0] !== 'NaN' && ranges[1] !== 'NaN') {
      get_price_filter_data('tours_result_array', '2', ranges[0], ranges[1]);
    }
  }
  ...
  var a = setTimeout(function() {
    ...
    sessionStorage.setItem('gtours_best_price', JSON.stringify(bestAmount_arr));
  }, 100);
</script>