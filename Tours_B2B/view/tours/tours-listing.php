...
$pax = intval($tours_array[0]->adult) + intval($tours_array[0]->child_wobed) + intval($tours_array[0]->child_wibed) + intval($tours_array[0]->infant);
...
//Hotel Search
if ($tour_id != '') {
  $sq_tour = mysqli_fetch_assoc(mysqlQuery("select package_id, package_name from custom_package_master where package_id='$tour_id' and status!='Inactive'"));
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      while (($row_query  = mysqli_fetch_assoc($sq_query))) {
        ...
        $sq_terms = mysqli_fetch_assoc(mysqlQuery("select terms_and_conditions from terms_and_conditions where type = 'Package Quotation' and active_flag!='Inactive'"));
        ...
        $coupon_flag = false;
        $coupon_array = array();
        $sq_offers_count = mysqli_num_rows(mysqlQuery("select * from custom_package_offers where (from_date <='$date1' and to_date>='$date1') and package_id='$package_id'"));
        if ($sq_offers_count == 1) {
          $row_offers = mysqli_fetch_assoc(mysqlQuery("select * from custom_package_offers where (from_date <='$date1' and to_date>='$date1') and package_id='$package_id'"));
          ...
          if ($row_offers['type'] == "Coupon") {
            array_push($coupon_array, array(
              'offer_type' => $row_offers['type'],
              'offer_in' => $row_offers['offer_in'],
              'offer_amount' => $row_offers['offer_amount'],
              'coupon_code' => $row_offers['coupon_code'],
        ...
        } else if ($sq_offers_count > 1) {

          $sq_offers_count = mysqli_num_rows(mysqlQuery("select * from custom_package_offers where (from_date <='$date1' and to_date>='$date1') and package_id='$package_id' and type='Offer'"));
          if ($sq_offers_count > 0) {

            $row_offers = mysqli_fetch_assoc(mysqlQuery("select * from custom_package_offers where (from_date <='$date1' and to_date>='$date1') and package_id='$package_id' and type='Offer'"));
            $offer_type = $row_offers['type'];
            $offer_in = $row_offers['offer_in'];
            $offer_amount = $row_offers['offer_amount'];
            $coupon_code = $row_offers['coupon_code'];
            ...
            $coupon_flag = true;
            $sq_offers = mysqlQuery("select * from custom_package_offers where (from_date <='$date1' and to_date>='$date1') and package_id='$package_id' and type='Coupon'");
        ...
        $total_cost1 = 0;
        $sq_tariff = mysqlQuery("select * from custom_package_tariff where (`from_date` <= '$date1' and `to_date` >= '$date1') and (`min_pax` <= '$costing_pax' and `max_pax` >= '$costing_pax') and `package_id`='$row_query[package_id]'");
        while ($row_tariff = mysqli_fetch_assoc($sq_tariff)) {

          $total_cost1 = ($adult_count * (float)($row_tariff['badult'])) + ($child_wocount * (float)($row_tariff['bcwob'])) + ($child_wicount * (float)($row_tariff['bcwb'])) + ($extra_bed * (float)($row_tariff['bextra'])) + ($infant_count * (float)($row_tariff['binfant']));
          ...
          array_push($all_costs_array, array('amount' => (float)($total_cost1), 'id' => $currency_id));
          $c_amount1 = ($to_currency_rate != '') ? $from_currency_rate1 / $to_currency_rate * $total_cost1 : 0;
        ...
        if ($total_cost1 != 0) {
          array_push($actual_ccosts_array, $total_cost1);
...
<script>
  ...
  function get_price_filter_data_result(final_arr) {
    var base_url = $('#base_url').val();
    $.post(base_url + 'Tours_B2B/view/tours/tours_results.php', {
      final_arr: final_arr
    }, function(data) {
