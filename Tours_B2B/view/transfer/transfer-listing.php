...
$duration = $pick_drop_array[0]->duration;
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                      <div class="form-group">
                        ...
                        <div class="selector">
                          <select name="duration" id="duration" title="*Service Duration" data-toggle="tooltip" class="full-width">
                            <?php
                            $row = mysqli_fetch_assoc(mysqlQuery("select entry_id,duration from service_duration_master where entry_id='$duration'"));
                            ?>
        ...
        while (($row_query  = mysqli_fetch_assoc($sq_query))) {
          ...
          while ($row_tariff_master = mysqli_fetch_assoc($sq_tariff_master)) {
            ...
            for ($i_date = 0; $i_date < sizeof($checkDate_array); $i_date++) {
              ...
              if ($i_date == 0) {
                $query11 = "select * from b2b_transfer_tariff_entries where tariff_id='$row_tariff_master[tariff_id]' and service_duration = '$duration' and pickup_location = '$pickup_id' and drop_location = '$drop_id' and pickup_type='$tpick_up_type' and drop_type='$tdrop_type' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') order by tariff_entries_id desc";
              } else {
                if ($pick_drop_array[0]->trip_type == 'roundtrip')
                  $query11 = "select * from b2b_transfer_tariff_entries where tariff_id='$row_tariff_master[tariff_id]' and service_duration = '$duration' and pickup_location = '$drop_id' and drop_location = '$pickup_id' and pickup_type='$tpick_up_type' and drop_type='$tdrop_type' and (from_date <='$checkDate_array[$i_date]' and to_date>='$checkDate_array[$i_date]') order by tariff_entries_id desc";
              }
              if ($query11) {
                ...
                if ($row_tariff_count > 0) {
                  ...
                  $vehicle_count = ceil(intval($passengers) / intval($max_pax));
                  $total_cost = $tariff_data[0]->total_cost;
                  $markup_amount = $tariff_data[0]->markup_amount;

                  if ($tariff_data[0]->markup_in == 'Flat') {
                    $total_cost += (float)($markup_amount);
                  } else {
                    $total_cost = (float)($total_cost) + ((float)($total_cost) * (float)($markup_amount) / 100);
                  }
                  ...
                  if ($total_cost1 != '' && (float)($total_cost1) != 0) {
        ...
        if ($pick_drop_array[0]->trip_type == 'roundtrip') {
          ...
          for ($i = 0; $i < sizeof($transfer_result_array); $i++) {
            ...
            array_push($final_result_array, array(
              ...
              "best_lowest_cost" => array(
                'id' => $transfer_result_array[$i]['best_lowest_cost']['id'],
                'cost' => $total_cost
              )
            ));
          }
...
<script>
  ...
  function get_price_filter_data_result(final_arr) {
    var base_url = $('#base_url').val();
    $.post(base_url + 'Tours_B2B/view/transfer/transfer-results.php', {
      final_arr: final_arr
    }, function(data) {
