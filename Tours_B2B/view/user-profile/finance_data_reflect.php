...
if($agent_flag == '1'){
	...
	$query = "(select * from finance_transaction_master where module_name='B2B Booking' and payment_amount!='0' and module_entry_id in(select booking_id from b2b_booking_master where agent_flag='$agent_flag' and user_id='$user_id')";
}
...
$query .= "UNION ALL (select * from finance_transaction_master where module_name LIKE '%B2B Booking%' and payment_amount!='0' and module_entry_id in(select entry_id from b2b_payment_master where booking_id in(select booking_id from b2b_booking_master where customer_id='$customer_id')) and type in('CASH RECEIPT','BANK RECEIPT')";
if($from_date!="" && $to_date !=""){
	$from_date = date('Y-m-d',strtotime($from_date));
	$to_date = date('Y-m-d',strtotime($to_date));
	$query .=" and (payment_date>='$from_date' and payment_date<='$to_date') ";
