...
if($agent_flag == '1'){
	...
	$query = "select * from b2b_quotations where agent_flag='$agent_flag' and user_id='$user_id'";
}
...
while($row_quotation = mysqli_fetch_assoc($sq_quotation)){
	...
	if($row_quotation['agent_flag'] == '0'){
		...
		$sq_agent = mysqlI_fetch_assoc(mysqlQuery("select company_name from  b2b_registration where register_id='$row_quotation[user_id]'"));
