...
if(sizeof($cart_list_arr)>0 && $cart_list_arr[0]){
    ...
    for($i=0;$i<sizeof($cart_list_arr);$i++){
        ...
        if($cart_list_arr[$i]['service']['name'] == 'Group Tours'){
            array_push($group_tours_list_arr,$cart_list_arr[$i]);
        }
    ...
    if(sizeof($group_tours_list_arr) >0){
        for($i=0;$i<sizeof($group_tours_list_arr);$i++){
        $image = ($group_tours_list_arr[$i]['service']['name']=='Group Tours')?'itours-sun-umbrella':'';
        $tour_group = explode('=',$group_tours_list_arr[$i]['service']['service_arr'][0]['tour_group']);
        ?>
            ...
            <div class="c-cartItem">
                <i class="icon it <?= $image ?>"></i>
                <div class="clearfix ci-header">
                <span class="ci-title"><?= $group_tours_list_arr[$i]['service']['service_arr'][0]['tour_name'] ?></span>
                <div class="ci-btnRemove">
                    <button title='Remove' onclick="remove_item('<?= $group_tours_list_arr[$i]['service']['uuid'] ?>')"></button>
                </div>
                </div>

                <!-- **** Room Type **** -->
                <div class="clearfix ci-body">
                <span class="ci-subtitle"><?= $tour_group[1] ?></span>
                <div class="ci-cost">
                    <span class='currency-icon'></span>
                    <span class='cart-currency-price'><?= $group_tours_list_arr[$i]['service']['service_arr'][0]['total_cost']?></span>
                    <span class='c-hide cart-currency-id'><?= $group_tours_list_arr[$i]['service']['service_arr'][0]['currency_id'] ?></span>
                </div>
                </div>
                <!-- **** Room Type End **** -->
            </div>
