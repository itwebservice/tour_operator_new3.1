...
function add_to_cart (id,type){
	...
	if(type === "hotel"){
		...
		var duration = $('#service_duration').html();
	...
	else if(type === "GroupTours"){

		var image = $('#image-'+id).val();
		var tour_id = $('#tour_id-'+id).val();
		var dest_name = $('#dest_name-'+id).val();
		var tour_name = $('#tour-'+id).html();
		...
		var childwi = $('#child_wicount').val();
		var childwo = $('#child_wocount').val();
		var extra_bed = $('#extra_bed_count').val();
		var infant = $('#infant_count').val();
		var pp_adult_cost = $('#pp_adult_cost-'+id).val();
		var pp_child_without_cost = $('#pp_child_without_cost-'+id).val();
		var pp_child_with_cost = $('#pp_child_with_cost-'+id).val();
		var pp_with_bed_cost = $('#pp_with_bed_cost-'+id).val();
		var pp_infant_cost = $('#pp_infant_cost-'+id).val();
		var pp_extrabed_cost = $('#pp_extrabed_cost-'+id).val();
		...
		if (tour_group == '') {
			error_msg_alert('Please select at least one tour date!');
			return false;
		}

		var result = get_other_rules ('Group Tour', travel_date);
		var applied_taxes = get_service_tax(result);
		service_data_array.push({
			'id':id,
			'image':image,
			...
			'childwi':parseInt(childwi),
			'childwo':parseInt(childwo),
			'extra_bed':parseInt(extra_bed),
			'infant':parseInt(infant),
			'pp_adult_cost':parseFloat(pp_adult_cost),
			'pp_child_without_cost':parseFloat(pp_child_without_cost),
			'pp_child_with_cost':parseFloat(pp_child_with_cost),
			'pp_with_bed_cost':parseFloat(pp_with_bed_cost),
			'pp_infant_cost':parseFloat(pp_infant_cost),
			'pp_extrabed_cost':parseFloat(pp_extrabed_cost)
		});
		try {
			var cart_list_arr1 = JSON.parse(localStorage.getItem('cart_list_arr'));
			cart_list_arr1 = cart_list_arr1 !== null ? cart_list_arr1 : [];
...
function enq_to_action_page(pax, item_id,agent_flag,user_id) {

    var base_url = $('#base_url').val();
	var register_id = $('#register_id').val();
	var country_name = $('#country_name-'+item_id).html();
	...
	enq_data.push({
		...
		'visa_type':visa_type_arr[0],
		'pax':pax,
		'register_id':register_id,
		'agent_flag':agent_flag,
		'user_id':user_id
	});
	$.post(base_url + 'controller/visa_master/b2b_visa_enquiry.php', { enq_data: enq_data }, function(data) {
